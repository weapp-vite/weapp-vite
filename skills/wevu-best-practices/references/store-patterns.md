...
本次按 minor 发布，但包含需要旧 Store 消费者迁移的不兼容变化。请先阅读 [Store 迁移指南](https://vite.weapp.dev/wevu/store-migration)，再按下表检查消费者。推荐使用 `createPinia()` 创建管理器；旧名称 `createStore()` 仍可使用，通过应用安装、显式传参或测试中显式激活后使用：
