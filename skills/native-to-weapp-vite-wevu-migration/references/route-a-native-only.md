...
- 项目需要保留支付宝 `.axml/.acss`、抖音 `.ttml/.ttss` 或其他平台原生资产。
- 希望先建立可验证、可回滚的现代化基线，再决定是否进入路线 B。

## 必做事项

- 盘点目标平台、原生模板/样式/script module 扩展名、`app.json`、页面、分包、组件、插件、npm、云开发、CI 和各平台 IDE 配置。
...
- 多平台项目每次显式选择一个 `-p <platform>`；Web 只承担浏览器联调，不替代目标小程序 IDE/真机。
...
- 不重命名支付宝、抖音等原生文件来伪装微信资产，也不把 React 迁移并入路线 B。
