...
- [ ] 原生 `Page/Component`、WXML/WXSS/JSON 继续作为一等资产维护。
- [ ] 新增原生页面和组件的目录、脚本、路由、分包和 DevTools 打开方式已写清楚。
- [ ] 不因接入 `weapp-vite` 强制引入 `wevu` 或 `.vue`。
- [ ] 关键链路至少完成构建、打开、日志或截图验收。
- [ ] 记录未来进入路线 B 的触发条件和候选页面。
...
- [ ] 变更说明记录迁移前后行为对照。
- [ ] 已定义触发回滚的硬条件。
- [ ] 回滚点区分工具链接入、原生收口、试点页面、页面族迁移和语义升级。
