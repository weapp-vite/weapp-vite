...
description: 面向采用 weapp-vite monorepo 布局仓库的 GitHub issue 修复工作流。适用于修复已报告的 GitHub issue、在 `e2e-apps/github-issues` 中复现 bug、从隔离 worktree 准备 PR，或执行完整的“复现 -> 根因分析 -> 修 packages/weapp-vite|wevu|templates|website -> unit/e2e 覆盖 -> changeset -> PR”闭环。触发语句包括“修 GitHub issue”“复现这个 issue”“给这个 bug 补 github-issues case”“开 PR 修复”“按仓库流程修这个问题”等。
---
