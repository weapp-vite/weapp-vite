...
- 主要是项目级构建配置。使用 `weapp-vite-best-practices`。
- 主要是 `wevu` 生命周期和 store。使用 `wevu-best-practices`。
- 主要是迁移规划。使用 `native-to-weapp-vite-wevu-migration`。

## 快速开始

1. 先确认 SFC 是 App、Page 还是 Component。
2. 再选 JSON 宏。
3. 若类型提示或生成文件异常，先跑 `weapp-vite prepare`。
4. 若项目已安装 `weapp-vite`，优先读本地 `dist/docs/vue-sfc.md`。
...
- `v-model` 只能作用于可赋值左值
- 不要假设 Web Vue 所有模板特性都可用
- `usingComponents` 走 JSON 宏 / 自动导入，而不是 Web Vue 组件注册思路

4. 联动项目支持文件

- 若 `typed-router.d.ts`、`typed-components.d.ts`、`components.d.ts` 漂移，先跑：
  - `weapp-vite prepare`
- 若项目有根 `AGENTS.md`，SFC 写法要与其 AI 约束一致
