...
export const wevuCompatibilityCatalog: readonly WevuCompatibilityEntry[] = [
  ...
  entry({
    ...
    compatibility: 'supported',
    diagnostic: 'off',
    summary: '创建独立管理器，通过 app.use 或 use(pinia) 安装。',
  ...
  ...[
    ['defineStore', 'Setup/Options Store 的日常用法对齐 Pinia 4.0.3；不包含 Web SSR、HMR 与 devtools。'],
    ['storeToRefs', '提取响应式 state/getters，过滤方法和非响应式属性。'],
    ['$state', '访问两种 Store 的响应式状态，赋值通过 patch function 合并。'],
    ['$patch', '支持深层对象补丁或状态修改回调。'],
    ['$reset', 'Options Store 重新执行 state 工厂；Setup Store 自行实现。'],
    ['$subscribe', '默认按 watch 调度，并在注册作用域销毁时解绑；支持 detached。'],
    ['$onAction', '支持 detached 和 after/onError，已开始调用的结果回调不随退订取消。'],
    ['$dispose', '释放实例与副作用，保留管理器状态。'],
    ['getActivePinia', '读取当前注入或活动管理器。'],
    ['setActivePinia', '显式设置组件外的活动管理器。'],
    ['disposePinia', '释放管理器以及全部状态。'],
    ['install', '安装管理器并提供应用级注入。'],
    ['use', '注册 Pinia 风格的 Store 插件。'],
  ].map(([api, summary]) => entry({
