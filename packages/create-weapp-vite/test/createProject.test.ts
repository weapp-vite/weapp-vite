...
describe('createProject', () => {
  ...
  it('creates default template with resolved versions and gitignore rename', async () => {
    ...
    expect(pkgJson.overrides['@eslint-react/eslint-plugin']).toBe('2.13.0')
    expect(pkgJson.pnpm.overrides['@eslint-react/eslint-plugin']).toBe('2.13.0')
    expect(pkgJson.resolutions['@eslint-react/eslint-plugin']).toBe('2.13.0')
