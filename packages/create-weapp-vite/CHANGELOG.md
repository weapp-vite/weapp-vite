...
- 升级脚手架的 npm 配置、网络请求与子进程依赖至最新稳定版：@npmcli/config 11.1.0、make-fetch-happen 16.0.1、npm-registry-fetch 20.0.1、execa 10.0.1。Node.js 支持范围调整为 ^22.22.2 || ^24.15.0 || >=26.0.0，不再支持 Node.js 20；请先升级 Node.js 再运行脚手架。
...
- 为原生模板的 admin 布局声明标题和副标题属性，使 setPageLayout 传入的内容在真实微信界面中生效；为 Wevu 计数示例提供稳定节点标识以支持自动验收。
...
- 📦 **Dependencies** [`5fab295`](https://github.com/weapp-vite/weapp-vite/commit/5fab295bf59c714e39ebae7c617040c93692650b)
  → `@weapp-core/shared@3.1.1`, `@weapp-core/init@6.0.14`
...
- 🐛 **移除 wevu TDesign 模板中对小程序宿主无实际作用的系统字体族声明，生成的项目将直接使用宿主默认字体。** [`6a0a908`](https://github.com/weapp-vite/weapp-vite/commit/6a0a9084df84dc7a476fe1a50aa30634ab41ca4a) by @sonofmagic
...
- 📦 **Dependencies** [`75e79fb`](https://github.com/weapp-vite/weapp-vite/commit/75e79fb08afc3165ff66fefeffe42623ffb48d62)
  → `@weapp-core/shared@3.0.5`, `@weapp-core/init@6.0.11`
...
- 🐛 **模板项目的微信开发者工具私有配置默认关闭 URL 校验，便于本地调试 WebSocket、请求客户端和局域网接口场景。** [`dbe5f28`](https://github.com/weapp-vite/weapp-vite/commit/dbe5f28965a846e8750289d026ceebceca88028e) by @sonofmagic
...
- 🐛 **允许通过 `npx create-weapp-vite init` 执行已有项目初始化，复用 `@weapp-core/init` 的迁移能力，避免用户为了运行 init 单独下载体积更大的 `weapp-vite` 包。** [`c899b38`](https://github.com/weapp-vite/weapp-vite/commit/c899b381ef2d211c8e99ca52879330055dae22a4) by @sonofmagic
...
- 🐛 **修复脚手架模板同步与 smoke 校验会误包含 `dist-*` 生成产物的问题，避免本地构建残留进入新创建项目或 npm 包。** [`f3a8498`](https://github.com/weapp-vite/weapp-vite/commit/f3a8498af1db3b78fb9d8afc2d9a11626e0ab112) by @sonofmagic
...
- 🐛 **将 Tailwind 模板迁移到 `weapp-tailwindcss` v5 构建链路，移除安装后 patch 和小程序侧重复的 Tailwind 官方生成插件，改由 `WeappTailwindcss` 统一接管 CSS 生成。** [`4f46d45`](https://github.com/weapp-vite/weapp-vite/commit/4f46d45e2ba70844212b8d011383002c90b2b86b) by @sonofmagic
- 📦 **Dependencies** [`700b1f9`](https://github.com/weapp-vite/weapp-vite/commit/700b1f9c4d576bb754c30789d335844eb6fcddfa)
...
- 🐛 **修复嵌套 scoped slot 中纯原生默认内容被额外编译为 generic slot 组件后在微信开发者工具中可能卡死的问题，并避免 scoped slot 桥接字段继续发布为嵌套 owner snapshot。同步发布 create-weapp-vite，保持脚手架模板依赖版本与本次修复一致。** [`41d4cc2`](https://github.com/weapp-vite/weapp-vite/commit/41d4cc2eeeb9c3f077d1584acb08c83c5a2868a9) by @sonofmagic

- 🐛 **修复 performance 预设下组件 slot 桥接字段没有同步到顶层 data 的问题，避免普通插槽误渲染 fallback、scoped slot 拿不到 owner id。同步发布 create-weapp-vite，保持脚手架模板依赖版本与本次修复一致。** [#645](https://github.com/weapp-vite/weapp-vite/pull/645) by @sonofmagic
...
- 🐛 **修复 `6.16.21` 起 `<script setup>` props 解构派生字段可能作为同名顶层 `setData` 数据回写的问题，避免小程序属性 observer 与 wevu props 同步之间形成重复触发循环；同时保留 props 别名字段在模板中的正常渲染更新。** [#611](https://github.com/weapp-vite/weapp-vite/pull/611) by @sonofmagic
...
- 🐛 **修复 Tailwind CSS 小程序模板中 `bg-clip-text` 渐变文字在微信端缺少兼容前缀的问题，所有内置 Tailwind 模板默认接入 `autoprefixer` 后处理。** [`dc83390`](https://github.com/weapp-vite/weapp-vite/commit/dc83390a94e7648b6d9d7451cafedbbada2e0ff3) by @sonofmagic
...
- 🐛 **修复 wevu 组件式页面中 `onReachBottom` 未同步暴露到页面 `methods` 的问题，确保微信运行时按页面方法分发触底事件时也能触发通过组合式 API 注册的回调。** [#509](https://github.com/weapp-vite/weapp-vite/pull/509) by @sonofmagic
...
- 🐛 **调整 `create-weapp-vite` 的项目初始化行为：执行 `pnpm create weapp-vite` 创建新项目后，会把生成目录中的 `project.config.json.appid` 统一改写为 `touristappid`，避免直接继承仓库模板里的真实 AppID。** [`a03de37`](https://github.com/weapp-vite/weapp-vite/commit/a03de374e141270877f3d0a7564fc88ba53a57f6) by @sonofmagic
...
- 🐛 **修复非 wevu 原生模板缺少 `wevu` 依赖的问题。现在 `default`、`tailwindcss`、`vant`、`tdesign` 模板在保留 `wevu/api` 子路径导入与 `wpi` 跨平台调用方式的同时，会正确声明 `wevu`，避免通过 `pnpm create weapp-vite` 生成项目后在 `wv build` 阶段出现 `Rolldown failed to resolve import "wevu/api"` 的构建失败。** [`c5dea57`](https://github.com/weapp-vite/weapp-vite/commit/c5dea5740704be342349d8cad8f6f6b1cf3a9138) by @sonofmagic
...
- 🐛 **修复脚手架模板仍然导入 `@wevu/api` 的问题，统一改为使用 `wevu/api` 子路径入口，并移除 lib 模板里不再需要的 `@wevu/api` 直接依赖，避免新建模板项目在构建时出现导入解析失败。** [`1cce0f3`](https://github.com/weapp-vite/weapp-vite/commit/1cce0f3be69dea2dd84f43a4ef274bce9906d372) by @sonofmagic
...
- 🐛 **修复 `weapp-vite-wevu-tailwindcss-tdesign-template` 表单页“加急”开关的交互体验：现在既支持直接点击 `t-switch` 切换，也支持点击整行区域切换，并补充对应的 DevTools e2e 回归测试，避免后续模板回归为“看起来无响应”的状态。** [`6a58a09`](https://github.com/weapp-vite/weapp-vite/commit/6a58a0979ec87cc85802f07d495d21571cebfb31) by @sonofmagic
...
- 🐛 **修复通过 npm registry 安装并执行 `pnpm create weapp-vite` 时模板文件被错误过滤的问题。脚手架现在会基于模板目录内的相对路径判断是否跳过文件，不再把安装路径中的 `node_modules/create-weapp-vite` 误判为模板内部的 `node_modules`，从而确保各模板都能完整创建出源码、配置和静态资源文件。** [`8e17b7d`](https://github.com/weapp-vite/weapp-vite/commit/8e17b7d6f4e6cd6f8ba2d48ccdbec8c9e8cb0f37) by @sonofmagic

## 2.0.65

### Patch Changes

- 🐛 **修复 `create-weapp-vite` 在发布前模板目录已部分存在且同步标记仍存在时会误跳过模板同步的问题，避免发布包只包含不完整模板文件。** [#362](https://github.com/weapp-vite/weapp-vite/pull/362) by @sonofmagic
...
- 🐛 **统一仓库内小程序项目 `project.private.config.json` 的默认 `libVersion` 为 `3.15.0`。新增同步脚本集中维护该默认值，并让模板与 DevTools 自动化使用同一处配置来源，减少不同项目与产物之间的版本漂移。** [`0961e51`](https://github.com/weapp-vite/weapp-vite/commit/0961e51097290026944d6221af05b583ccab17b6) by @sonofmagic
...
- 🐛 **修复 workspace 场景下 `create-weapp-vite` 解析模板目录失败的问题，并在复制模板时跳过模板内的构建产物与 `node_modules` 等冗余内容。同时保留目标目录原有的 `.gitignore` 条目，避免初始化项目时覆盖用户已有忽略规则。** [#353](https://github.com/weapp-vite/weapp-vite/pull/353) by @sonofmagic
