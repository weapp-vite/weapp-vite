...
微信不同运行时版本不能统一假定存在 `queueMicrotask`。业务代码应接入 `@weapp-vite/eslint` 的 `miniProgramRuntimeRecommended`，框架代码应使用显式兼容层；需要 request globals 时明确配置 `weapp.appPrelude.webRuntime`。
