...
const workspaceRootNodeModulesDir = path.resolve(import.meta.dirname, '../../../node_modules')
const workspaceWeappViteDir = path.resolve(import.meta.dirname, '..')
const reportDir = path.resolve(
  import.meta.dirname,
  '../benchmark/auto-import-build',
...
async function runBuild(cwd: string) {
  ...
  await new Promise<void>((resolve, reject) => {
    const child = spawn(process.execPath, [cliPath, 'build'], {
      ...
      env: {
        ...process.env,
        PATH: `${path.resolve(import.meta.dirname, '../../../node_modules/.bin')}:${process.env.PATH ?? ''}`,
      },
      stdio: 'pipe',
    })
