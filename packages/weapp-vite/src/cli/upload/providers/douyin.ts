...
/** 校验抖音项目与 Token 对应的 AppID，仅在执行阶段调用官方上传 SDK。 */
export async function prepareDouyinUpload(context: UploadContext): Promise<PreparedUpload> {
  const token = requireUploadEnv(context, 'TT_UPLOAD_TOKEN')
  const appid = context.env.TT_APP_ID?.trim() || requireUploadAppId(context)
  if (!/^\d+\.\d+\.\d+$/.test(context.version)) {
    throw new Error('抖音上传版本号必须为 x.y.z 格式。')
  }
  if (!context.desc.trim()) {
