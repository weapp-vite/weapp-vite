...
function createRequestGlobalsPassiveBindingsCode(bindingTargets: string[]) {
  const helperCode = [
  ...
  const bindingCode = bindingTargets.map((target) => {
    ...
    if (target === 'URL') {
      return `var URL = __weappViteHasUsableRequestGlobalsConstructor__(${actualRef},["https://request-globals.test"])?${actualRef}:__weappViteHasUsableRequestGlobalsConstructor__(globalThis.URL,["https://request-globals.test"])?globalThis.URL:${placeholderFactory}`
    }
    ...
    if (target === 'Request') {
      return `var Request = __weappViteHasUsableRequestGlobalsConstructor__(${actualRef},["https://request-globals.test"])?${actualRef}:__weappViteHasUsableRequestGlobalsConstructor__(globalThis.Request,["https://request-globals.test"])?globalThis.Request:${placeholderFactory}`
    }
    if (target === 'Response') {
