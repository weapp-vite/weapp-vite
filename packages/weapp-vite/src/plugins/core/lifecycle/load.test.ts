...
describe('core lifecycle load hook injectWeapi', () => {
  it('auto injects request globals into app entry when matching dependencies exist', async () => {
    ...
    expect(code).toContain('weapp-vite/runtime/requestGlobals')
    expect(code).toContain('installRequestGlobals')
    expect(code).toContain('"XMLHttpRequest"')
  })

  it('does not inject request globals in auto mode when dependencies do not match', async () => {
    const loadEntry = vi.fn(async () => ({
      code: 'App({})',
    }))
    ...
    expect(code).not.toContain('weapp-vite/runtime/requestGlobals')
  })

  it('injects request globals into lib entry components', async () => {
