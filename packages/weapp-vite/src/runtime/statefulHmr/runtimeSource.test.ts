...
describe('stateful hmr runtime source', () => {
  it('uses static runtime hooks without dynamic evaluation', () => {
    ...
    expect(statefulHmrRolldownRuntimeSource).toContain('pending.push(nativeDefinition)')
    ...
    expect(statefulHmrRolldownRuntimeSource).toContain('getDebugSnapshot(includeState = false)')
    ...
    expect(statefulHmrRolldownRuntimeSource).toContain('original.call(globalThis, nativeDefinition)')
  ...
  it('renders authenticated metadata transport without executable HTTP payloads', () => {
    ...
    expect(source).toContain('applyChangedModules(changedIds, preparedUpdate)')
    expect(source).toContain('applyChangedModules(meta.changedIds, preparedUpdate)')
    expect(source).toContain('runtime.applyPreparedUpdate(preparedUpdate ?? runtime.prepareUpdate(changedIds))')
