...
export function getNativeComponentSidecarSource(id: string, root: string, entryIds?: Iterable<string>): string | undefined {
  ...
  const sourceId = normalizeSourceId(path.resolve(root, sidecar.sourceId))
  // 只有真实脚本已作为独立入口接受更新时，父页面的纯依赖摘要才能独立接受更新。
  for (const entryId of entryIds) {
    if (normalizeSourceId(path.resolve(root, entryId)) === sourceId) {
      return sourceId
    }
  }
}
...
export function isChangedNativeComponentSidecar(
  ...
  const srcRoot = normalizeSourceId(options.srcRoot ?? root).replace(/\/$/, '')
  return sourceId !== undefined
    && sourceId.startsWith(`${srcRoot}/`)
    && files.some(file => normalizeSourceId(path.resolve(root, file)) === sourceId)
}
