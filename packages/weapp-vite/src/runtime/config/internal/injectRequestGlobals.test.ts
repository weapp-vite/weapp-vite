...
describe('injectRequestGlobals helpers', () => {
  ...
  it('can create passive local binding injection code for manual installers', () => {
    ...
    expect(code).toContain(`var File = ${REQUEST_GLOBAL_EXPOSE_HELPER}("File",typeof ${REQUEST_GLOBAL_ACTUALS_KEY}["File"]==="function"`)
    expect(code).toContain(`var TextDecoder = ${REQUEST_GLOBAL_EXPOSE_HELPER}("TextDecoder",typeof ${REQUEST_GLOBAL_ACTUALS_KEY}["TextDecoder"]==="function"`)
    expect(code).not.toContain(`${REQUEST_GLOBAL_USABLE_CONSTRUCTOR_HELPER}(${REQUEST_GLOBAL_ACTUALS_KEY}["File"]`)
  ...
  it('keeps free-variable bindings for request libraries instead of relying on app-level globals only', () => {
    ...
    expect(code).not.toContain(`${REQUEST_GLOBAL_USABLE_CONSTRUCTOR_HELPER}(${REQUEST_GLOBAL_ACTUALS_KEY}["WebSocket"]`)
    expect(code).toContain(`var URL = ${REQUEST_GLOBAL_EXPOSE_HELPER}("URL"`)
    expect(code).toContain(`var File = ${REQUEST_GLOBAL_EXPOSE_HELPER}("File"`)
