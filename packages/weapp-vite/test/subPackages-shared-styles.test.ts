...
describe('subPackages shared styles', () => {
  ...
  it('injects scoped shared imports into subpackage outputs', async () => {
    ...
    expect(componentContent).toContain('@import \'../../../styles/main.wxss\';')
    expect(componentContent).toContain('@import \'../../../styles/components.wxss\';')
  ...
  it('emits main-package entries and injects them without modifying app.wxss', async () => {
    ...
    expect(content.match(/styles\/main\.wxss/g)).toHaveLength(1)
    expect(content).toContain('@import \'../../styles/pages.wxss\';')
    expect(content).not.toContain('@import \'../../styles/components.wxss\';')
    expect(content).not.toContain('manual.wxss')

    const componentContent = await read(path.resolve(distDir, 'components/main-card/index.wxss'))
    expect(componentContent).toContain('@import \'../../styles/main.wxss\';')
    expect(componentContent.match(/styles\/main\.wxss/g)).toHaveLength(1)
    expect(componentContent).toContain('@import \'../../styles/components.wxss\';')
    expect(componentContent).not.toContain('@import \'../../styles/pages.wxss\';')
    expect(componentContent).not.toContain('manual.wxss')

    const sharedContent = await read(path.resolve(distDir, 'styles/main.wxss'))
    const pageSharedContent = await read(path.resolve(distDir, 'styles/pages.wxss'))
    const componentSharedContent = await read(path.resolve(distDir, 'styles/components.wxss'))
    const manualContent = await read(path.resolve(distDir, 'styles/manual.wxss'))
    expect(sharedContent).toContain('color: #3367d6;')
    expect(pageSharedContent).toContain('border-top: 2px solid #0f766e;')
    expect(componentSharedContent).toContain('border-bottom: 2px solid #be123c;')
