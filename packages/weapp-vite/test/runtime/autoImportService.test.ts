...
describe('autoImportService', () => {
  ...
  beforeAll(async () => {
    ...
    manifestPath = path.resolve(cwd, '.weapp-vite/auto-import-components.json')
    typedDefinitionPath = path.resolve(cwd, '.weapp-vite/typed-components.d.ts')
    htmlDataPath = path.resolve(cwd, '.weapp-vite/mini-program.html-data.json')
    vueComponentsDefinitionPath = path.resolve(cwd, '.weapp-vite/components.d.ts')
    componentTemplates = [
  ...
  it('writes typed component definitions for discovered components', async () => {
    ...
    expect(typedContent).toContain('readonly items?: unknown[];')
    expect(typedContent).toContain('readonly dataSet?: Record<string, unknown>;')
    expect(typedContent).toContain('readonly anyValue?: unknown;')
