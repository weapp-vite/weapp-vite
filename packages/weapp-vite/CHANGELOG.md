...
<!--
- 支持在导入末尾追加 `?take` 指令强制将模块复制到当前分包：只要分包使用 `import 'foo?take'`，`foo` 就会复制到该分包的 `weapp-shared/common.js`，允许 `hoist` 策略下的按需复制；若同时存在普通导入，构建日志会提示代码既保留在主包也会复制到使用 `?take` 的分包。
- 模板项目默认在 `tsconfig.json` 中开启 `"moduleSuffixes": ["?take", ""]`，TypeScript 会自动把 `import 'foo?take'` 解析为 `foo`，恢复原有的类型提示。
-->
