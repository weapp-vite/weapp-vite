...
`weapp-vite` 内置了对 `weapp-ide-cli` 的透传能力，除了 `dev/build/upload/close/open/init/generate/analyze/npm/prepare/mcp` 等原生命令外，其它 IDE 相关命令都可以直接调用。旧微信上传使用显式 `ide upload`，不再占用顶层 `upload`：
