...
| `offAfterPageLoad`                            | `offAppShow`                         | ✅   | `mapped`   | ✅       | 映射到 `my.offAppShow`（近似事件回调解绑）                             |
| `offAfterPageUnload`                          | `offAppShow`                         | ✅   | `mapped`   | ✅       | 映射到 `my.offAppShow`（近似事件回调解绑）                             |
| `offApiCategoryChange`                        | `offAppShow`                         | ✅   | `mapped`   | ✅       | 映射到 `my.offAppShow`（近似事件回调解绑）                             |
| `offAppHide`                                  | `offAppHide`                         | ✅   | `native`   | ✅       | 直连 `my.offAppHide`                                                   |
| `offAppRoute`                                 | `offAppShow`                         | ✅   | `mapped`   | ✅       | 映射到 `my.offAppShow`（近似事件回调解绑）                             |
| `offAppRouteDone`                             | `offAppShow`                         | ✅   | `mapped`   | ✅       | 映射到 `my.offAppShow`（近似事件回调解绑）                             |
...
| `offBatteryInfoChange`                        | `offAppShow`                         | ✅   | `mapped`   | ✅       | 映射到 `my.offAppShow`（近似事件回调解绑）                             |
...
| `offBeforeAppRoute`                           | `offAppShow`                         | ✅   | `mapped`   | ✅       | 映射到 `my.offAppShow`（近似事件回调解绑）                             |
| `offBeforePageLoad`                           | `offAppShow`                         | ✅   | `mapped`   | ✅       | 映射到 `my.offAppShow`（近似事件回调解绑）                             |
| `offBeforePageUnload`                         | `offAppShow`                         | ✅   | `mapped`   | ✅       | 映射到 `my.offAppShow`（近似事件回调解绑）                             |
| `offBLECharacteristicValueChange`             | `offBLECharacteristicValueChange`    | ✅   | `native`   | ✅       | 直连 `my.offBLECharacteristicValueChange`                              |
| `offBLEConnectionStateChange`                 | `offAppShow`                         | ✅   | `mapped`   | ✅       | 映射到 `my.offAppShow`（近似事件回调解绑）                             |
| `offBLEMTUChange`                             | `offAppShow`                         | ✅   | `mapped`   | ✅       | 映射到 `my.offAppShow`（近似事件回调解绑）                             |
| `offBLEPeripheralConnectionStateChanged`      | `offAppShow`                         | ✅   | `mapped`   | ✅       | 映射到 `my.offAppShow`（近似事件回调解绑）                             |
...
| `offCopyUrl`                                  | `offAppShow`                         | ✅   | `mapped`   | ✅       | 映射到 `my.offAppShow`（近似事件回调解绑）                             |
| `offDeviceMotionChange`                       | `offDeviceMotionChange`              | ✅   | `native`   | ✅       | 直连 `my.offDeviceMotionChange`                                        |
| `offEmbeddedMiniProgramHeightChange`          | `offAppShow`                         | ✅   | `mapped`   | ✅       | 映射到 `my.offAppShow`（近似事件回调解绑）                             |
| `offError`                                    | `offError`                           | ✅   | `native`   | ✅       | 直连 `my.offError`                                                     |
| `offGeneratePoster`                           | `offAppShow`                         | ✅   | `mapped`   | ✅       | 映射到 `my.offAppShow`（近似事件回调解绑）                             |
...
| `offHCEMessage`                               | `offAppShow`                         | ✅   | `mapped`   | ✅       | 映射到 `my.offAppShow`（近似事件回调解绑）                             |
| `offKeyboardHeightChange`                     | `offAppShow`                         | ✅   | `mapped`   | ✅       | 映射到 `my.offAppShow`（近似事件回调解绑）                             |
| `offKeyDown`                                  | `offAppShow`                         | ✅   | `mapped`   | ✅       | 映射到 `my.offAppShow`（近似事件回调解绑）                             |
| `offKeyUp`                                    | `offAppShow`                         | ✅   | `mapped`   | ✅       | 映射到 `my.offAppShow`（近似事件回调解绑）                             |
| `offLazyLoadError`                            | `offAppShow`                         | ✅   | `fallback` | ❌       | 回退映射到 `my.offAppShow`（通用兜底）                                 |
| `offLocalServiceDiscoveryStop`                | `offAppShow`                         | ✅   | `mapped`   | ✅       | 映射到 `my.offAppShow`（近似事件回调解绑）                             |
...
| `onAfterPageLoad`                             | `onAppShow`                          | ✅   | `mapped`   | ✅       | 映射到 `my.onAppShow`（近似事件回调）                                  |
| `onAfterPageUnload`                           | `onAppShow`                          | ✅   | `mapped`   | ✅       | 映射到 `my.onAppShow`（近似事件回调）                                  |
| `onApiCategoryChange`                         | `onAppShow`                          | ✅   | `mapped`   | ✅       | 映射到 `my.onAppShow`（近似事件回调）                                  |
| `onAppHide`                                   | `onAppHide`                          | ✅   | `native`   | ✅       | 直连 `my.onAppHide`                                                    |
| `onAppRoute`                                  | `onAppShow`                          | ✅   | `mapped`   | ✅       | 映射到 `my.onAppShow`（近似事件回调）                                  |
| `onAppRouteDone`                              | `onAppShow`                          | ✅   | `mapped`   | ✅       | 映射到 `my.onAppShow`（近似事件回调）                                  |
...
| `onBackgroundAudioPause`                      | `onAppShow`                          | ✅   | `mapped`   | ✅       | 映射到 `my.onAppShow`（近似事件回调）                                  |
| `onBackgroundAudioPlay`                       | `onAppShow`                          | ✅   | `mapped`   | ✅       | 映射到 `my.onAppShow`（近似事件回调）                                  |
| `onBackgroundAudioStop`                       | `onAppShow`                          | ✅   | `mapped`   | ✅       | 映射到 `my.onAppShow`（近似事件回调）                                  |
| `onBackgroundFetchData`                       | `onAppShow`                          | ✅   | `mapped`   | ✅       | 映射到 `my.onAppShow`（近似事件回调）                                  |
| `onBatteryInfoChange`                         | `onAppShow`                          | ✅   | `mapped`   | ✅       | 映射到 `my.onAppShow`（近似事件回调）                                  |
...
| `onBeforeAppRoute`                            | `onAppShow`                          | ✅   | `mapped`   | ✅       | 映射到 `my.onAppShow`（近似事件回调）                                  |
| `onBeforePageLoad`                            | `onAppShow`                          | ✅   | `mapped`   | ✅       | 映射到 `my.onAppShow`（近似事件回调）                                  |
| `onBeforePageUnload`                          | `onAppShow`                          | ✅   | `mapped`   | ✅       | 映射到 `my.onAppShow`（近似事件回调）                                  |
| `onBLECharacteristicValueChange`              | `onBLECharacteristicValueChange`     | ✅   | `native`   | ✅       | 直连 `my.onBLECharacteristicValueChange`                               |
| `onBLEConnectionStateChange`                  | `onAppShow`                          | ✅   | `mapped`   | ✅       | 映射到 `my.onAppShow`（近似事件回调）                                  |
| `onBLEMTUChange`                              | `onAppShow`                          | ✅   | `mapped`   | ✅       | 映射到 `my.onAppShow`（近似事件回调）                                  |
| `onBLEPeripheralConnectionStateChanged`       | `onAppShow`                          | ✅   | `mapped`   | ✅       | 映射到 `my.onAppShow`（近似事件回调）                                  |
...
| `onCopyUrl`                                   | `onAppShow`                          | ✅   | `mapped`   | ✅       | 映射到 `my.onAppShow`（近似事件回调）                                  |
| `onDeviceMotionChange`                        | `onDeviceMotionChange`               | ✅   | `native`   | ✅       | 直连 `my.onDeviceMotionChange`                                         |
| `onEmbeddedMiniProgramHeightChange`           | `onAppShow`                          | ✅   | `mapped`   | ✅       | 映射到 `my.onAppShow`（近似事件回调）                                  |
| `onError`                                     | `onError`                            | ✅   | `native`   | ✅       | 直连 `my.onError`                                                      |
| `onGeneratePoster`                            | `onAppShow`                          | ✅   | `mapped`   | ✅       | 映射到 `my.onAppShow`（近似事件回调）                                  |
| `onGetWifiList`                               | `onGetWifiList`                      | ✅   | `native`   | ✅       | 直连 `my.onGetWifiList`                                                |
| `onGyroscopeChange`                           | `onGyroscopeChange`                  | ✅   | `native`   | ✅       | 直连 `my.onGyroscopeChange`                                            |
| `onHCEMessage`                                | `onAppShow`                          | ✅   | `mapped`   | ✅       | 映射到 `my.onAppShow`（近似事件回调）                                  |
