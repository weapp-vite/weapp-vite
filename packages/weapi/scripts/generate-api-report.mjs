...
| 三端完全对齐方法数 | ${fullyAlignedMethods.length} |

## 覆盖率

| 平台 | 已支持 API 数 | API 总数 | 覆盖率 |
| --- | ---: | ---: | ---: |
${report.platforms.map(item => `| ${item.platform} (\`${item.alias}\`) | ${item.supportedApis} | ${item.totalApis} | ${item.coverage} |`).join('\n')}
| 三端完全对齐 (wx/my/tt) | ${report.fullyAlignedApis} | ${report.totalApis} | ${report.fullyAlignedCoverage} |
...
- 支付宝兼容方法数：${alipaySupportedMethods.length}
- 抖音兼容方法数：${douyinSupportedMethods.length}
- 三端完全对齐方法数：${fullyAlignedMethods.length}
...
${alipayUnsupportedMethods.slice(0, 40).map(item => `- \`${item.method}\` -> 目标 \`${item.alipayTarget}\``).join('\n')}

### 抖音不兼容示例

${douyinUnsupportedMethods.slice(0, 40).map(item => `- \`${item.method}\` -> 目标 \`${item.douyinTarget}\``).join('\n')}
`
...
| --- | --- | --- | --- |
${matrix.map(item => `| \`${item.method}\` | \`${item.alipayTarget}\` | ${formatBool(item.alipaySupported)} | ${escapePipe(item.alipayStrategy)} |`).join('\n')}
`
...
| --- | --- | --- | --- |
${matrix.map(item => `| \`${item.method}\` | \`${item.douyinTarget}\` | ${formatBool(item.douyinSupported)} | ${escapePipe(item.douyinStrategy)} |`).join('\n')}
`
