...
| `install(app)`    | 支持                                 | 不支持                                    | 不对齐    | `wevu` 不走 Vue Web 插件安装模型。                                             |
...
| `beforeEnter`       | 支持         | 支持        | 已对齐 | 支持单个或数组守卫。                            |
| `redirect`          | 支持         | 支持        | 已对齐 | 支持字符串/对象/函数，并保留 `redirectedFrom`。 |
| 嵌套路由 `children` | 支持         | 不支持      | 不对齐 | 当前仅支持扁平命名路由记录。                    |
| `alias`             | 支持         | 不支持      | 不对齐 | 可通过额外命名路由近似实现。                    |
