...
import type { JsxAutoComponentContext, JsxImportedComponent } from '../../plugins/jsx/compileJsx/types'
...
function collectJsxTemplateTagsFromOxc(renderExpression: any) {
  const tags = new Set<string>()
...
function collectWithBabel(source: string): JsxAutoComponentContext {
  ...
  const renderExpression = resolveRenderExpression(exportDefaultExpression)
  if (!renderExpression) {
    return {
      templateTags: new Set<string>(),
      importedComponents,
...
function collectWithOxc(source: string): JsxAutoComponentContext {
  ...
  return {
    templateTags: collectJsxTemplateTagsFromOxc(renderExpression),
    importedComponents,
  }
}
...
export function collectJsxAutoComponentsFromCode(
  ...
  try {
    return engine === 'oxc'
      ? collectWithOxc(source)
      : collectWithBabel(source)
  }
  catch {
    return {
      templateTags: new Set<string>(),
