...
function createScope(initial?: LegacyTemplateScope) {
  return Object.assign(Object.create(null), initial ?? {})
}

function createChildScope(parent: LegacyTemplateScope) {
...
function evaluateExpression(expression: string, scope: LegacyTemplateScope) {
  if (!expression) {
    return undefined
  }
...
function interpolateText(source: string, scope: LegacyTemplateScope, escapeResult: boolean) {
  if (!source.includes('{{')) {
...
function resolveAttributeValue(value: string, scope: LegacyTemplateScope) {
