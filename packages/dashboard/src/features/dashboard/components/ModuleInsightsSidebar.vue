...
<template>
  <div class="grid min-h-0 min-w-0 gap-3 overflow-visible xl:grid-rows-[auto_minmax(0,1fr)_minmax(0,0.8fr)] xl:overflow-hidden">
    <section :class="surfaceStyles({ padding: 'md' })" class="min-w-0 overflow-visible xl:overflow-hidden">
    ...
    <section :class="surfaceStyles({ padding: 'md' })" class="min-h-0 min-w-0 overflow-visible xl:overflow-hidden">
      <AppPanelHeader icon-name="metric-time" title="增量归因" />
      <div v-if="incrementItems.length" class="mt-4 grid min-h-0 min-w-0 gap-3 overflow-visible lg:grid-cols-[minmax(0,0.78fr)_minmax(0,1fr)] xl:h-[calc(100%-3.5rem)] xl:overflow-hidden">
        <div class="max-h-80 space-y-2 overflow-y-auto pr-1 xl:max-h-none">
          <AppSummaryValueCard
        ...
        <ul class="max-h-80 space-y-2 overflow-y-auto pr-1 text-sm text-(--dashboard-text-muted) xl:max-h-none">
          <AppCompactListItem
    ...
    <section :class="surfaceStyles({ padding: 'md' })" class="min-h-0 min-w-0 overflow-visible xl:overflow-hidden">
      ...
      <div class="mt-4 grid min-h-0 min-w-0 gap-3 overflow-visible lg:grid-cols-2 xl:h-[calc(100%-3.5rem)] xl:overflow-hidden">
        <div class="max-h-80 space-y-2.5 overflow-y-auto pr-1 xl:max-h-none">
          <AppSummaryValueCard
        ...
        <ul class="max-h-80 space-y-2.5 overflow-y-auto pr-1 text-sm text-(--dashboard-text-muted) xl:max-h-none">
          <AppCompactListItem
