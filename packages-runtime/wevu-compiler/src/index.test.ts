...
describe('compiler package entry', () => {
  it('re-exports the public compiler surface', () => {
    expect(compiler).toMatchObject({
      ...
      classifyVueSfcBlockChanges: expect.any(Function),
      ...
      getDefaultMiniProgramPlatform: expect.any(Function),
      ...
      resolveVueSfcHmrSignatures: expect.any(Function),
  ...
  it('keeps aliased exports pointing to the same implementation', () => {
    ...
    expect(compiler.getDefaultMiniProgramPlatform).toBe(compiler.getDefaultMiniProgramTemplatePlatform)
