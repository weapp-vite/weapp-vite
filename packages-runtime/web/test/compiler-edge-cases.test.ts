...
describe('compiler and plugin edge cases', () => {
  ...
  it('normalizes plugin paths and resolves real extension candidates', async () => {
    ...
    expect(resolveImportBase('./part', importer, src)).toBe(join(pages, 'part'))
    expect(resolveImportBase('/pages/part', importer, src)).toBe(join(pages, 'part'))
    expect(resolveImportBase('pages/part', importer, src)).toBe(join(pages, 'part'))
    ...
    expect(resolveTemplatePathSync('./part', importer, src)).toBe(template)
    expect(resolveTemplatePathSync('', importer, src)).toBeUndefined()
    expect(resolveWxsPathSync('./tool', importer, src)).toBe(wxs)
    expect(resolveWxsPathSync('/pages/tool', importer, src)).toBe(wxs)
