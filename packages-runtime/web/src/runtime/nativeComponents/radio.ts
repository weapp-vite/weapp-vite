...
export function collectRadioGroupValue(radios: ArrayLike<WeappRadio>) {
  return Array.from(radios).find(radio => radio.checked && !radio.disabled)?.value ?? ''
}

export class WeappRadioGroup extends BaseElement {
  static observedAttributes = [...getNativeComponentDescriptor('radio-group')!.attributes]
  ...
  get formControlValue() {
    return collectRadioGroupValue(this.querySelectorAll<WeappRadio>('weapp-radio'))
  }
  ...
  formReset() {
    for (const radio of this.querySelectorAll<WeappRadio>('weapp-radio')) {
      radio.formReset()
    }
    this.#normalizeCheckedRadios()
  }

  notifyRadioChange(source: WeappRadio) {
    for (const radio of this.querySelectorAll<WeappRadio>('weapp-radio')) {
      if (radio !== source) {
        radio.setCheckedFromGroup(false)
      }
    }
    dispatchMiniProgramEvent(this, 'change', { value: source.value })
  }

  #normalizeCheckedRadios() {
    let selected = false
    for (const radio of this.querySelectorAll<WeappRadio>('weapp-radio')) {
      if (!radio.checked) {
        continue
...
export class WeappRadio extends BaseElement {
  static observedAttributes = [...getNativeComponentDescriptor('radio')!.attributes]
  ...
  setCheckedFromGroup(checked: boolean) {
    ...
    input.addEventListener('change', (event) => {
      ...
      const group = findClosestComposedElement(this, 'weapp-radio-group') as WeappRadioGroup | null
