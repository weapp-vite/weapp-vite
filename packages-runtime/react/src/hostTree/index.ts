...
import { WEAPP_REACT_ROOT_DATA_KEY, WEAPP_REACT_SLOTS_DATA_KEY } from '@weapp-core/constants'
...
export class HostRoot {
  ...
  private flushStaticBindings() {
    ...
    if (!this.mounted) {
      this.mounted = true
      this.staticBindings.clear()
      this.adapter.setData({ [WEAPP_REACT_SLOTS_DATA_KEY]: this.staticBindings.snapshot(this.staticElements()) })
      return
    }
  ...
  flush() {
    ...
    if (!this.mounted) {
      ...
      this.adapter.setData({ [WEAPP_REACT_ROOT_DATA_KEY]: this.snapshot() })
      return
    }
