...
| 浏览器开发模式     | `pnpm --filter weapp-vite-web-demo dev:web`                | 内置脚本等价于 `weapp-vite dev -p web`，可通过 `--` 继续向 Vite 透传参数（例如 `--host 0.0.0.0`）。 |
| 生产构建           | `pnpm --filter weapp-vite-web-demo build:web`              | 使用 `-p web` 生成浏览器产物 `dist/web`；`build` 仍生成小程序产物 `dist/`。                         |
...
浏览器端：运行 `dev:web` 后默认访问 <http://127.0.0.1:5173/>，支持在按钮间跳转 About 页验证 `wx.navigateTo` / `wx.navigateBack`、`Page` 生命周期等运行时能力。若需在浏览器中查看二维码或真实设备调试，请确保 Dev Server 监听地址可被局域网访问（参见下方环境变量配置）。
