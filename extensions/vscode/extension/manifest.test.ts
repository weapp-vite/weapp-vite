...
it('manifest exposes local verification scripts', () => {
  ...
  assert.equal(packageJson.scripts['release:plan'], 'node --import tsx scripts/prepare-release.ts plan')
  assert.equal(packageJson.scripts['release:apply'], 'node --import tsx scripts/prepare-release.ts apply')
