...
describeSwanRuntime('optional multi-platform template Baidu runtime smoke', { concurrent: false }, () => {
  let device: any
  let smartapp: any
  ...
  beforeAll(async () => {
    ...
    await runWeappViteBuildWithLogCapture({
      ...
      platform: 'swan',
      skipNpm: true,
      label: 'ide:multi-platform-template:swan',
    })
    ...
    smartapp = await device.newSmartapp('devtools', 'weapp-vite-multi-platform-template')
  }, 120_000)
  ...
  it('reuses one session and exposes the template runtime state', async () => {
