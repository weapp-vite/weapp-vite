...
describe.sequential('e2e app: script-setup-macros-mapping (build)', () => {
  it('covers native component + wevu SFC script setup macro mappings in TS/JS variants', async () => {
    ...
    expect(tsWithDefaultsJs).toContain('tags:{type:Array,required:!1,default:()=>[`a`,`b`]}')
    expect(tsWithDefaultsJs).toContain('meta:{type:Object,required:!1,default:()=>({source:`ts-defaults`})}')
    ...
    expect(tsWithDefaultsAliasJs).toContain('list:{type:Array,required:!1,default:()=>[1,2,3]}')
    expect(tsWithDefaultsAliasJs).toContain('options:{type:Object,required:!1,default:()=>({source:`alias-defaults`})}')
