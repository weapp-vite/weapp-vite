...
describe.sequential('e2e app: github-issues (build)', () => {
  ...
  it('issue #521: keeps generated scoped slot components virtual for flex layouts', async () => {
    ...
    expect(scopedSlotWxml).toMatch(/<FlexItem label="A" val="\{\{__wv_bind_\d+\}\}" \/><FlexItem label="B" val="\{\{__wv_bind_\d+\}\}" \/>/)
    expect(await fs.readFile(scopedSlotWxmlPath.replace(/\.wxml$/, '.js'), 'utf-8')).toContain('this.__wvSlotPropsData).xyz')
  ...
  it('issue #289: compiles split pages with per-page controls and safe class bindings', async () => {
    ...
    expect(objectPageWxml).toMatch(/show-list="\{\{__wv_bind_\d+\}\}"/)
    expect(objectPageWxml).toMatch(/compact-mode="\{\{__wv_bind_\d+\}\}"/)
    ...
    expect(mapPageWxml).toMatch(/callout-expanded="\{\{__wv_bind_\d+\}\}"/)
    expect(mapPageWxml).toMatch(/show-callout-list="\{\{__wv_bind_\d+\}\}"/)
    expect(mapPageWxml).toMatch(/selected-event-idx="\{\{__wv_bind_\d+\}\}"/)
    ...
    expect(rootPageWxml).toMatch(/show-options="\{\{__wv_bind_\d+\}\}"/)
    ...
    expect(computedPageWxml).toMatch(/source-enabled="\{\{__wv_bind_\d+\}\}"/)
    expect(computedPageWxml).toMatch(/show-items="\{\{__wv_bind_\d+\}\}"/)
    expect(computedPageWxml).toMatch(/selected-index="\{\{__wv_bind_\d+\}\}"/)
