...
describe.sequential('e2e app: github-issues (build)', () => {
  ...
  it('issue #613: wraps forwarded slot outlets inside plain named slot content', async () => {
    ...
    expect(pageWxml).toContain('<scoped-slots-default slot="header" wx:if="{{__wvSlotOwnerId}}"')
    expect(pageWxml).toContain('<block wx:else><slot /></block>')
    expect(pageWxml).not.toContain('<slot slot="header"')
    expect(pageWxml).toContain('issue613-block-forwarder')
    expect(forwarderWxml).toContain('<Issue613Card slot-wrapper="cover-view" slot-wrapper-footer="view"')
    expect(forwarderWxml).toContain('<scoped-slots-default slot="header" wx:if="{{__wvSlotOwnerId}}"')
    expect(forwarderWxml).toContain('<slot slot="footer" name="footer" />')
