...
// 新增 issue 回归必须放到 cases/，本文件仅保留尚未迁移的历史用例。
...
async function runIssue826Build(jsFormat: 'cjs' | 'esm') {
  const distRoot = issue826DistRoot(jsFormat)
  ...
  await runWeappViteBuildWithLogCapture({
    ...
    cwd: APP_ROOT,
    jsFormat,
    label: `ci:github-issues:issue826:${jsFormat}`,
    outDir: `dist-issue-826-${jsFormat}`,
    skipNpm: true,
    env: {
      WEAPP_GITHUB_ISSUE_826_PRESERVE: 'true',
      WEAPP_GITHUB_ISSUE_826_OUT_DIR: `dist-issue-826-${jsFormat}`,
    },
  })
...
export function createGithubIssuesBuildCaseContext(): GithubIssuesBuildCaseContext {
  return {
    appRoot: APP_ROOT,
    cliPath: CLI_PATH,
    distRoot: DIST_ROOT,
    runStandardBuild: runBuild,
  }
}

export function registerGithubIssuesBuildLegacyCases() {
  ...
  it('issue #826: preserves configured source module paths for debugging', async () => {
    for (const jsFormat of ['cjs', 'esm'] as const) {
      ...
      const files = await scanFiles(distRoot)
      ...
      expect(files).not.toContain('pages/issue-826/local.js')

      const pageCode = await fs.readFile(path.join(distRoot, 'pages/issue-826/index.js'), 'utf8')
      expect(pageCode).toContain('__ISSUE_826_LOCAL_UNMATCHED__')
      ...
      const barrelCode = await fs.readFile(path.join(distRoot, 'issue-fixtures/issue-826/utils/barrel.js'), 'utf8')
      ...
      const secondPageCode = await fs.readFile(path.join(distRoot, 'pages/issue-826/second.js'), 'utf8')
      expect(secondPageCode).not.toContain('__ISSUE_826_UTIL_SHARED__')
      expect(secondPageCode).toMatch(/issue-fixtures\/issue-826\/utils\/shared\.js/)
      await fs.remove(distRoot)
    }
  })
  ...
  it('issue #553: maps component v-model arguments to the matching prop and update event', async () => {
    ...
    const componentJsPath = path.join(DIST_ROOT, 'components/issue-553/ModelArgumentProbe/index.js')
    const pageWxml = await fs.readFile(pageWxmlPath, 'utf-8')
    const pageJs = await fs.readFile(pageJsPath, 'utf-8')
    const componentWxml = await fs.readFile(componentWxmlPath, 'utf-8')
    ...
    expect(componentWxml).toContain('issue553-update-abc')
    expect(componentWxml).toContain('issue553-update-model')
    expect(componentWxml).toContain('bindtap="__weapp_vite_inline"')
    expect(componentWxml).toContain('data-wi-tap=')
    expect(componentJs).toContain('updateAbc')
    expect(componentJs).toContain('updateModelValue')
  })
  ...
  it('issue #615: compiles scoped slot v-for owner list through a safe runtime binding', async () => {
    ...
    const scopedSlotWxmlFiles = files
      .filter(file => file.startsWith('pages/issue-615/index.__scoped-slot-') && file.endsWith('.wxml'))
      .sort()
    const scopedSlotJsPath = path.join(DIST_ROOT, 'pages/issue-615/index.__scoped-slot-default-0.js')
    const pageWxml = await fs.readFile(pageWxmlPath, 'utf-8')
    const scopedSlotWxml = (await Promise.all(scopedSlotWxmlFiles.map(file => fs.readFile(path.join(DIST_ROOT, file), 'utf-8')))).join('\n')
  ...
  it('issue #494: removes synthetic view wrappers from plain template v-slot content when possible', async () => {
    ...
    expect(pageWxml).toContain('<weapp-slot-wrapper slot="header"><view class="issue494-header-probe"')
  ...
  it('issue #599: prefers props named data in generated computed bindings', async () => {
    ...
    expect(componentJs).toContain('props: { data: {')
  ...
  it('issue #627: compiles class/style defineProps probe', async () => {
    ...
    expect(nativeComponentJs).toContain('"hidden": Boolean')
    expect(nativeComponentJs).toContain('"dataFoo": String')
    expect(nativeComponentJs).toContain('"data-foo": String')
    expect(nativeComponentJs).toContain('"markFoo": String')
    expect(nativeComponentJs).toContain('"mark:foo": String')
    expect(nativeComponentJs).toContain('"slot": String')
  ...
  it('issue #613: wraps forwarded slot outlets inside plain named slot content', async () => {
    ...
    expect(wrapperJs).toContain('multipleSlots:true')
    expect(legacyForwarderWxml).toContain('<view slot="header"><slot /></view>')
    expect(legacyForwarderJson.usingComponents?.['weapp-slot-wrapper']).toBeUndefined()
  ...
  it('issue #558: augmented scoped slot runtime bindings read owner proxy', async () => {
    ...
    const namedSlotCardJsPath = path.join(DIST_ROOT, 'components/issue-558/NamedSlotCard/index.js')
    ...
    const namedSlotCardJs = await fs.readFile(namedSlotCardJsPath, 'utf-8')
    ...
    expect(cellWxml).not.toContain('<scoped-slots-default')
  ...
  it('experiment: flex parent keeps projected multi-node slot groups visible via view wrappers', async () => {
    ...
    expect(pageWxml).toContain('<weapp-slot-wrapper slot="middle"><view class="slot-flex-item slot-flex-item--middle-multi-center-a" data-case="middle-multi-center-a">')
    expect(pageWxml).toContain('<weapp-slot-wrapper slot="left"><view class="slot-flex-item slot-flex-item--all-multi-left-a" data-case="all-multi-left-a">')
    expect(pageWxml).toContain('<weapp-slot-wrapper slot="right"><view class="slot-flex-item slot-flex-item--all-multi-right-a" data-case="all-multi-right-a">')
  ...
  it('issue #398: emits layout and component importers that share the same wevu runtime chunk', async () => {
    ...
    expect(layoutJs).not.toContain('../components/issue-398/BaseNavbar/index.vue')
    expect(layoutJs).not.toContain('../components/issue-398/BaseFooter/index.vue')
  ...
  it('issue #289: compiles split pages with per-page controls and safe class bindings', async () => {
    ...
    expect(objectPageWxml).toContain('issue289-object-toggle-list')
    expect(objectPageWxml).toContain('issue289-object-toggle-compact')
    expect(objectPageWxml).toContain('issue289-object-cycle-active')
    expect(objectPageWxml).toContain('bindtap="__weapp_vite_inline"')
    expect(objectPageWxml).toContain('data-wi-tap=')
    ...
    expect(objectPageJs).toContain('toggleShowList')
    expect(objectPageJs).toContain('toggleCompactMode')
    expect(objectPageJs).toContain('cycleActive')
    ...
    expect(mapPageWxml).toContain('气泡尺寸')
    expect(mapPageWxml).toContain('气泡列表')
    expect(mapPageWxml).toContain('选中事件')
    expect(mapPageWxml).toContain('bindtap="__weapp_vite_inline"')
    expect(mapPageWxml).toContain('data-wi-tap=')
    ...
    expect(mapPageJs).toContain('toggleCalloutExpanded')
    expect(mapPageJs).toContain('toggleShowCalloutList')
    expect(mapPageJs).toContain('cycleSelectedEvent')
    ...
    expect(rootPageWxml).toContain('issue289-root-cycle-option')
    expect(rootPageWxml).toContain('bindtap="__weapp_vite_inline"')
    expect(rootPageWxml).toContain('data-wi-tap=')
    expect(rootPageJs).toContain('controlState')
    expect(rootPageJs).toContain('.showOptions')
    expect(rootPageJs).toContain('toggleShowOptions')
    expect(rootPageJs).toContain('cycleOption')
    ...
    expect(computedPageWxml).toContain('source：')
    expect(computedPageWxml).toContain('选中项')
    expect(computedPageWxml).toContain('bindtap="__weapp_vite_inline"')
    expect(computedPageWxml).toContain('data-wi-tap=')
    ...
    expect(computedPageJs).toContain('toggleSourceEnabled')
    expect(computedPageJs).toContain('toggleShowItems')
    expect(computedPageJs).toContain('cycleSelected')
  ...
  it('issue #316: compiles kebab-case component events to colon-prefixed bindings', async () => {
    ...
    const emitterJsPath = path.join(DIST_ROOT, 'components/issue-316/HyphenEventEmitter/index.js')

    const issuePageWxml = await fs.readFile(issuePageWxmlPath, 'utf-8')
    const issuePageJs = await fs.readFile(issuePageJsPath, 'utf-8')
    const emitterWxml = await fs.readFile(emitterWxmlPath, 'utf-8')
    const emitterJs = await fs.readFile(emitterJsPath, 'utf-8')
    ...
    expect(emitterWxml).toContain('bindtap="__weapp_vite_inline"')
    expect(emitterWxml).toContain('data-wi-tap=')
    expect(emitterJs).toContain('emitOverlayClick')
  })
  ...
  it('issue #300: keeps boolean props available in runtime call-expression bindings', async () => {
    ...
    expect(probeJs).toContain('boolTextFromProps')
    expect(probeJs).toContain('boolTextFromDestructured')
    expect(probeJs).toContain('allowNullPropInput: true')
    expect(probeJs).toContain('__wevuPropsDerivedKeys: [')
    ...
    expect(strictProbeJs).toContain('allowNullPropInput: true')
    expect(strictProbeJs).toContain('__wevuPropsDerivedKeys: [')
