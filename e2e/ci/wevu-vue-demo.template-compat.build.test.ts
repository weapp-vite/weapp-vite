...
describe.sequential('e2e app: wevu-vue-demo (template compat)', () => {
  it('compiles tuple/object destructuring, object-map v-for and array mutations', async () => {
    ...
    expect(pageWxml).toContain('单节点多事件（参数 + $event）')
    expect(pageWxml).toContain('class="event-zone"')
    expect(pageWxml).toContain('bindtap="__weapp_vite_inline"')
    expect(pageWxml).toContain('bindlongpress="__weapp_vite_inline"')
    expect(pageWxml).toContain('bindtouchstart="__weapp_vite_inline"')
    expect(pageWxml).toContain('bindtouchmove="__weapp_vite_inline"')
    expect(pageWxml).toContain('bindtouchend="__weapp_vite_inline"')
    expect(pageWxml).toContain('bindtouchcancel="__weapp_vite_inline"')
    expect(pageWxml).toContain('data-wv-inline-id-tap="__wv_inline_')
    expect(pageWxml).toContain('data-wv-inline-id-longpress="__wv_inline_')
    expect(pageWxml).toContain('data-wv-inline-id-touchstart="__wv_inline_')
    expect(pageWxml).toContain('data-wv-inline-id-touchmove="__wv_inline_')
    expect(pageWxml).toContain('data-wv-inline-id-touchend="__wv_inline_')
    expect(pageWxml).toContain('data-wv-inline-id-touchcancel="__wv_inline_')
