...
describe.sequential('e2e app: wevu-vue-demo (template compat)', () => {
  it('compiles tuple/object destructuring, object-map v-for and array mutations', async () => {
    ...
    expect(pageWxml).toContain('data-wi-tap="i')
    expect(pageWxml).toContain('data-wi-longpress="i')
    expect(pageWxml).toContain('data-wi-touchstart="i')
    expect(pageWxml).toContain('data-wi-touchmove="i')
    expect(pageWxml).toContain('data-wi-touchend="i')
    expect(pageWxml).toContain('data-wi-touchcancel="i')
    ...
    expect(pageJs).toMatch(/['"`]touchcancel['"`],\s*['"`]zone['"`],\s*[A-Za-z_$][\w$]*/)
  })
})
