...
import { createIssue892ProjectConfig } from './githubIssuesBuild/cases/issue892Project'
...
async function waitForStyleOutput(outputPath: string, markers: string[], timeoutMs = 60_000) {
  ...
  throw new Error(`Timed out waiting for ${path.relative(APP_ROOT, outputPath)} to contain ${markers.join(', ')}`)
}
...
describe('issue #892 Sass asset placeholders', { concurrent: false }, () => {
  ...
  it('keeps unquoted and quoted asset URLs valid across initial dev output and HMR', async () => {
    ...
    const sharedAppPath = path.join(APP_ROOT, 'dist/app.json')
    const sharedApp = await fs.pathExists(sharedAppPath) ? await fs.readFile(sharedAppPath, 'utf8') : undefined
    const project = await createIssue892ProjectConfig(APP_ROOT, 'dev')
    const styleOutputPath = path.join(project.distRoot, 'styles/issue-892-app.wxss')
    await fs.remove(project.distRoot)
    ...
    try {
      ...
      const [initialStyle, initialOutput] = await devProcess.waitFor(Promise.all([
        waitForStyleOutput(styleOutputPath, [
          '.issue-892-unquoted',
          '.issue-892-quoted',
          'goods-1.png',
        ]),
      ...
      const app = await fs.readJSON(path.join(project.distRoot, 'app.json')) as {
        pages?: string[]
        subPackages?: unknown[]
        subpackages?: unknown[]
      }
      ...
      const updatedStyle = await devProcess.waitFor(
        waitForStyleOutput(styleOutputPath, ['color: #ac6824;']),
