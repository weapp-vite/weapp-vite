...
describe('e2e app: wevu-vue-demo (script setup emit compat)', { concurrent: false }, () => {
  it('compiles component emits with Vue-style payload handler semantics', async () => {
    ...
    expect(pageWxml).toContain('<compat-emit-matrix')
    ...
    expect(pageWxml).toContain('data-wd-payload="1"')
    expect(pageWxml).toContain('data-wd-native="1"')
    expect(pageWxml).toContain('data-wd-tuple="1"')
    expect(pageWxml).toContain('data-wd-empty="1"')
    expect(pageWxml).toContain('data-wd-options="1"')
    expect(pageWxml).toContain('data-wi-run="i')
    expect(pageWxml).toContain('data-wi-runevent="i')
    ...
    expect(emitMatrixJs).toMatch(/emit\(\s*['"`]tuple['"`],\s*['"`]alpha['"`],\s*2,\s*\{\s*ok:\s*true\s*\}\s*\)/)
    expect(emitMatrixJs).toMatch(/emit\(\s*['"`]empty['"`]\s*\)/)
    ...
    expect(emitMatrixJs).toMatch(/fn:\s*\(([\w$]+),\s*[\w$]+,\s*([\w$]+)\)\s*=>\s*\1\.emitNative\(\2\)/)
  })
})
