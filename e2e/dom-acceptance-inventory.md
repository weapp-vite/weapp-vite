...
- Operations: `reLaunch(/pages/index/index)`, `check(native:initial)`, `callMethod(refreshLifecycleSummary)`, `check(native:finalized)`, `check(wevu-ts:initial)`, `check(wevu-ts:finalized)`, `check(wevu-vue:initial)`, `check(wevu-vue:finalized)`
