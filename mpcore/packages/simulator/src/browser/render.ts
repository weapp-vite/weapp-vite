...
export function renderBrowserPageTree(
  ...
  const templatePath = join(project.miniprogramRootPath, `${route}.wxml`)
  const templateSource = readBrowserVirtualFile(files, templatePath)
  if (typeof templateSource !== 'string') {
    throw new Error(`Missing page template in browser simulator runtime: ${templatePath}`)
  }
