...
interface RuntimeComponentRegistryEntry {
...
function readTemplateSource(filePath: string) {
...
function resolveComponentAttributeValue(value: string, scope: RuntimeRenderScope) {
  if (isMustacheOnly(value)) {
    const expression = value.trim().slice(2, -2)
...
function applyNodeBindings(node: DomNodeLike, scope: RuntimeRenderScope) {
  if (!isTagNode(node)) {
...
function createLoopScope(scope: RuntimeRenderScope, itemName: string, indexName: string, item: unknown, index: number): RuntimeRenderScope {
  return {
    ...scope,
    data: {
      ...scope.data,
...
function evaluateConditionalBranch(node: DomNodeLike, scope: RuntimeRenderScope) {
...
function renderNodeTree(
  ...
  if (componentEntry) {
    ...
    const componentScope: RuntimeRenderScope = {
