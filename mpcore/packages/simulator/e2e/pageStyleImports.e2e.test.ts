...
import { createIssue779StyleOutputFiles, pageStyleGlobal, pageStylePage, pageStyleTemplate } from '../test/helpers/pageStyleImports'
...
it.each(['imported', 'inline'] as const)('mounts repeated %s WXSS and WXML updates through the real workbench preview without resetting state', async (source) => {
  ...
  try {
    ...
    preview.workbench.run(() => {
      session.files.set('styles/palette.wxss', '')
      if (source === 'inline') {
        session.files.set('pages/shared/index.wxss', '.local-probe { background-color: rgb(31, 41, 55); }')
      }
    })
    await nextTick()
