...
import type { WorkspaceHmrBaseline, WorkspaceHmrThresholds } from '../../scripts/workspace-hmr/baseline'
import type { DynamicReactDeliveryEvidence, DynamicReactMutation } from '../../scripts/workspace-hmr/dynamicReactDelivery'
import type { StatefulHmrAuditEvent } from '../../scripts/workspace-hmr/statefulAuditUpdate'
...
import { assertBenchmarkPrepareCompleted, assertBenchmarkTypeScriptPrepared, createBenchmarkPrepareArgs, discoverBenchmarkTypeScriptProjects } from '../../scripts/benchmarkCheckoutPreparation/typescript'
import {
...
import { collectWorkspaceHmrCleanupErrors, isWorkspaceHmrScenarioRetryable } from '../../scripts/workspace-hmr/cleanup'
import { prepareDynamicReactMutation } from '../../scripts/workspace-hmr/dynamicReactDelivery'
import { isDynamicReactTemplateOutput } from '../../scripts/workspace-hmr/reactTemplate'
import { renderWorkspaceHmrExecution, summarizeWorkspaceHmrExecution } from '../../scripts/workspace-hmr/report'
import {
...
} from '../../scripts/workspace-hmr/scenarios'
import { assertWorkspaceHmrSelection, selectWorkspaceHmrProjects } from '../../scripts/workspace-hmr/selection'
import { StatefulHmrAuditClient } from '../../scripts/workspace-hmr/statefulAuditClient'
import { waitForStatefulHmrAuditUpdate } from '../../scripts/workspace-hmr/statefulAuditUpdate'
...
const scenarioRetries = 0 // 私有诊断固定单次执行，不以重试消除失败。
...
async function auditProject(project: ProjectCase): Promise<ProjectResult> {
  ...
  traceRecord('project-end', { project: project.id })
  await writeFile(path.join(reportRoot, 'dev-output.log'), dev.getOutput(), 'utf8')
  return result
}
...
async function warmupProjectHmr(
  ...
  tracePhase = `${scenario.id}:warmup`
  traceRecord('warmup-start', { scenario: scenario.id })
  ...
  await rm(profilePath, { force: true }).catch(() => {})
  await sleep(settleMs)
  traceRecord('warmup-end', { scenario: scenario.id })
...
async function auditScenario(
  ...
  const original = await readFile(scenario.sourcePath, 'utf8')
  tracePhase = `${scenario.id}:edit`
  traceRecord('scenario-start', { scenario: scenario.id })
  ...
  try {
    const mutation = await prepareScenarioMutation(project, scenario)
    const profileLineCount = await countJsonlLines(profilePath)
    traceRecord('measurement-before')
...
async function restoreScenarioMutation(project: ProjectCase, scenario: ScenarioCase, source: string, marker: string) {
  tracePhase = `${scenario.id}:restore`
  traceRecord('restore-start', { scenario: scenario.id })
  ...
  const result = await waitForScenarioMutation(project, scenario, mutation, marker, false)
  traceRecord('restore-marker-delivered', { scenario: scenario.id })
  return result
}
...
async function snapshotDist(distRoot: string) {
  ...
  for (const filePath of await listFiles(distRoot)) {
    ...
    traceBlobs.set(createHash('sha256').update(content).digest('hex'), content)
    snapshot.set(normalizePath(path.relative(distRoot, filePath)), {
      hash: createHash('sha256').update(content).digest('hex'),
      size: fileStat.size,
    })
  }
  traceRecord('disk-snapshot', { startedAt: traceStartedAt, files: Object.fromEntries(snapshot) })
  return snapshot
}
  ...
  while (Date.now() - startedAt < timeoutMs) {
    ...
    if (signature === previousSignature) {
      if (Date.now() - stableStartedAt >= stableMs) {
        traceRecord('stable-dist-return', { stableMs, signature: createHash('sha256').update(signature).digest('hex') })
        return snapshot
      }
    }
    else {
...
async function writeScenarioSource(filePath: string, content: string) {
  ...
  traceBlobs.set(sha256, Buffer.from(content))
  traceRecord('source-write-start', { file: formatProjectPath(filePath), sha256 })
  if (writeMode === 'rename') {
    await replaceFileByRename(filePath, content)
    return
  }
  await writeFile(filePath, content, 'utf8')
  traceRecord('source-write-end', { file: formatProjectPath(filePath), sha256 })
}
...
let tracePhase = 'startup'
const traceEvents: unknown[] = []
const traceBlobs = new Map<string, Uint8Array>()
function traceRecord(stage: string, data: Record<string, unknown> = {}) {
  traceEvents.push({ at: new Date().toISOString(), monotonicMs: performance.now(), phase: tracePhase, stage, ...data })
}
async function flushTrace() {
  await mkdir(path.join(reportRoot, 'blobs'), { recursive: true })
  await writeFile(path.join(reportRoot, 'timeline.json'), JSON.stringify(traceEvents, null, 2))
  for (const [hash, content] of traceBlobs) {
    await writeFile(path.join(reportRoot, 'blobs', hash), content)
  }
}
