    ...
    <view v-for="(store, si) in _storeGoods" :key="store.storeId || si" class="goods-wrap">
      <view class="cart-store">
        ...
        <view class="cart-store__content">
          ...
          <view class="get-coupon" @tap.stop="gotoCoupons">
            优惠券
          </view>
        </view>
      </view>
      <block v-for="(promotion, promoindex) in store.promotionGoodsList" :key="promoindex">
        <view
          v-if="hasPromotion(promotion.promotionCode)"
        ...
        >
          ...
          <view :class="promotionActionClass" hover-class="action-btn--active [opacity:0.5]">
        ...
        >
          <swipeout :right-width="72">
            <view class="goods-item-info">
              <view class="check-wrap" :data-goods="goods" @tap.stop="selectGoods">
                <t-icon
                  size="40rpx"
              ...
              <view :class="goodsSkuInfoClass">
                ...
                >
                  <template #thumb-cover>
                    <view v-if="goods.shortageStock || goods.stockQuantity <= 3" class="stock-mask">
                      仅剩{{ goods.stockQuantity }}件
                    </view>
                  </template>
                  <template #append-body>
                    <view :class="goodsStepperClass">
                      <view v-if="goods.shortageStock" class="stepper-tip">
                        库存不足
                      </view>
                      <t-stepper
                        ...
                        :data-si="si"
                        theme="filled"
                        @change.stop="changeStepper"
                        @blur.stop="input"
                        @overlimit.stop="overlimit"
                      />
                    </view>
                  </template>
                </goods-card>
            ...
            <template #right>
              <view :class="swipeDeleteClass" :data-goods="goods" @tap="deleteGoods">
                删除
              </view>
            </template>
          </swipeout>
          ...
          <swipeout :right-width="72">
            <view class="goods-item-info">
              ...
              <view :class="goodsSkuInfoClass">
                ...
                >
                  <template #thumb-cover>
                    <view v-if="goods.stockQuantity <= 0" :class="noStorageContentClass">
                      <view class="no-storage-content">
                        无货
                      </view>
                    </view>
                  </template>
