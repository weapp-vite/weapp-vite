...
definePageJson({ navigationBarTitleText: 'up-calendar-strip' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['up-calendar-strip', 'u-calendar-strip'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('up-calendar-strip')
    ?? page?.selectComponent?.('up-calendar-strip')
    ?? registeredChild
    ?? null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="up-calendar-strip">
    <view class="scenario-header">
      <view class="scenario-title">up-calendar-strip</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <up-calendar-strip id="e2e-component" ref="e2eComponent" v-model="selectedDate" />
    </view>
