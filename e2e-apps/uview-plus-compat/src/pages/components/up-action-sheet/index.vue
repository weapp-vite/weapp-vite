...
definePageJson({ navigationBarTitleText: 'up-action-sheet' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['up-action-sheet', 'u-action-sheet'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('up-action-sheet')
    ?? page?.selectComponent?.('up-action-sheet up-action-sheet')
    ?? registeredChild
    ?? null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="up-action-sheet">
    <view class="scenario-header">
      <view class="scenario-title">up-action-sheet</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <up-action-sheet id="e2e-component" ref="e2eComponent" :show="actionSheetVisible" :actions="actions" title="Actions"><up-action-sheet-data :show="false" /></up-action-sheet>
    </view>
