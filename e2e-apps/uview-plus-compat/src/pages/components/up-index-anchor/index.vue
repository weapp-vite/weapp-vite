...
definePageJson({ navigationBarTitleText: 'up-index-anchor' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['up-index-anchor', 'u-index-anchor'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('up-index-anchor')
    ?? page?.selectComponent?.('up-index-list up-index-anchor')
    ?? registeredChild
    ?? null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="up-index-anchor">
    <view class="scenario-header">
      <view class="scenario-title">up-index-anchor</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <up-index-list id="e2e-parent" :index-list="['A']"><up-index-item><up-index-anchor id="e2e-component" ref="e2eComponent" index="A" /><up-cell title="Alpha" /></up-index-item></up-index-list>
    </view>
