...
definePageJson({ navigationBarTitleText: 'up-count-to' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['up-count-to', 'u-count-to'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('up-count-to')
    ?? page?.selectComponent?.('up-count-to')
    ?? registeredChild
    ?? null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="up-count-to">
    <view class="scenario-header">
      <view class="scenario-title">up-count-to</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <up-count-to id="e2e-component" ref="e2eComponent" :start-val="0" :end-val="128" :duration="0" />
    </view>
