...
definePageJson({ navigationBarTitleText: 'up-canvas' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['up-canvas', 'u-canvas'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('up-canvas')
    ?? page?.selectComponent?.('up-canvas')
    ?? registeredChild
    ?? null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="up-canvas">
    <view class="scenario-header">
      <view class="scenario-title">up-canvas</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <up-canvas id="e2e-component" ref="e2eComponent" />
    </view>
