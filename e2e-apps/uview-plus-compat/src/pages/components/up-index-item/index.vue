...
definePageJson({ navigationBarTitleText: 'up-index-item' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['up-index-item', 'u-index-item'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('up-index-item')
    ?? page?.selectComponent?.('up-index-list up-index-item')
    ?? registeredChild
    ?? null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="up-index-item">
    <view class="scenario-header">
      <view class="scenario-title">up-index-item</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <up-index-list id="e2e-parent" :index-list="['A']"><up-index-item id="e2e-component" ref="e2eComponent"><up-index-anchor index="A" /><up-cell title="Alpha" /></up-index-item></up-index-list>
    </view>
