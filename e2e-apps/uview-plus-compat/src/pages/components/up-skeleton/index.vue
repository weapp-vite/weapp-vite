...
definePageJson({ navigationBarTitleText: 'up-skeleton' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['up-skeleton', 'u-skeleton'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('up-skeleton')
    ?? page?.selectComponent?.('up-skeleton')
    ?? registeredChild
    ?? null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="up-skeleton">
    <view class="scenario-header">
      <view class="scenario-title">up-skeleton</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <up-skeleton id="e2e-component" ref="e2eComponent" :loading="false"><view>Loaded content</view></up-skeleton>
    </view>
