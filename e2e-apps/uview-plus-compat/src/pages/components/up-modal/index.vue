...
definePageJson({ navigationBarTitleText: 'up-modal' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['up-modal', 'u-modal'].includes(child?.$options?.name))
    : null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="up-modal">
    <view class="scenario-header">
      <view class="scenario-title">up-modal</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <up-modal id="e2e-component" ref="e2eComponent" :show="modalVisible" title="Modal" content="Stable modal content" />
    </view>
