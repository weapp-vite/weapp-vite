...
definePageJson({ navigationBarTitleText: 'up-car-keyboard' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['up-car-keyboard', 'u-car-keyboard'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('up-car-keyboard')
    ?? page?.selectComponent?.('up-car-keyboard')
    ?? registeredChild
    ?? null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="up-car-keyboard">
    <view class="scenario-header">
      <view class="scenario-title">up-car-keyboard</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <up-car-keyboard id="e2e-component" ref="e2eComponent" v-model="modelValue" />
    </view>
