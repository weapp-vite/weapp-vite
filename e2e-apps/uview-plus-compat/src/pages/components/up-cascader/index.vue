...
definePageJson({ navigationBarTitleText: 'up-cascader' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['up-cascader', 'u-cascader'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('up-cascader')
    ?? page?.selectComponent?.('up-cascader')
    ?? registeredChild
    ?? null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="up-cascader">
    <view class="scenario-header">
      <view class="scenario-title">up-cascader</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <up-cascader id="e2e-component" ref="e2eComponent" v-model="modelValue" :options="list" />
    </view>
