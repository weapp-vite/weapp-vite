...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['up-checkbox-group', 'u-checkbox-group'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('up-checkbox-group')
    ?? page?.selectComponent?.('up-checkbox-group up-checkbox-group')
    ?? registeredChild
    ?? null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="up-checkbox-group">
    <view class="scenario-header">
      <view class="scenario-title">up-checkbox-group</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <up-checkbox-group id="e2e-component" ref="e2eComponent" v-model="modelValue"><up-checkbox name="alpha" label="Alpha" /></up-checkbox-group>
    </view>
