...
definePageJson({ navigationBarTitleText: 'up-picker' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['up-picker', 'u-picker'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('up-picker')
    ?? page?.selectComponent?.('up-picker')
    ?? registeredChild
    ?? null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="up-picker">
    <view class="scenario-header">
      <view class="scenario-title">up-picker</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <up-picker id="e2e-component" ref="e2eComponent" :show="pickerVisible" :columns="columns" />
    </view>
