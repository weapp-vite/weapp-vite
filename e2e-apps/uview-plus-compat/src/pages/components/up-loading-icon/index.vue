...
definePageJson({ navigationBarTitleText: 'up-loading-icon' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['up-loading-icon', 'u-loading-icon'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('up-loading-icon')
    ?? page?.selectComponent?.('up-loading-icon')
    ?? registeredChild
    ?? null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="up-loading-icon">
    <view class="scenario-header">
      <view class="scenario-title">up-loading-icon</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <up-loading-icon id="e2e-component" ref="e2eComponent" />
    </view>
