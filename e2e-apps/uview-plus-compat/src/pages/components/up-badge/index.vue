...
definePageJson({ navigationBarTitleText: 'up-badge' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['up-badge', 'u-badge'].includes(child?.$options?.name))
    : null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="up-badge">
    <view class="scenario-header">
      <view class="scenario-title">up-badge</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <up-badge id="e2e-component" ref="e2eComponent" :value="8"><view class="badge-anchor">Badge</view></up-badge>
    </view>
