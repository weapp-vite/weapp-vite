...
definePageJson({ navigationBarTitleText: 'up-td' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['up-td', 'u-td'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('up-td')
    ?? page?.selectComponent?.('up-table up-td')
    ?? registeredChild
    ?? null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="up-td">
    <view class="scenario-header">
      <view class="scenario-title">up-td</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <up-table id="e2e-parent"><up-tr><up-th>Key</up-th><up-td id="e2e-component" ref="e2eComponent">Value</up-td></up-tr></up-table>
    </view>
