...
definePageJson({ navigationBarTitleText: 'up-checkbox' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['up-checkbox', 'u-checkbox'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('up-checkbox')
    ?? page?.selectComponent?.('up-checkbox')
    ?? registeredChild
    ?? null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="up-checkbox">
    <view class="scenario-header">
      <view class="scenario-title">up-checkbox</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <up-checkbox id="e2e-component" ref="e2eComponent" v-model="modelValue" label="Accepted" name="accepted" used-alone />
    </view>
