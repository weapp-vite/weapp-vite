...
definePageJson({ navigationBarTitleText: 'up-input' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['up-input', 'u-input'].includes(child?.$options?.name))
    : null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="up-input">
    <view class="scenario-header">
      <view class="scenario-title">up-input</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <up-input id="e2e-component" ref="e2eComponent" v-model="modelValue" placeholder="Input" />
    </view>
