...
definePageJson({ navigationBarTitleText: 'up-row' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['up-row', 'u-row'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('up-row')
    ?? page?.selectComponent?.('up-row up-row')
    ?? registeredChild
    ?? null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="up-row">
    <view class="scenario-header">
      <view class="scenario-title">up-row</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <up-row id="e2e-component" ref="e2eComponent" :gutter="12"><up-col :span="12"><view class="grid-block">Left</view></up-col><up-col :span="12"><view class="grid-block">Right</view></up-col></up-row>
    </view>
