...
definePageJson({ navigationBarTitleText: 'up-back-top' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['up-back-top', 'u-back-top'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('up-back-top')
    ?? page?.selectComponent?.('up-back-top')
    ?? registeredChild
    ?? null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="up-back-top">
    <view class="scenario-header">
      <view class="scenario-title">up-back-top</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <up-back-top id="e2e-component" ref="e2eComponent" :scroll-top="800" mode="square" />
    </view>
