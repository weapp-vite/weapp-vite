...
definePageJson({ navigationBarTitleText: 'up-goods-sku' })

const interactionCount = ref(0)
const scenarioState = ref('pending')
const e2eComponent = ref<Record<string, unknown> | null>(null)
const goodsInfo = { title: 'Stable product', price: 88, stock: 5 }
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['up-goods-sku', 'u-goods-sku'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('up-goods-sku')
    ?? page?.selectComponent?.('up-goods-sku')
    ?? registeredChild
    ?? null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="up-goods-sku">
    <view class="scenario-header">
      <view class="scenario-title">up-goods-sku</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <up-goods-sku id="e2e-component" ref="e2eComponent" :goods-info="goodsInfo" :sku-tree="skuTree" :sku-list="skuList" page-inline />
    </view>
