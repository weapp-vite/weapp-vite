...
definePageJson({ navigationBarTitleText: 'up-sticky' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['up-sticky', 'u-sticky'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('up-sticky')
    ?? page?.selectComponent?.('up-sticky')
    ?? registeredChild
    ?? null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="up-sticky">
    <view class="scenario-header">
      <view class="scenario-title">up-sticky</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <up-sticky id="e2e-component" ref="e2eComponent" />
    </view>
