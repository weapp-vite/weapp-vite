...
definePageJson({ navigationBarTitleText: 'up-icon' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['up-icon', 'u-icon'].includes(child?.$options?.name))
    : null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="up-icon">
    <view class="scenario-header">
      <view class="scenario-title">up-icon</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <up-icon id="e2e-component" ref="e2eComponent" name="checkmark-circle" size="28" color="#16a34a" />
    </view>
