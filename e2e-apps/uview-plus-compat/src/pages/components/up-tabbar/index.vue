...
definePageJson({ navigationBarTitleText: 'up-tabbar' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['up-tabbar', 'u-tabbar'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('up-tabbar')
    ?? page?.selectComponent?.('up-tabbar up-tabbar')
    ?? registeredChild
    ?? null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="up-tabbar">
    <view class="scenario-header">
      <view class="scenario-title">up-tabbar</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <up-tabbar id="e2e-component" ref="e2eComponent" v-model="modelValue" :fixed="false"><up-tabbar-item text="Home" icon="home" /><up-tabbar-item text="Profile" icon="account" /></up-tabbar>
    </view>
