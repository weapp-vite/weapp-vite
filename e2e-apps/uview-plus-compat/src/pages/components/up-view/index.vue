...
definePageJson({ navigationBarTitleText: 'up-view' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['up-view', 'u-view'].includes(child?.$options?.name))
    : null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="up-view">
    <view class="scenario-header">
      <view class="scenario-title">up-view</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <up-view id="e2e-component" ref="e2eComponent" text="Stable view content" />
    </view>
