...
definePageJson({ navigationBarTitleText: 'up-grid-item' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['up-grid-item', 'u-grid-item'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('up-grid-item')
    ?? page?.selectComponent?.('up-grid up-grid-item')
    ?? registeredChild
    ?? null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="up-grid-item">
    <view class="scenario-header">
      <view class="scenario-title">up-grid-item</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <up-grid id="e2e-parent" :col="1"><up-grid-item id="e2e-component" ref="e2eComponent" bg-color="#eef3fa" :custom-style="{ minHeight: '80rpx' }"><view class="grid-block">Grid item</view></up-grid-item></up-grid>
    </view>
