...
definePageJson({ navigationBarTitleText: 'up-link' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['up-link', 'u-link'].includes(child?.$options?.name))
    : null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="up-link">
    <view class="scenario-header">
      <view class="scenario-title">up-link</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <up-link id="e2e-component" ref="e2eComponent" href="/" text="Stable link" :under-line="true" />
    </view>
