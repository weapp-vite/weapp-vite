...
definePageJson({ navigationBarTitleText: 'up-code' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['up-code', 'u-code'].includes(child?.$options?.name))
    : null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="up-code">
    <view class="scenario-header">
      <view class="scenario-title">up-code</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <up-code id="e2e-component" ref="e2eComponent" :seconds="60" />
    </view>
