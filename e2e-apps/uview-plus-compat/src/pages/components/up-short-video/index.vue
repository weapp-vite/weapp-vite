...
definePageJson({ navigationBarTitleText: 'up-short-video' })

const interactionCount = ref(0)
const scenarioState = ref('pending')
const e2eComponent = ref<Record<string, unknown> | null>(null)
const videos = [{ videoUrl: sampleVideo, author: { avatar: sampleImage, name: 'Tester', desc: 'Local fixture' }, playbackRate: 1, isLiked: false, likeCount: 0, commentCount: 0, shareCount: 0, isCollected: false, collectCount: 0, progress: 0 }]
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['up-short-video', 'u-short-video'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('up-short-video')
    ?? page?.selectComponent?.('up-short-video')
    ?? registeredChild
    ?? null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="up-short-video">
    <view class="scenario-header">
      <view class="scenario-title">up-short-video</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <up-short-video id="e2e-component" ref="e2eComponent" :video-list="videos" />
    </view>
