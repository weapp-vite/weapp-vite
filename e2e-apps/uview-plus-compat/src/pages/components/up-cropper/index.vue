...
definePageJson({ navigationBarTitleText: 'up-cropper' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['up-cropper', 'u-cropper'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('up-cropper')
    ?? page?.selectComponent?.('up-cropper')
    ?? registeredChild
    ?? null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="up-cropper">
    <view class="scenario-header">
      <view class="scenario-title">up-cropper</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <up-cropper id="e2e-component" ref="e2eComponent" />
    </view>
