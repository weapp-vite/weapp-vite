...
definePageJson({ navigationBarTitleText: 'up-read-more' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['up-read-more', 'u-read-more'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('up-read-more')
    ?? page?.selectComponent?.('up-read-more')
    ?? registeredChild
    ?? null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="up-read-more">
    <view class="scenario-header">
      <view class="scenario-title">up-read-more</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <up-read-more id="e2e-component" ref="e2eComponent" :show-height="60"><view>Stable long content. Stable long content. Stable long content.</view></up-read-more>
    </view>
