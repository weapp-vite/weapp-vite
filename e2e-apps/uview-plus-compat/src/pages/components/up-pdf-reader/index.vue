...
definePageJson({ navigationBarTitleText: 'up-pdf-reader' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['up-pdf-reader', 'u-pdf-reader'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('up-pdf-reader')
    ?? page?.selectComponent?.('up-pdf-reader')
    ?? registeredChild
    ?? null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="up-pdf-reader">
    <view class="scenario-header">
      <view class="scenario-title">up-pdf-reader</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <up-pdf-reader id="e2e-component" ref="e2eComponent" />
    </view>
