...
definePageJson({ navigationBarTitleText: 'up-grid' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['up-grid', 'u-grid'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('up-grid')
    ?? page?.selectComponent?.('up-grid up-grid')
    ?? registeredChild
    ?? null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="up-grid">
    <view class="scenario-header">
      <view class="scenario-title">up-grid</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <up-grid id="e2e-component" ref="e2eComponent" :col="2"><up-grid-item bg-color="#eef3fa" :custom-style="{ minHeight: '80rpx' }"><view class="grid-block">Home</view></up-grid-item><up-grid-item bg-color="#e8f5ec" :custom-style="{ minHeight: '80rpx' }"><view class="grid-block">Profile</view></up-grid-item></up-grid>
    </view>
