...
definePageJson({ navigationBarTitleText: 'up-picker-data' })

const interactionCount = ref(0)
const scenarioState = ref('pending')
const e2eComponent = ref<Record<string, unknown> | null>(null)
const pickerData = [{ label: 'Alpha', value: 'alpha' }]
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['up-picker-data', 'u-picker-data'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('up-picker-data')
    ?? page?.selectComponent?.('up-picker-data')
    ?? registeredChild
    ?? null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="up-picker-data">
    <view class="scenario-header">
      <view class="scenario-title">up-picker-data</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <up-picker-data id="e2e-component" ref="e2eComponent" :columns="pickerData" />
    </view>
