...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['up-album', 'u-album'].includes(child?.$options?.name))
    : null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="up-album">
    <view class="scenario-header">
      <view class="scenario-title">up-album</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <up-album id="e2e-component" ref="e2eComponent" :urls="urls" :row-count="2" />
    </view>
