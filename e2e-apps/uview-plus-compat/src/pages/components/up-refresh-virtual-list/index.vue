...
definePageJson({ navigationBarTitleText: 'up-refresh-virtual-list' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['up-refresh-virtual-list', 'u-refresh-virtual-list'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('up-refresh-virtual-list')
    ?? page?.selectComponent?.('up-refresh-virtual-list')
    ?? registeredChild
    ?? null
  ...
  return {
    ok: rendered,
    component: 'up-refresh-virtual-list',
    rendered,
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="up-refresh-virtual-list">
    <view class="scenario-header">
      <view class="scenario-title">up-refresh-virtual-list</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <up-refresh-virtual-list id="e2e-component" ref="e2eComponent" />
    </view>
