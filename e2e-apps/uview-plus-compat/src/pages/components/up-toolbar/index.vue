...
definePageJson({ navigationBarTitleText: 'up-toolbar' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['up-toolbar', 'u-toolbar'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('up-toolbar')
    ?? page?.selectComponent?.('up-toolbar')
    ?? registeredChild
    ?? null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="up-toolbar">
    <view class="scenario-header">
      <view class="scenario-title">up-toolbar</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <up-toolbar id="e2e-component" ref="e2eComponent" />
    </view>
