...
definePageJson({ navigationBarTitleText: 'up-message-input' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['up-message-input', 'u-message-input'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('up-message-input')
    ?? page?.selectComponent?.('up-message-input')
    ?? registeredChild
    ?? null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="up-message-input">
    <view class="scenario-header">
      <view class="scenario-title">up-message-input</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <up-message-input id="e2e-component" ref="e2eComponent" v-model="modelValue" :maxlength="6" />
    </view>
