...
definePageJson({ navigationBarTitleText: 'up-number-keyboard' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['up-number-keyboard', 'u-number-keyboard'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('up-number-keyboard')
    ?? page?.selectComponent?.('up-number-keyboard')
    ?? registeredChild
    ?? null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="up-number-keyboard">
    <view class="scenario-header">
      <view class="scenario-title">up-number-keyboard</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <up-number-keyboard id="e2e-component" ref="e2eComponent" v-model="modelValue" />
    </view>
