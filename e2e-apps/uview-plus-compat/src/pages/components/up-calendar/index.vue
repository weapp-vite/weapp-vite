...
definePageJson({ navigationBarTitleText: 'up-calendar' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['up-calendar', 'u-calendar'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('up-calendar')
    ?? page?.selectComponent?.('up-calendar')
    ?? registeredChild
    ?? null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="up-calendar">
    <view class="scenario-header">
      <view class="scenario-title">up-calendar</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <up-calendar id="e2e-component" ref="e2eComponent" :show="calendarVisible" :min-date="1735689600000" :max-date="1738281600000" />
    </view>
