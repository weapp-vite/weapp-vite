...
definePageJson({ navigationBarTitleText: 'up-th' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['up-th', 'u-th'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('up-th')
    ?? page?.selectComponent?.('up-table up-th')
    ?? registeredChild
    ?? null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="up-th">
    <view class="scenario-header">
      <view class="scenario-title">up-th</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <up-table id="e2e-parent"><up-tr><up-th id="e2e-component" ref="e2eComponent">Header</up-th></up-tr></up-table>
    </view>
