...
definePageJson({ navigationBarTitleText: 'up-markdown' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['up-markdown', 'u-markdown'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('up-markdown')
    ?? page?.selectComponent?.('up-markdown')
    ?? registeredChild
    ?? null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="up-markdown">
    <view class="scenario-header">
      <view class="scenario-title">up-markdown</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <up-markdown id="e2e-component" ref="e2eComponent" content="# Stable markdown" />
    </view>
