...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['up-form-item', 'u-form-item'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('up-form-item')
    ?? page?.selectComponent?.('up-form up-form-item')
    ?? registeredChild
    ?? null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="up-form-item">
    <view class="scenario-header">
      <view class="scenario-title">up-form-item</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <up-form id="e2e-parent" :model="formModel"><up-form-item id="e2e-component" ref="e2eComponent" label="Name" prop="name"><up-input v-model="formModel.name" /></up-form-item></up-form>
    </view>
