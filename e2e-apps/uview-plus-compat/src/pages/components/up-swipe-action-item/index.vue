...
definePageJson({ navigationBarTitleText: 'up-swipe-action-item' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['up-swipe-action-item', 'u-swipe-action-item'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('up-swipe-action-item')
    ?? page?.selectComponent?.('up-swipe-action up-swipe-action-item')
    ?? registeredChild
    ?? null
  ...
  return {
    ok: rendered,
    component: 'up-swipe-action-item',
    rendered,
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="up-swipe-action-item">
    <view class="scenario-header">
      <view class="scenario-title">up-swipe-action-item</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <up-swipe-action id="e2e-parent"><up-swipe-action-item id="e2e-component" ref="e2eComponent" :options="[{ text: 'Delete' }]"><up-cell title="Swipe item" /></up-swipe-action-item></up-swipe-action>
    </view>
