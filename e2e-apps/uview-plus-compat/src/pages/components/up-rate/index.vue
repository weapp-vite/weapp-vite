...
definePageJson({ navigationBarTitleText: 'up-rate' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['up-rate', 'u-rate'].includes(child?.$options?.name))
    : null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="up-rate">
    <view class="scenario-header">
      <view class="scenario-title">up-rate</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <up-rate id="e2e-component" ref="e2eComponent" v-model="modelValue" :count="5" />
    </view>
