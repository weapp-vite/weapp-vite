...
definePageJson({ navigationBarTitleText: 'up-pull-refresh' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['up-pull-refresh', 'u-pull-refresh'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('up-pull-refresh')
    ?? page?.selectComponent?.('up-pull-refresh')
    ?? registeredChild
    ?? null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="up-pull-refresh">
    <view class="scenario-header">
      <view class="scenario-title">up-pull-refresh</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <up-pull-refresh id="e2e-component" ref="e2eComponent" v-model="modelValue"><view>Pull refresh content</view></up-pull-refresh>
    </view>
