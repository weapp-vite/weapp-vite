...
definePageJson({ navigationBarTitleText: 'up-picker-column' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['up-picker-column', 'u-picker-column'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('up-picker-column')
    ?? page?.selectComponent?.('up-picker-column')
    ?? registeredChild
    ?? null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="up-picker-column">
    <view class="scenario-header">
      <view class="scenario-title">up-picker-column</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <up-picker-column id="e2e-component" ref="e2eComponent" :item-height="44" :default-index="0" :props="{ values: columns }" />
    </view>
