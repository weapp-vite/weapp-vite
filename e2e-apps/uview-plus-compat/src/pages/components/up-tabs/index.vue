...
definePageJson({ navigationBarTitleText: 'up-tabs' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['up-tabs', 'u-tabs'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('up-tabs')
    ?? page?.selectComponent?.('up-tabs up-tabs')
    ?? registeredChild
    ?? null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="up-tabs">
    <view class="scenario-header">
      <view class="scenario-title">up-tabs</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <up-tabs id="e2e-component" ref="e2eComponent" v-model="modelValue" :list="tabs"><up-tabs-item name="alpha">Alpha content</up-tabs-item></up-tabs>
    </view>
