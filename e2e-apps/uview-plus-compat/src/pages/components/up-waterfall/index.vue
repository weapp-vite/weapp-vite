...
definePageJson({ navigationBarTitleText: 'up-waterfall' })

const interactionCount = ref(0)
const scenarioState = ref('pending')
const e2eComponent = ref<Record<string, unknown> | null>(null)
const waterfallList = ref([{ id: 1, title: 'Alpha' }, { id: 2, title: 'Beta' }])
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['up-waterfall', 'u-waterfall'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('up-waterfall')
    ?? page?.selectComponent?.('up-waterfall')
    ?? registeredChild
    ?? null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="up-waterfall">
    <view class="scenario-header">
      <view class="scenario-title">up-waterfall</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <up-waterfall id="e2e-component" ref="e2eComponent" v-model="waterfallList"><template #left="{ leftList }"><view v-for="item in leftList" :key="item.id">{{ item.title }}</view></template><template #right="{ rightList }"><view v-for="item in rightList" :key="item.id">{{ item.title }}</view></template></up-waterfall>
    </view>
