...
definePageJson({ navigationBarTitleText: 'up-tree' })

const interactionCount = ref(0)
const scenarioState = ref('pending')
const e2eComponent = ref<Record<string, unknown> | null>(null)
const treeData = [{ id: 1, label: 'Alpha', children: [{ id: 2, label: 'Beta' }] }]
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['up-tree', 'u-tree'].includes(child?.$options?.name))
    : null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="up-tree">
    <view class="scenario-header">
      <view class="scenario-title">up-tree</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <up-tree id="e2e-component" ref="e2eComponent" :data="treeData" node-key="id" />
    </view>
