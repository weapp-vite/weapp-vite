...
definePageJson({ navigationBarTitleText: 'up-no-network' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['up-no-network', 'u-no-network'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('up-no-network')
    ?? page?.selectComponent?.('up-no-network')
    ?? registeredChild
    ?? null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="up-no-network">
    <view class="scenario-header">
      <view class="scenario-title">up-no-network</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <up-no-network id="e2e-component" ref="e2eComponent" />
    </view>
