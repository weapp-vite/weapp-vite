...
definePageJson({ navigationBarTitleText: 'up-slider' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['up-slider', 'u-slider'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('up-slider')
    ?? page?.selectComponent?.('up-slider')
    ?? registeredChild
    ?? null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="up-slider">
    <view class="scenario-header">
      <view class="scenario-title">up-slider</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <up-slider id="e2e-component" ref="e2eComponent" v-model="modelValue" />
    </view>
