...
definePageJson({ navigationBarTitleText: 'up-alert' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['up-alert', 'u-alert'].includes(child?.$options?.name))
    : null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="up-alert">
    <view class="scenario-header">
      <view class="scenario-title">up-alert</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <up-alert id="e2e-component" ref="e2eComponent" title="Compatibility alert" description="Stable local content" type="success" />
    </view>
