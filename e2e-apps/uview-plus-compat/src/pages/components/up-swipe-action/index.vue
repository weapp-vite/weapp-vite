...
definePageJson({ navigationBarTitleText: 'up-swipe-action' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['up-swipe-action', 'u-swipe-action'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('up-swipe-action')
    ?? page?.selectComponent?.('up-swipe-action up-swipe-action')
    ?? registeredChild
    ?? null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="up-swipe-action">
    <view class="scenario-header">
      <view class="scenario-title">up-swipe-action</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <up-swipe-action id="e2e-component" ref="e2eComponent"><up-swipe-action-item :options="[{ text: 'Delete' }]"><up-cell title="Swipe item" /></up-swipe-action-item></up-swipe-action>
    </view>
