...
definePageJson({ navigationBarTitleText: 'up-navbar' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['up-navbar', 'u-navbar'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('up-navbar')
    ?? page?.selectComponent?.('up-navbar')
    ?? registeredChild
    ?? null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="up-navbar">
    <view class="scenario-header">
      <view class="scenario-title">up-navbar</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <up-navbar id="e2e-component" ref="e2eComponent" title="Navigation" :auto-back="false" />
    </view>
