...
definePageJson({ navigationBarTitleText: 'up-parse' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['up-parse', 'u-parse'].includes(child?.$options?.name))
    : null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="up-parse">
    <view class="scenario-header">
      <view class="scenario-title">up-parse</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <up-parse id="e2e-component" ref="e2eComponent" content="<p><strong>Stable</strong> parsed content</p>" />
    </view>
