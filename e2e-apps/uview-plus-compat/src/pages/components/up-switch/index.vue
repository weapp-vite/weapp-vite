...
definePageJson({ navigationBarTitleText: 'up-switch' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['up-switch', 'u-switch'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('up-switch')
    ?? page?.selectComponent?.('up-switch')
    ?? registeredChild
    ?? null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="up-switch">
    <view class="scenario-header">
      <view class="scenario-title">up-switch</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <up-switch id="e2e-component" ref="e2eComponent" v-model="modelValue" />
    </view>
