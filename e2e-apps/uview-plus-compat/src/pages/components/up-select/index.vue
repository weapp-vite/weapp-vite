...
definePageJson({ navigationBarTitleText: 'up-select' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['up-select', 'u-select'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('up-select')
    ?? page?.selectComponent?.('up-select')
    ?? registeredChild
    ?? null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="up-select">
    <view class="scenario-header">
      <view class="scenario-title">up-select</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <up-select id="e2e-component" ref="e2eComponent" v-model="modelValue" :list="list" />
    </view>
