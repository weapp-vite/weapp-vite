...
definePageJson({ navigationBarTitleText: 'up-card' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['up-card', 'u-card'].includes(child?.$options?.name))
    : null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="up-card">
    <view class="scenario-header">
      <view class="scenario-title">up-card</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <up-card id="e2e-component" ref="e2eComponent" title="Compatibility card"><view>Stable card content</view></up-card>
    </view>
