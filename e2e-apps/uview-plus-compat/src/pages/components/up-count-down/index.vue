...
definePageJson({ navigationBarTitleText: 'up-count-down' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['up-count-down', 'u-count-down'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('up-count-down')
    ?? page?.selectComponent?.('up-count-down')
    ?? registeredChild
    ?? null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="up-count-down">
    <view class="scenario-header">
      <view class="scenario-title">up-count-down</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <up-count-down id="e2e-component" ref="e2eComponent" :time="65000" format="mm:ss" :auto-start="false" />
    </view>
