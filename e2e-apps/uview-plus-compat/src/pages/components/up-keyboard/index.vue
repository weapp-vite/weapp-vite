...
definePageJson({ navigationBarTitleText: 'up-keyboard' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['up-keyboard', 'u-keyboard'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('up-keyboard')
    ?? page?.selectComponent?.('up-keyboard')
    ?? registeredChild
    ?? null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="up-keyboard">
    <view class="scenario-header">
      <view class="scenario-title">up-keyboard</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <up-keyboard id="e2e-component" ref="e2eComponent" v-model="modelValue" mode="number" />
    </view>
