...
definePageJson({ navigationBarTitleText: 'up-list-item' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['up-list-item', 'u-list-item'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('up-list-item')
    ?? page?.selectComponent?.('up-list up-list-item')
    ?? registeredChild
    ?? null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="up-list-item">
    <view class="scenario-header">
      <view class="scenario-title">up-list-item</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <up-list id="e2e-parent" height="180"><up-list-item id="e2e-component" ref="e2eComponent"><up-cell title="List item" /></up-list-item></up-list>
    </view>
