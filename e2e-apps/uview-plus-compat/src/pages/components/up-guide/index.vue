...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['up-guide', 'u-guide'].includes(child?.$options?.name))
    : null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="up-guide">
    <view class="scenario-header">
      <view class="scenario-title">up-guide</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <up-guide id="e2e-component" ref="e2eComponent" v-model="guideVisible" :steps="guideSteps" />
    </view>
