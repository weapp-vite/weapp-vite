...
definePageJson({ navigationBarTitleText: 'up-textarea' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['up-textarea', 'u-textarea'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('up-textarea')
    ?? page?.selectComponent?.('up-textarea')
    ?? registeredChild
    ?? null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="up-textarea">
    <view class="scenario-header">
      <view class="scenario-title">up-textarea</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <up-textarea id="e2e-component" ref="e2eComponent" v-model="modelValue" placeholder="Textarea" />
    </view>
