...
definePageJson({ navigationBarTitleText: 'up-choose' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['up-choose', 'u-choose'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('up-choose')
    ?? page?.selectComponent?.('up-choose')
    ?? registeredChild
    ?? null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="up-choose">
    <view class="scenario-header">
      <view class="scenario-title">up-choose</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <up-choose id="e2e-component" ref="e2eComponent" :list="list" />
    </view>
