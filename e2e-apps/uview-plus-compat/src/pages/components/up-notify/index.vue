...
definePageJson({ navigationBarTitleText: 'up-notify' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['up-notify', 'u-notify'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('up-notify')
    ?? page?.selectComponent?.('up-notify')
    ?? registeredChild
    ?? null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="up-notify">
    <view class="scenario-header">
      <view class="scenario-title">up-notify</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <up-notify id="e2e-component" ref="e2eComponent" />
    </view>
