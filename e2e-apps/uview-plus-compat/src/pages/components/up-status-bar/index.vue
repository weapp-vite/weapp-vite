...
definePageJson({ navigationBarTitleText: 'up-status-bar' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['up-status-bar', 'u-status-bar'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('up-status-bar')
    ?? page?.selectComponent?.('up-status-bar')
    ?? registeredChild
    ?? null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="up-status-bar">
    <view class="scenario-header">
      <view class="scenario-title">up-status-bar</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <up-status-bar id="e2e-component" ref="e2eComponent" />
    </view>
