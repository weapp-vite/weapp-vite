...
definePageJson({ navigationBarTitleText: 'up-tag' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['up-tag', 'u-tag'].includes(child?.$options?.name))
    : null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="up-tag">
    <view class="scenario-header">
      <view class="scenario-title">up-tag</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <up-tag id="e2e-component" ref="e2eComponent" />
    </view>
