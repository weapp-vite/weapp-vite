...
definePageJson({ navigationBarTitleText: 'up-lazy-load' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['up-lazy-load', 'u-lazy-load'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('up-lazy-load')
    ?? page?.selectComponent?.('up-lazy-load')
    ?? registeredChild
    ?? null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="up-lazy-load">
    <view class="scenario-header">
      <view class="scenario-title">up-lazy-load</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <up-lazy-load id="e2e-component" ref="e2eComponent" />
    </view>
