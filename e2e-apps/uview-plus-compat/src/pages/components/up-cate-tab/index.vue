...
definePageJson({ navigationBarTitleText: 'up-cate-tab' })

const interactionCount = ref(0)
const scenarioState = ref('pending')
const e2eComponent = ref<Record<string, unknown> | null>(null)
const categories = [{ name: 'Alpha', children: [{ name: 'Item' }] }]
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['up-cate-tab', 'u-cate-tab'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('up-cate-tab')
    ?? page?.selectComponent?.('up-cate-tab')
    ?? registeredChild
    ?? null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="up-cate-tab">
    <view class="scenario-header">
      <view class="scenario-title">up-cate-tab</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <up-cate-tab id="e2e-component" ref="e2eComponent" :list="categories" />
    </view>
