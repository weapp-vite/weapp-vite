...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['up-color-picker', 'u-color-picker'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('up-color-picker')
    ?? page?.selectComponent?.('up-color-picker')
    ?? registeredChild
    ?? null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="up-color-picker">
    <view class="scenario-header">
      <view class="scenario-title">up-color-picker</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <up-color-picker id="e2e-component" ref="e2eComponent" v-model="modelValue" />
    </view>
