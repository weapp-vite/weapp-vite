...
definePageJson({ navigationBarTitleText: 'up-table' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['up-table', 'u-table'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('up-table')
    ?? page?.selectComponent?.('up-table up-table')
    ?? registeredChild
    ?? null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="up-table">
    <view class="scenario-header">
      <view class="scenario-title">up-table</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <up-table id="e2e-component" ref="e2eComponent"><up-tr><up-th>Key</up-th><up-th>Value</up-th></up-tr><up-tr><up-td>Status</up-td><up-td>Ready</up-td></up-tr></up-table>
    </view>
