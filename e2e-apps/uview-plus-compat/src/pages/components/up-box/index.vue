...
definePageJson({ navigationBarTitleText: 'up-box' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['up-box', 'u-box'].includes(child?.$options?.name))
    : null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="up-box">
    <view class="scenario-header">
      <view class="scenario-title">up-box</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <up-box id="e2e-component" ref="e2eComponent" text="Stable box content" />
    </view>
