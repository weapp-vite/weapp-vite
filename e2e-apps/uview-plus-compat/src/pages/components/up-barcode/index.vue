...
definePageJson({ navigationBarTitleText: 'up-barcode' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['up-barcode', 'u-barcode'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('up-barcode')
    ?? page?.selectComponent?.('up-barcode')
    ?? registeredChild
    ?? null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="up-barcode">
    <view class="scenario-header">
      <view class="scenario-title">up-barcode</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <up-barcode id="e2e-component" ref="e2eComponent" value="123456789012" :width="280" :height="90" />
    </view>
