...
definePageJson({ navigationBarTitleText: 'up-avatar-group' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['up-avatar-group', 'u-avatar-group'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('up-avatar-group')
    ?? page?.selectComponent?.('up-avatar-group up-avatar-group')
    ?? registeredChild
    ?? null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="up-avatar-group">
    <view class="scenario-header">
      <view class="scenario-title">up-avatar-group</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <up-avatar-group id="e2e-component" ref="e2eComponent"><up-avatar text="A" /><up-avatar text="B" /></up-avatar-group>
    </view>
