...
definePageJson({ navigationBarTitleText: 'up-swiper' })

const interactionCount = ref(0)
const scenarioState = ref('pending')
const e2eComponent = ref<Record<string, unknown> | null>(null)
const swiperList = [{ image: sampleImage, title: 'Alpha' }, { image: sampleAltImage, title: 'Beta' }]
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['up-swiper', 'u-swiper'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('up-swiper')
    ?? page?.selectComponent?.('up-swiper up-swiper')
    ?? registeredChild
    ?? null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="up-swiper">
    <view class="scenario-header">
      <view class="scenario-title">up-swiper</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <up-swiper id="e2e-component" ref="e2eComponent" :list="swiperList" :autoplay="false"><up-swiper-indicator :current="0" :length="2" /></up-swiper>
    </view>
