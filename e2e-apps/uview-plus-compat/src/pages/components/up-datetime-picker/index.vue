...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['up-datetime-picker', 'u-datetime-picker'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('up-datetime-picker')
    ?? page?.selectComponent?.('up-datetime-picker')
    ?? registeredChild
    ?? null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="up-datetime-picker">
    <view class="scenario-header">
      <view class="scenario-title">up-datetime-picker</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <up-datetime-picker id="e2e-component" ref="e2eComponent" v-model="modelValue" :show="pickerVisible" mode="datetime" />
    </view>
