...
definePageJson({ navigationBarTitleText: 'up-radio' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['up-radio', 'u-radio'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('up-radio')
    ?? page?.selectComponent?.('up-radio-group up-radio')
    ?? registeredChild
    ?? null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="up-radio">
    <view class="scenario-header">
      <view class="scenario-title">up-radio</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <up-radio-group id="e2e-parent" v-model="modelValue"><up-radio id="e2e-component" ref="e2eComponent" name="alpha" label="Alpha" /></up-radio-group>
    </view>
