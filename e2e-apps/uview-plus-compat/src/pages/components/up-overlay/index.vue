...
definePageJson({ navigationBarTitleText: 'up-overlay' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['up-overlay', 'u-overlay'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('up-overlay')
    ?? page?.selectComponent?.('up-overlay')
    ?? registeredChild
    ?? null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="up-overlay">
    <view class="scenario-header">
      <view class="scenario-title">up-overlay</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <up-overlay id="e2e-component" ref="e2eComponent" />
    </view>
