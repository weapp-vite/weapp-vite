...
definePageJson({ navigationBarTitleText: 'up-circle-progress' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['up-circle-progress', 'u-circle-progress'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('up-circle-progress')
    ?? page?.selectComponent?.('up-circle-progress')
    ?? registeredChild
    ?? null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="up-circle-progress">
    <view class="scenario-header">
      <view class="scenario-title">up-circle-progress</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <up-circle-progress id="e2e-component" ref="e2eComponent" />
    </view>
