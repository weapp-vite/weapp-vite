...
definePageJson({ navigationBarTitleText: 'up-form' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['up-form', 'u-form'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('up-form')
    ?? page?.selectComponent?.('up-form up-form')
    ?? registeredChild
    ?? null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="up-form">
    <view class="scenario-header">
      <view class="scenario-title">up-form</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <up-form id="e2e-component" ref="e2eComponent" :model="formModel"><up-form-item label="Name" prop="name"><up-input v-model="formModel.name" /></up-form-item></up-form>
    </view>
