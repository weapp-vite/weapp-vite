...
definePageJson({ navigationBarTitleText: 'up-col' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['up-col', 'u-col'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('up-col')
    ?? page?.selectComponent?.('up-row up-col')
    ?? registeredChild
    ?? null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="up-col">
    <view class="scenario-header">
      <view class="scenario-title">up-col</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <up-row id="e2e-parent" :gutter="12"><up-col id="e2e-component" ref="e2eComponent" :span="12"><view class="grid-block">Column</view></up-col></up-row>
    </view>
