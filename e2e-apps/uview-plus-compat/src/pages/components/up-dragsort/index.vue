...
definePageJson({ navigationBarTitleText: 'up-dragsort' })

const interactionCount = ref(0)
const scenarioState = ref('pending')
const e2eComponent = ref<Record<string, unknown> | null>(null)
const dragList = ref([{ id: 1, name: 'Alpha' }, { id: 2, name: 'Beta' }])
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['up-dragsort', 'u-dragsort'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('up-dragsort')
    ?? page?.selectComponent?.('up-dragsort')
    ?? registeredChild
    ?? null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="up-dragsort">
    <view class="scenario-header">
      <view class="scenario-title">up-dragsort</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <up-dragsort id="e2e-component" ref="e2eComponent" v-model="dragList" key-name="id" />
    </view>
