...
definePageJson({ navigationBarTitleText: 'up-line-progress' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['up-line-progress', 'u-line-progress'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('up-line-progress')
    ?? page?.selectComponent?.('up-line-progress')
    ?? registeredChild
    ?? null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="up-line-progress">
    <view class="scenario-header">
      <view class="scenario-title">up-line-progress</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <up-line-progress id="e2e-component" ref="e2eComponent" />
    </view>
