...
definePageJson({ navigationBarTitleText: 'up-collapse' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['up-collapse', 'u-collapse'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('up-collapse')
    ?? page?.selectComponent?.('up-collapse up-collapse')
    ?? registeredChild
    ?? null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="up-collapse">
    <view class="scenario-header">
      <view class="scenario-title">up-collapse</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <up-collapse id="e2e-component" ref="e2eComponent" v-model="modelValue"><up-collapse-item name="item" title="Item">Expanded content</up-collapse-item></up-collapse>
    </view>
