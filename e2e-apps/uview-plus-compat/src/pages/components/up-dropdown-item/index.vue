...
definePageJson({ navigationBarTitleText: 'up-dropdown-item' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['up-dropdown-item', 'u-dropdown-item'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('up-dropdown-item')
    ?? page?.selectComponent?.('up-dropdown up-dropdown-item')
    ?? registeredChild
    ?? null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="up-dropdown-item">
    <view class="scenario-header">
      <view class="scenario-title">up-dropdown-item</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <up-dropdown id="e2e-parent"><up-dropdown-item id="e2e-component" ref="e2eComponent" v-model="dropdownValue" :options="list" /></up-dropdown>
    </view>
