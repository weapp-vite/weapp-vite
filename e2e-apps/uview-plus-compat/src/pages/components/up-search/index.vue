...
definePageJson({ navigationBarTitleText: 'up-search' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['up-search', 'u-search'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('up-search')
    ?? page?.selectComponent?.('up-search')
    ?? registeredChild
    ?? null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="up-search">
    <view class="scenario-header">
      <view class="scenario-title">up-search</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <up-search id="e2e-component" ref="e2eComponent" v-model="modelValue" placeholder="Search" :show-action="false" />
    </view>
