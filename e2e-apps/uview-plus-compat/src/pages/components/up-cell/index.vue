...
definePageJson({ navigationBarTitleText: 'up-cell' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['up-cell', 'u-cell'].includes(child?.$options?.name))
    : null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="up-cell">
    <view class="scenario-header">
      <view class="scenario-title">up-cell</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <up-cell id="e2e-component" ref="e2eComponent" title="Status" value="Ready" label="Representative cell" />
    </view>
