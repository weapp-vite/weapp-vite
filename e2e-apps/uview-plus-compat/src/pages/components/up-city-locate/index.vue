...
definePageJson({ navigationBarTitleText: 'up-city-locate' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['up-city-locate', 'u-city-locate'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('up-city-locate')
    ?? page?.selectComponent?.('up-city-locate')
    ?? registeredChild
    ?? null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="up-city-locate">
    <view class="scenario-header">
      <view class="scenario-title">up-city-locate</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <up-city-locate id="e2e-component" ref="e2eComponent" city="Shanghai" />
    </view>
