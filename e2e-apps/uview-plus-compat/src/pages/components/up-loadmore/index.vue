...
definePageJson({ navigationBarTitleText: 'up-loadmore' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['up-loadmore', 'u-loadmore'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('up-loadmore')
    ?? page?.selectComponent?.('up-loadmore')
    ?? registeredChild
    ?? null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="up-loadmore">
    <view class="scenario-header">
      <view class="scenario-title">up-loadmore</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <up-loadmore id="e2e-component" ref="e2eComponent" status="nomore" nomore-text="No more data" />
    </view>
