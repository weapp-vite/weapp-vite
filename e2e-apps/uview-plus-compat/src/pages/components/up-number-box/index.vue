...
definePageJson({ navigationBarTitleText: 'up-number-box' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['up-number-box', 'u-number-box'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('up-number-box')
    ?? page?.selectComponent?.('up-number-box')
    ?? registeredChild
    ?? null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="up-number-box">
    <view class="scenario-header">
      <view class="scenario-title">up-number-box</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <up-number-box id="e2e-component" ref="e2eComponent" v-model="modelValue" :min="0" :max="5" />
    </view>
