...
definePageJson({ navigationBarTitleText: 'up-avatar' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['up-avatar', 'u-avatar'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('up-avatar')
    ?? page?.selectComponent?.('up-avatar')
    ?? registeredChild
    ?? null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="up-avatar">
    <view class="scenario-header">
      <view class="scenario-title">up-avatar</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <up-avatar id="e2e-component" ref="e2eComponent" text="UP" size="52" />
    </view>
