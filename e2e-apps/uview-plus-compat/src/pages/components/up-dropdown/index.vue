...
definePageJson({ navigationBarTitleText: 'up-dropdown' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['up-dropdown', 'u-dropdown'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('up-dropdown')
    ?? page?.selectComponent?.('up-dropdown up-dropdown')
    ?? registeredChild
    ?? null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="up-dropdown">
    <view class="scenario-header">
      <view class="scenario-title">up-dropdown</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <up-dropdown id="e2e-component" ref="e2eComponent"><up-dropdown-item v-model="dropdownValue" :options="list" /></up-dropdown>
    </view>
