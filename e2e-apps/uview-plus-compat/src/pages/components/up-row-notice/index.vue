...
definePageJson({ navigationBarTitleText: 'up-row-notice' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['up-row-notice', 'u-row-notice'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('up-row-notice')
    ?? page?.selectComponent?.('up-row-notice')
    ?? registeredChild
    ?? null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="up-row-notice">
    <view class="scenario-header">
      <view class="scenario-title">up-row-notice</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <up-row-notice id="e2e-component" ref="e2eComponent" text="Stable notice" :speed="0" />
    </view>
