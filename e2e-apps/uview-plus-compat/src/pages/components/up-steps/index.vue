...
definePageJson({ navigationBarTitleText: 'up-steps' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['up-steps', 'u-steps'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('up-steps')
    ?? page?.selectComponent?.('up-steps up-steps')
    ?? registeredChild
    ?? null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="up-steps">
    <view class="scenario-header">
      <view class="scenario-title">up-steps</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <up-steps id="e2e-component" ref="e2eComponent" :current="1"><up-steps-item title="First" /><up-steps-item title="Second" /></up-steps>
    </view>
