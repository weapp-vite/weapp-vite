...
definePageJson({ navigationBarTitleText: 'up-float-button' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['up-float-button', 'u-float-button'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('up-float-button')
    ?? page?.selectComponent?.('up-float-button')
    ?? registeredChild
    ?? null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="up-float-button">
    <view class="scenario-header">
      <view class="scenario-title">up-float-button</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <up-float-button id="e2e-component" ref="e2eComponent" />
    </view>
