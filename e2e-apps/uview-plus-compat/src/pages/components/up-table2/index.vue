...
definePageJson({ navigationBarTitleText: 'up-table2' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['up-table2', 'u-table2'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('up-table2')
    ?? page?.selectComponent?.('up-table2')
    ?? registeredChild
    ?? null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="up-table2">
    <view class="scenario-header">
      <view class="scenario-title">up-table2</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <up-table2 id="e2e-component" ref="e2eComponent" />
    </view>
