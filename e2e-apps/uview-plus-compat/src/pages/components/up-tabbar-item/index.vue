...
definePageJson({ navigationBarTitleText: 'up-tabbar-item' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['up-tabbar-item', 'u-tabbar-item'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('up-tabbar-item')
    ?? page?.selectComponent?.('up-tabbar up-tabbar-item')
    ?? registeredChild
    ?? null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="up-tabbar-item">
    <view class="scenario-header">
      <view class="scenario-title">up-tabbar-item</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <up-tabbar id="e2e-parent" v-model="modelValue" :fixed="false"><up-tabbar-item id="e2e-component" ref="e2eComponent" text="Home" icon="home" /><up-tabbar-item text="Profile" icon="account" /></up-tabbar>
    </view>
