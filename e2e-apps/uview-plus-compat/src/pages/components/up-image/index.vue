...
definePageJson({ navigationBarTitleText: 'up-image' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['up-image', 'u-image'].includes(child?.$options?.name))
    : null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="up-image">
    <view class="scenario-header">
      <view class="scenario-title">up-image</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <up-image id="e2e-component" ref="e2eComponent" width="120" height="80" :src="sampleImage" mode="aspectFill" />
    </view>
