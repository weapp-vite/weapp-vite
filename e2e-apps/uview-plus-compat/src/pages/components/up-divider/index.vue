...
definePageJson({ navigationBarTitleText: 'up-divider' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['up-divider', 'u-divider'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('up-divider')
    ?? page?.selectComponent?.('up-divider')
    ?? registeredChild
    ?? null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="up-divider">
    <view class="scenario-header">
      <view class="scenario-title">up-divider</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <up-divider id="e2e-component" ref="e2eComponent" text="Divider label" />
    </view>
