...
definePageJson({ navigationBarTitleText: 'up-notice-bar' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['up-notice-bar', 'u-notice-bar'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('up-notice-bar')
    ?? page?.selectComponent?.('up-notice-bar')
    ?? registeredChild
    ?? null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="up-notice-bar">
    <view class="scenario-header">
      <view class="scenario-title">up-notice-bar</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <up-notice-bar id="e2e-component" ref="e2eComponent" :text="['Alpha notice', 'Beta notice']" direction="column" :duration="60000" />
    </view>
