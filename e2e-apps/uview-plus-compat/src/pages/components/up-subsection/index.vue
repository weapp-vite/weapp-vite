...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['up-subsection', 'u-subsection'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('up-subsection')
    ?? page?.selectComponent?.('up-subsection')
    ?? registeredChild
    ?? null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="up-subsection">
    <view class="scenario-header">
      <view class="scenario-title">up-subsection</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <up-subsection id="e2e-component" ref="e2eComponent" v-model="modelValue" :list="sections" />
    </view>
