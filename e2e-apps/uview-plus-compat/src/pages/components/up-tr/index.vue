...
definePageJson({ navigationBarTitleText: 'up-tr' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['up-tr', 'u-tr'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('up-tr')
    ?? page?.selectComponent?.('up-table up-tr')
    ?? registeredChild
    ?? null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="up-tr">
    <view class="scenario-header">
      <view class="scenario-title">up-tr</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <up-table id="e2e-parent"><up-tr id="e2e-component" ref="e2eComponent"><up-td>Row</up-td></up-tr></up-table>
    </view>
