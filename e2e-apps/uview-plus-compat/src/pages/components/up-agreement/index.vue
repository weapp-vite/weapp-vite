...
definePageJson({ navigationBarTitleText: 'up-agreement' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['up-agreement', 'u-agreement'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('up-agreement')
    ?? page?.selectComponent?.('up-agreement')
    ?? registeredChild
    ?? null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="up-agreement">
    <view class="scenario-header">
      <view class="scenario-title">up-agreement</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <up-agreement id="e2e-component" ref="e2eComponent" v-model="modelValue" text="I accept the compatibility terms" />
    </view>
