...
definePageJson({ navigationBarTitleText: 'up-code-input' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['up-code-input', 'u-code-input'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('up-code-input')
    ?? page?.selectComponent?.('up-code-input')
    ?? registeredChild
    ?? null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="up-code-input">
    <view class="scenario-header">
      <view class="scenario-title">up-code-input</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <up-code-input id="e2e-component" ref="e2eComponent" v-model="modelValue" :maxlength="6" />
    </view>
