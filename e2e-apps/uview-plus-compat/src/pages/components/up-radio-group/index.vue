...
definePageJson({ navigationBarTitleText: 'up-radio-group' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['up-radio-group', 'u-radio-group'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('up-radio-group')
    ?? page?.selectComponent?.('up-radio-group up-radio-group')
    ?? registeredChild
    ?? null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="up-radio-group">
    <view class="scenario-header">
      <view class="scenario-title">up-radio-group</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <up-radio-group id="e2e-component" ref="e2eComponent" v-model="modelValue"><up-radio name="alpha" label="Alpha" /><up-radio name="beta" label="Beta" /></up-radio-group>
    </view>
