...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['up-coupon', 'u-coupon'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('up-coupon')
    ?? page?.selectComponent?.('up-coupon')
    ?? registeredChild
    ?? null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="up-coupon">
    <view class="scenario-header">
      <view class="scenario-title">up-coupon</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <up-coupon id="e2e-component" ref="e2eComponent" :list="coupons" />
    </view>
