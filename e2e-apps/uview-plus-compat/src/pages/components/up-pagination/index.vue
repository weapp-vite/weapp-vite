...
definePageJson({ navigationBarTitleText: 'up-pagination' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['up-pagination', 'u-pagination'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('up-pagination')
    ?? page?.selectComponent?.('up-pagination')
    ?? registeredChild
    ?? null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="up-pagination">
    <view class="scenario-header">
      <view class="scenario-title">up-pagination</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <up-pagination id="e2e-component" ref="e2eComponent" v-model="modelValue" :total="60" :page-size="10" />
    </view>
