...
import sampleAltImage from '../../../assets/sample-alt.png'
import sampleImage from '../../../assets/sample.png'

definePageJson({ navigationBarTitleText: 'up-swiper-indicator' })

const interactionCount = ref(0)
const scenarioState = ref('pending')
const e2eComponent = ref<Record<string, unknown> | null>(null)
const swiperList = [sampleImage, sampleAltImage]
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['up-swiper-indicator', 'u-swiper-indicator'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('up-swiper-indicator')
    ?? page?.selectComponent?.('up-swiper up-swiper-indicator')
    ?? registeredChild
    ?? null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="up-swiper-indicator">
    <view class="scenario-header">
      <view class="scenario-title">up-swiper-indicator</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <up-swiper id="e2e-parent" :list="swiperList" :autoplay="false"><up-swiper-indicator id="e2e-component" ref="e2eComponent" :current="0" :length="2" /></up-swiper>
    </view>
