...
definePageJson({ navigationBarTitleText: 'up-gap' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['up-gap', 'u-gap'].includes(child?.$options?.name))
    : null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="up-gap">
    <view class="scenario-header">
      <view class="scenario-title">up-gap</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <up-gap id="e2e-component" ref="e2eComponent" />
    </view>
