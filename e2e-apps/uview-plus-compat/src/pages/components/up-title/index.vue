...
definePageJson({ navigationBarTitleText: 'up-title' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['up-title', 'u-title'].includes(child?.$options?.name))
    : null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="up-title">
    <view class="scenario-header">
      <view class="scenario-title">up-title</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <up-title id="e2e-component" ref="e2eComponent" title="Stable title" subtitle="Compatibility" />
    </view>
