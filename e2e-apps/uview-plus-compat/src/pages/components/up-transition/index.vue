...
definePageJson({ navigationBarTitleText: 'up-transition' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['up-transition', 'u-transition'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('up-transition')
    ?? page?.selectComponent?.('up-transition')
    ?? registeredChild
    ?? null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="up-transition">
    <view class="scenario-header">
      <view class="scenario-title">up-transition</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <up-transition id="e2e-component" ref="e2eComponent" :show="transitionVisible"><view class="transition-content">Transition content</view></up-transition>
    </view>
