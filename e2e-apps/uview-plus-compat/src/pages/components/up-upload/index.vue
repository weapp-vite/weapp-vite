...
definePageJson({ navigationBarTitleText: 'up-upload' })

const interactionCount = ref(0)
const scenarioState = ref('pending')
const e2eComponent = ref<Record<string, unknown> | null>(null)
const fileList = ref([{ url: sampleImage, status: 'success' }])
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['up-upload', 'u-upload'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('up-upload')
    ?? page?.selectComponent?.('up-upload')
    ?? registeredChild
    ?? null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="up-upload">
    <view class="scenario-header">
      <view class="scenario-title">up-upload</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <up-upload id="e2e-component" ref="e2eComponent" :file-list="fileList" :max-count="1" />
    </view>
