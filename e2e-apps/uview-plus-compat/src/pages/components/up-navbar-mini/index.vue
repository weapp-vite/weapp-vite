...
definePageJson({ navigationBarTitleText: 'up-navbar-mini' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['up-navbar-mini', 'u-navbar-mini'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('up-navbar-mini')
    ?? page?.selectComponent?.('up-navbar-mini')
    ?? registeredChild
    ?? null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="up-navbar-mini">
    <view class="scenario-header">
      <view class="scenario-title">up-navbar-mini</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <up-navbar-mini id="e2e-component" ref="e2eComponent" title="Mini navigation" />
    </view>
