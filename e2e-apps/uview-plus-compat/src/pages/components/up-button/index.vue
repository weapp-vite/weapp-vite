...
definePageJson({ navigationBarTitleText: 'up-button' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['up-button', 'u-button'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('up-button')
    ?? page?.selectComponent?.('up-button')
    ?? registeredChild
    ?? null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="up-button">
    <view class="scenario-header">
      <view class="scenario-title">up-button</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <up-button id="e2e-component" ref="e2eComponent" type="primary" text="Primary button" @click="markInteraction" />
    </view>
