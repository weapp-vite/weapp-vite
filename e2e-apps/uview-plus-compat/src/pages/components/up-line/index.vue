...
definePageJson({ navigationBarTitleText: 'up-line' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['up-line', 'u-line'].includes(child?.$options?.name))
    : null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="up-line">
    <view class="scenario-header">
      <view class="scenario-title">up-line</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <up-line id="e2e-component" ref="e2eComponent" />
    </view>
