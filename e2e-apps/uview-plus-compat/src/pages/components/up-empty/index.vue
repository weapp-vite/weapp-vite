...
definePageJson({ navigationBarTitleText: 'up-empty' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['up-empty', 'u-empty'].includes(child?.$options?.name))
    : null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="up-empty">
    <view class="scenario-header">
      <view class="scenario-title">up-empty</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <up-empty id="e2e-component" ref="e2eComponent" text="No data" mode="data" />
    </view>
