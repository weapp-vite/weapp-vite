...
definePageJson({ navigationBarTitleText: 'up-loading-page' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['up-loading-page', 'u-loading-page'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('up-loading-page')
    ?? page?.selectComponent?.('up-loading-page')
    ?? registeredChild
    ?? null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="up-loading-page">
    <view class="scenario-header">
      <view class="scenario-title">up-loading-page</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <up-loading-page id="e2e-component" ref="e2eComponent" :loading="false" loading-text="Ready" />
    </view>
