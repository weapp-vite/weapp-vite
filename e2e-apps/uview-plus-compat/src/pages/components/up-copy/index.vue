...
definePageJson({ navigationBarTitleText: 'up-copy' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['up-copy', 'u-copy'].includes(child?.$options?.name))
    : null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="up-copy">
    <view class="scenario-header">
      <view class="scenario-title">up-copy</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <up-copy id="e2e-component" ref="e2eComponent" content="Stable local text" />
    </view>
