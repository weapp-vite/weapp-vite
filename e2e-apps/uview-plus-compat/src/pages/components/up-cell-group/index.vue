...
definePageJson({ navigationBarTitleText: 'up-cell-group' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['up-cell-group', 'u-cell-group'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('up-cell-group')
    ?? page?.selectComponent?.('up-cell-group up-cell-group')
    ?? registeredChild
    ?? null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="up-cell-group">
    <view class="scenario-header">
      <view class="scenario-title">up-cell-group</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <up-cell-group id="e2e-component" ref="e2eComponent" title="Group"><up-cell title="Status" value="Ready" /></up-cell-group>
    </view>
