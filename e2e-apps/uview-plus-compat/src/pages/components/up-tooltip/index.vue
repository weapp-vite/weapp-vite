...
definePageJson({ navigationBarTitleText: 'up-tooltip' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['up-tooltip', 'u-tooltip'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('up-tooltip')
    ?? page?.selectComponent?.('up-tooltip')
    ?? registeredChild
    ?? null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="up-tooltip">
    <view class="scenario-header">
      <view class="scenario-title">up-tooltip</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <up-tooltip id="e2e-component" ref="e2eComponent" text="Tooltip content"><up-button text="Tooltip" /></up-tooltip>
    </view>
