...
definePageJson({ navigationBarTitleText: 'up-toast' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['up-toast', 'u-toast'].includes(child?.$options?.name))
    : null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="up-toast">
    <view class="scenario-header">
      <view class="scenario-title">up-toast</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <up-toast id="e2e-component" ref="e2eComponent" />
    </view>
