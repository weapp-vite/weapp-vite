...
definePageJson({ navigationBarTitleText: 'up-popover' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['up-popover', 'u-popover'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('up-popover')
    ?? page?.selectComponent?.('up-popover')
    ?? registeredChild
    ?? null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="up-popover">
    <view class="scenario-header">
      <view class="scenario-title">up-popover</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <up-popover id="e2e-component" ref="e2eComponent" v-model="popoverVisible" :list="list"><up-button text="Popover" /></up-popover>
    </view>
