...
definePageJson({ navigationBarTitleText: 'up-list' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['up-list', 'u-list'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('up-list')
    ?? page?.selectComponent?.('up-list up-list')
    ?? registeredChild
    ?? null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="up-list">
    <view class="scenario-header">
      <view class="scenario-title">up-list</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <up-list id="e2e-component" ref="e2eComponent" height="180"><up-list-item><up-cell title="List item" /></up-list-item></up-list>
    </view>
