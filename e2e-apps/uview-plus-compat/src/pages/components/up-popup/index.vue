...
definePageJson({ navigationBarTitleText: 'up-popup' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['up-popup', 'u-popup'].includes(child?.$options?.name))
    : null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="up-popup">
    <view class="scenario-header">
      <view class="scenario-title">up-popup</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <up-popup id="e2e-component" ref="e2eComponent" :show="popupVisible" mode="center"><view class="transition-content">Popup content</view></up-popup>
    </view>
