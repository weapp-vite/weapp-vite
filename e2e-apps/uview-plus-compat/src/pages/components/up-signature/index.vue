...
definePageJson({ navigationBarTitleText: 'up-signature' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['up-signature', 'u-signature'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('up-signature')
    ?? page?.selectComponent?.('up-signature')
    ?? registeredChild
    ?? null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="up-signature">
    <view class="scenario-header">
      <view class="scenario-title">up-signature</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <up-signature id="e2e-component" ref="e2eComponent" />
    </view>
