...
definePageJson({ navigationBarTitleText: 'up-collapse-item' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['up-collapse-item', 'u-collapse-item'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('up-collapse-item')
    ?? page?.selectComponent?.('up-collapse up-collapse-item')
    ?? registeredChild
    ?? null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="up-collapse-item">
    <view class="scenario-header">
      <view class="scenario-title">up-collapse-item</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <up-collapse id="e2e-parent" v-model="modelValue"><up-collapse-item id="e2e-component" ref="e2eComponent" name="item" title="Item">Expanded content</up-collapse-item></up-collapse>
    </view>
