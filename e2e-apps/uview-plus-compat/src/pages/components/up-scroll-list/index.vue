...
definePageJson({ navigationBarTitleText: 'up-scroll-list' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['up-scroll-list', 'u-scroll-list'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('up-scroll-list')
    ?? page?.selectComponent?.('up-scroll-list')
    ?? registeredChild
    ?? null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="up-scroll-list">
    <view class="scenario-header">
      <view class="scenario-title">up-scroll-list</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <up-scroll-list id="e2e-component" ref="e2eComponent"><view class="grid-block">Scrollable content</view></up-scroll-list>
    </view>
