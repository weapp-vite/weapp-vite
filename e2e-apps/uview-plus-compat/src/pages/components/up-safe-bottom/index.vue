...
definePageJson({ navigationBarTitleText: 'up-safe-bottom' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['up-safe-bottom', 'u-safe-bottom'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('up-safe-bottom')
    ?? page?.selectComponent?.('up-safe-bottom')
    ?? registeredChild
    ?? null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="up-safe-bottom">
    <view class="scenario-header">
      <view class="scenario-title">up-safe-bottom</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <up-safe-bottom id="e2e-component" ref="e2eComponent" />
    </view>
