...
definePageJson({ navigationBarTitleText: 'up-text' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['up-text', 'u-text'].includes(child?.$options?.name))
    : null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="up-text">
    <view class="scenario-header">
      <view class="scenario-title">up-text</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <up-text id="e2e-component" ref="e2eComponent" text="Stable text" type="primary" />
    </view>
