...
definePageJson({ navigationBarTitleText: 'up-steps-item' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['up-steps-item', 'u-steps-item'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('up-steps-item')
    ?? page?.selectComponent?.('up-steps up-steps-item')
    ?? registeredChild
    ?? null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="up-steps-item">
    <view class="scenario-header">
      <view class="scenario-title">up-steps-item</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <up-steps id="e2e-parent" :current="1"><up-steps-item id="e2e-component" ref="e2eComponent" title="First" /><up-steps-item title="Second" /></up-steps>
    </view>
