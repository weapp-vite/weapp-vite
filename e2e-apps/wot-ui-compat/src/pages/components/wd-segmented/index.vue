...
definePageJson({ navigationBarTitleText: 'wd-segmented' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['wd-segmented'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('wd-segmented')
    ?? page?.selectComponent?.('wd-segmented')
    ?? registeredChild
    ?? null
  const rendered = target !== null
  const commandReceiver = target
  const command = commandReceiver?.updateActiveStyle
  ...
  if (ok) {
    scenarioState.value = 'pass:command:updateActiveStyle'
  }
  else if (commandError) {
    scenarioState.value = `fail:error:${commandError}`
  }
  else if (!callable) {
    scenarioState.value = 'fail:missing-command:updateActiveStyle'
  }
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="wd-segmented">
    <view class="scenario-header">
      <view class="scenario-title">wd-segmented</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <wd-segmented id="e2e-component" ref="e2eComponent" v-model:value="modelValue" :options="options" />
    </view>
