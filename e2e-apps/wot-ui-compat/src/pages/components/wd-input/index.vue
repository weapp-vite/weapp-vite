...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['wd-input'].includes(child?.$options?.name))
    : null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="wd-input">
    <view class="scenario-header">
      <view class="scenario-title">wd-input</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <wd-input id="e2e-component" ref="e2eComponent" v-model="modelValue" label="Input" clearable @clear="markInteraction" />
    </view>
