...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['wd-textarea'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('wd-textarea')
    ?? page?.selectComponent?.('wd-textarea')
    ?? registeredChild
    ?? null
  const rendered = target !== null
  const commandReceiver = target
  const command = commandReceiver?.handleClear
  ...
  if (ok) {
    scenarioState.value = 'pass:command:handleClear'
  }
  else if (commandError) {
    scenarioState.value = `fail:error:${commandError}`
  }
  else if (!callable) {
    scenarioState.value = 'fail:missing-command:handleClear'
  }
  else if (!eventMatched) {
    scenarioState.value = 'fail:event:clear'
  }
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="wd-textarea">
    <view class="scenario-header">
      <view class="scenario-title">wd-textarea</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <wd-textarea id="e2e-component" ref="e2eComponent" v-model="modelValue" label="Notes" :maxlength="80" @clear="markInteraction" />
    </view>
