...
definePageJson({ navigationBarTitleText: 'wd-tab' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['wd-tab'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('wd-tab')
    ?? page?.selectComponent?.('wd-tabs wd-tab')
    ?? registeredChild
    ?? null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="wd-tab">
    <view class="scenario-header">
      <view class="scenario-title">wd-tab</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <wd-tabs id="e2e-parent" v-model="modelValue"><wd-tab id="e2e-component" ref="e2eComponent" name="first" title="First">Tab content</wd-tab></wd-tabs>
    </view>
