...
definePageJson({ navigationBarTitleText: 'wd-password-input' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['wd-password-input'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('wd-password-input')
    ?? page?.selectComponent?.('wd-password-input')
    ?? registeredChild
    ?? null
  const rendered = target !== null
  const commandReceiver = target
  const command = commandReceiver?.onTouchStart
  ...
  if (ok) {
    scenarioState.value = 'pass:command:onTouchStart'
  }
  else if (commandError) {
    scenarioState.value = `fail:error:${commandError}`
  }
  else if (!callable) {
    scenarioState.value = 'fail:missing-command:onTouchStart'
  }
  else if (!eventMatched) {
    scenarioState.value = 'fail:event:focus'
  }
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="wd-password-input">
    <view class="scenario-header">
      <view class="scenario-title">wd-password-input</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <wd-password-input id="e2e-component" ref="e2eComponent" v-model="modelValue" :length="6" @focus="markInteraction" />
    </view>
