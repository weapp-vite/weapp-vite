...
definePageJson({ navigationBarTitleText: 'wd-swiper' })

const interactionCount = ref(0)
const scenarioState = ref('pending')
const e2eComponent = ref<Record<string, unknown> | null>(null)
const modelValue = ref(0)
const swiperItems = ['/assets/sample.png', '/assets/sample-alt.png']
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['wd-swiper'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('wd-swiper')
    ?? page?.selectComponent?.('wd-swiper wd-swiper')
    ?? registeredChild
    ?? null
  const rendered = target !== null
  const commandReceiver = target
  const command = commandReceiver?.navTo
  const callable = typeof command === 'function'
  let commandError = ''
  if (callable) {
    try {
      const commandResult = command.apply(commandReceiver, [1])
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="wd-swiper">
    <view class="scenario-header">
      <view class="scenario-title">wd-swiper</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <wd-swiper id="e2e-component" ref="e2eComponent" v-model:current="modelValue" :list="swiperItems" :autoplay="false" />
    </view>
