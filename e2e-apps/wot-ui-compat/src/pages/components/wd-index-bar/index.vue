...
definePageJson({ navigationBarTitleText: 'wd-index-bar' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['wd-index-bar'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('wd-index-bar')
    ?? page?.selectComponent?.('wd-index-bar wd-index-bar')
    ?? registeredChild
    ?? null
  const rendered = target !== null
  const commandReceiver = target
  const command = commandReceiver?.init
  ...
  if (ok) {
    scenarioState.value = 'pass:command:init'
  }
  else if (commandError) {
    scenarioState.value = `fail:error:${commandError}`
  }
  else if (!callable) {
    scenarioState.value = 'fail:missing-command:init'
  }
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="wd-index-bar">
    <view class="scenario-header">
      <view class="scenario-title">wd-index-bar</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <wd-index-bar id="e2e-component" ref="e2eComponent"><wd-index-anchor index="A" /><wd-cell title="Alpha" /></wd-index-bar>
    </view>
