...
definePageJson({ navigationBarTitleText: 'wd-slider' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['wd-slider'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('wd-slider')
    ?? page?.selectComponent?.('wd-slider')
    ?? registeredChild
    ?? null
  const rendered = target !== null
  const commandReceiver = target
  const command = commandReceiver?.initSlider
  ...
  if (ok) {
    scenarioState.value = 'pass:command:initSlider'
  }
  else if (commandError) {
    scenarioState.value = `fail:error:${commandError}`
  }
  else if (!callable) {
    scenarioState.value = 'fail:missing-command:initSlider'
  }
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="wd-slider">
    <view class="scenario-header">
      <view class="scenario-title">wd-slider</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <wd-slider id="e2e-component" ref="e2eComponent" v-model="modelValue" :min="0" :max="100" />
    </view>
