...
definePageJson({ navigationBarTitleText: 'wd-navbar-capsule' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['wd-navbar-capsule'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('wd-navbar-capsule')
    ?? page?.selectComponent?.('wd-navbar-capsule')
    ?? registeredChild
    ?? null
  const rendered = target !== null
  const commandReceiver = target
  const command = commandReceiver?.handleBack
  ...
  if (ok) {
    scenarioState.value = 'pass:command:handleBack'
  }
  else if (commandError) {
    scenarioState.value = `fail:error:${commandError}`
  }
  else if (!callable) {
    scenarioState.value = 'fail:missing-command:handleBack'
  }
  else if (!eventMatched) {
    scenarioState.value = 'fail:event:back'
  }
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="wd-navbar-capsule">
    <view class="scenario-header">
      <view class="scenario-title">wd-navbar-capsule</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <wd-navbar-capsule id="e2e-component" ref="e2eComponent" @back="markInteraction" />
    </view>
