...
definePageJson({ navigationBarTitleText: 'wd-row' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['wd-row'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('wd-row')
    ?? page?.selectComponent?.('wd-row wd-row')
    ?? registeredChild
    ?? null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="wd-row">
    <view class="scenario-header">
      <view class="scenario-title">wd-row</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <wd-row id="e2e-component" ref="e2eComponent" :gutter="12"><wd-col :span="12"><view class="grid-block">Left</view></wd-col><wd-col :span="12"><view class="grid-block">Right</view></wd-col></wd-row>
    </view>
