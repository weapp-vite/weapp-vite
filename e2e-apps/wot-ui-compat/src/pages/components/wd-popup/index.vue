...
definePageJson({ navigationBarTitleText: 'wd-popup' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['wd-popup'].includes(child?.$options?.name))
    : null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="wd-popup">
    <view class="scenario-header">
      <view class="scenario-title">wd-popup</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <wd-popup id="e2e-component" ref="e2eComponent" v-model="modelValue" position="bottom" @close="markInteraction"><view class="popup-content">Popup content</view></wd-popup>
    </view>
