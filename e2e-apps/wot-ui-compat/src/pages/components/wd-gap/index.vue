...
definePageJson({ navigationBarTitleText: 'wd-gap' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['wd-gap'].includes(child?.$options?.name))
    : null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="wd-gap">
    <view class="scenario-header">
      <view class="scenario-title">wd-gap</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <wd-gap id="e2e-component" ref="e2eComponent" height="32" bg-color="#dbeafe" />
    </view>
