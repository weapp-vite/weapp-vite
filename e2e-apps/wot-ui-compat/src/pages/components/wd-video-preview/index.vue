...
definePageJson({ navigationBarTitleText: 'wd-video-preview' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['wd-video-preview'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('wd-video-preview')
    ?? page?.selectComponent?.('wd-video-preview')
    ?? registeredChild
    ?? null
  ...
  if (callable) {
    try {
      const commandResult = command.apply(commandReceiver, [{ url: '/assets/sample.mp4', poster: '/assets/sample.png', title: 'Local video', fullScreen: false }])
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="wd-video-preview">
    <view class="scenario-header">
      <view class="scenario-title">wd-video-preview</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <wd-video-preview id="e2e-component" ref="e2eComponent" @open="markInteraction" />
    </view>
