...
definePageJson({ navigationBarTitleText: 'wd-avatar' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['wd-avatar'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('wd-avatar')
    ?? page?.selectComponent?.('wd-avatar')
    ?? registeredChild
    ?? null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="wd-avatar">
    <view class="scenario-header">
      <view class="scenario-title">wd-avatar</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <wd-avatar id="e2e-component" ref="e2eComponent" text="WU" size="large" @click="markInteraction" />
    </view>
