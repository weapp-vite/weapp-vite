...
definePageJson({ navigationBarTitleText: 'wd-signature' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['wd-signature'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('wd-signature')
    ?? page?.selectComponent?.('wd-signature')
    ?? registeredChild
    ?? null
  const rendered = target !== null
  const commandReceiver = target
  const command = commandReceiver?.clear
  ...
  if (ok) {
    scenarioState.value = 'pass:command:clear'
  }
  else if (commandError) {
    scenarioState.value = `fail:error:${commandError}`
  }
  else if (!callable) {
    scenarioState.value = 'fail:missing-command:clear'
  }
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="wd-signature">
    <view class="scenario-header">
      <view class="scenario-title">wd-signature</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <wd-signature id="e2e-component" ref="e2eComponent" @clear="markInteraction" />
    </view>
