...
definePageJson({ navigationBarTitleText: 'wd-image-preview' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['wd-image-preview'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('wd-image-preview')
    ?? page?.selectComponent?.('wd-image-preview')
    ?? registeredChild
    ?? null
  ...
  if (callable) {
    try {
      const commandResult = command.apply(commandReceiver, [['/assets/sample.png', '/assets/sample-alt.png']])
  ...
  if (ok) {
    ...
    scenarioState.value = 'fail:event:open'
  }
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="wd-image-preview">
    <view class="scenario-header">
      <view class="scenario-title">wd-image-preview</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <wd-image-preview id="e2e-component" ref="e2eComponent" :images="['/assets/sample.png', '/assets/sample-alt.png']" @open="markInteraction" />
    </view>
