...
definePageJson({ navigationBarTitleText: 'wd-navbar' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['wd-navbar'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('wd-navbar')
    ?? page?.selectComponent?.('wd-navbar')
    ?? registeredChild
    ?? null
  const rendered = target !== null
  const commandReceiver = target
  const command = commandReceiver?.handleClickLeft
  ...
  if (ok) {
    scenarioState.value = 'pass:command:handleClickLeft'
  }
  else if (commandError) {
    scenarioState.value = `fail:error:${commandError}`
  }
  else if (!callable) {
    scenarioState.value = 'fail:missing-command:handleClickLeft'
  }
  else if (!eventMatched) {
    scenarioState.value = 'fail:event:click-left'
  }
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="wd-navbar">
    <view class="scenario-header">
      <view class="scenario-title">wd-navbar</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <wd-navbar id="e2e-component" ref="e2eComponent" title="Compatibility" left-text="Back" right-text="Done" @click-left="markInteraction" />
    </view>
