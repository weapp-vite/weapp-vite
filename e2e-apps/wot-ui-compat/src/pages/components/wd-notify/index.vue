...
definePageJson({ navigationBarTitleText: 'wd-notify' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['wd-notify'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('wd-notify')
    ?? page?.selectComponent?.('wd-notify')
    ?? registeredChild
    ?? null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="wd-notify">
    <view class="scenario-header">
      <view class="scenario-title">wd-notify</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <wd-notify id="e2e-component" ref="e2eComponent" @click="markInteraction" />
    </view>
