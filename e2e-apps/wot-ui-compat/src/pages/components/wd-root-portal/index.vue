...
definePageJson({ navigationBarTitleText: 'wd-root-portal' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['wd-root-portal'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('wd-root-portal')
    ?? page?.selectComponent?.('wd-root-portal')
    ?? registeredChild
    ?? null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="wd-root-portal">
    <view class="scenario-header">
      <view class="scenario-title">wd-root-portal</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <wd-root-portal id="e2e-component" ref="e2eComponent"><view class="portal-content">Portal content</view></wd-root-portal>
    </view>
