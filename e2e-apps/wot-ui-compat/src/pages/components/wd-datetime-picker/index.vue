...
definePageJson({ navigationBarTitleText: 'wd-datetime-picker' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['wd-datetime-picker'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('wd-datetime-picker')
    ?? page?.selectComponent?.('wd-datetime-picker')
    ?? registeredChild
    ?? null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="wd-datetime-picker">
    <view class="scenario-header">
      <view class="scenario-title">wd-datetime-picker</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <wd-datetime-picker id="e2e-component" ref="e2eComponent" v-model="modelValue" label="Datetime" @open="markInteraction" />
    </view>
