...
definePageJson({ navigationBarTitleText: 'wd-checkbox-group' })

const interactionCount = ref(0)
const scenarioState = ref('pending')
const e2eComponent = ref<Record<string, unknown> | null>(null)
const modelValue = ref<string[]>(['option-a'])
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['wd-checkbox-group'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('wd-checkbox-group')
    ?? page?.selectComponent?.('wd-checkbox-group wd-checkbox-group')
    ?? registeredChild
    ?? null
  const rendered = target !== null
  const commandReceiver = target
  const command = commandReceiver?.toggleAll
  ...
  if (ok) {
    scenarioState.value = 'pass:command:toggleAll'
  }
  else if (commandError) {
    scenarioState.value = `fail:error:${commandError}`
  }
  else if (!callable) {
    scenarioState.value = 'fail:missing-command:toggleAll'
  }
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="wd-checkbox-group">
    <view class="scenario-header">
      <view class="scenario-title">wd-checkbox-group</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <wd-checkbox-group id="e2e-component" ref="e2eComponent" v-model="modelValue" @change="markInteraction"><wd-checkbox model-value="option-a">Option A</wd-checkbox></wd-checkbox-group>
    </view>
