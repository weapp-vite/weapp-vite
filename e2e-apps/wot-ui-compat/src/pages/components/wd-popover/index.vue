...
definePageJson({ navigationBarTitleText: 'wd-popover' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['wd-popover'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('wd-popover')
    ?? page?.selectComponent?.('wd-popover')
    ?? registeredChild
    ?? null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="wd-popover">
    <view class="scenario-header">
      <view class="scenario-title">wd-popover</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <wd-popover id="e2e-component" ref="e2eComponent" v-model="modelValue" content="Popover content" @open="markInteraction"><wd-button size="small">Popover</wd-button></wd-popover>
    </view>
