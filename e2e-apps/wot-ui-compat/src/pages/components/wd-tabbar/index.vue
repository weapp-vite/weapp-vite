...
definePageJson({ navigationBarTitleText: 'wd-tabbar' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['wd-tabbar'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('wd-tabbar')
    ?? page?.selectComponent?.('wd-tabbar wd-tabbar')
    ?? registeredChild
    ?? null
  const rendered = target !== null
  const commandReceiver = target
  const command = commandReceiver?.setChange
  const callable = typeof command === 'function'
  let commandError = ''
  if (callable) {
    try {
      const commandResult = command.apply(commandReceiver, [{ name: 1 }])
  ...
  if (ok) {
    scenarioState.value = 'pass:command:setChange'
  }
  else if (commandError) {
    scenarioState.value = `fail:error:${commandError}`
  }
  else if (!callable) {
    scenarioState.value = 'fail:missing-command:setChange'
  }
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="wd-tabbar">
    <view class="scenario-header">
      <view class="scenario-title">wd-tabbar</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <wd-tabbar id="e2e-component" ref="e2eComponent" v-model="modelValue" fixed="false" @change="markInteraction"><wd-tabbar-item title="Home" icon="home" /><wd-tabbar-item title="Profile" icon="user" /></wd-tabbar>
    </view>
