...
definePageJson({ navigationBarTitleText: 'wd-calendar' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['wd-calendar'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('wd-calendar')
    ?? page?.selectComponent?.('wd-calendar')
    ?? registeredChild
    ?? null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="wd-calendar">
    <view class="scenario-header">
      <view class="scenario-title">wd-calendar</view>
