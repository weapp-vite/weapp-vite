...
definePageJson({ navigationBarTitleText: 'wd-dialog' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['wd-dialog'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('wd-dialog')
    ?? page?.selectComponent?.('wd-dialog')
    ?? registeredChild
    ?? null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="wd-dialog">
    <view class="scenario-header">
      <view class="scenario-title">wd-dialog</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <wd-dialog id="e2e-component" ref="e2eComponent" v-model="modelValue" title="Dialog"><view>Dialog content</view></wd-dialog>
    </view>
