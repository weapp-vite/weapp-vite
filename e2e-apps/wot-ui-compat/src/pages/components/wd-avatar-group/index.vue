...
definePageJson({ navigationBarTitleText: 'wd-avatar-group' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['wd-avatar-group'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('wd-avatar-group')
    ?? page?.selectComponent?.('wd-avatar-group wd-avatar-group')
    ?? registeredChild
    ?? null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="wd-avatar-group">
    <view class="scenario-header">
      <view class="scenario-title">wd-avatar-group</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <wd-avatar-group id="e2e-component" ref="e2eComponent"><wd-avatar text="A" /><wd-avatar text="B" /></wd-avatar-group>
    </view>
