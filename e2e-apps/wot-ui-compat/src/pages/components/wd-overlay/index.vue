...
definePageJson({ navigationBarTitleText: 'wd-overlay' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['wd-overlay'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('wd-overlay')
    ?? page?.selectComponent?.('wd-overlay')
    ?? registeredChild
    ?? null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="wd-overlay">
    <view class="scenario-header">
      <view class="scenario-title">wd-overlay</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <wd-overlay id="e2e-component" ref="e2eComponent" :show="true" @click="markInteraction"><view class="overlay-content">Overlay content</view></wd-overlay>
    </view>
