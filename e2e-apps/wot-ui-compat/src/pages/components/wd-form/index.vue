...
definePageJson({ navigationBarTitleText: 'wd-form' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['wd-form'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('wd-form')
    ?? page?.selectComponent?.('wd-form wd-form')
    ?? registeredChild
    ?? null
  const rendered = target !== null
  const commandReceiver = target
  const command = commandReceiver?.validate
  ...
  if (ok) {
    scenarioState.value = 'pass:command:validate'
  }
  else if (commandError) {
    scenarioState.value = `fail:error:${commandError}`
  }
  else if (!callable) {
    scenarioState.value = 'fail:missing-command:validate'
  }
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="wd-form">
    <view class="scenario-header">
      <view class="scenario-title">wd-form</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <wd-form id="e2e-component" ref="e2eComponent" :model="formModel" :schema="formSchema"><wd-form-item label="Name" prop="name"><wd-input v-model="formModel.name" /></wd-form-item></wd-form>
    </view>
