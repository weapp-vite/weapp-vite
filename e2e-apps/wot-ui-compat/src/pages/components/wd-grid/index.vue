...
definePageJson({ navigationBarTitleText: 'wd-grid' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['wd-grid'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('wd-grid')
    ?? page?.selectComponent?.('wd-grid wd-grid')
    ?? registeredChild
    ?? null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="wd-grid">
    <view class="scenario-header">
      <view class="scenario-title">wd-grid</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <wd-grid id="e2e-component" ref="e2eComponent" :column="2"><wd-grid-item icon="home" text="Home" /><wd-grid-item icon="user" text="Profile" /></wd-grid>
    </view>
