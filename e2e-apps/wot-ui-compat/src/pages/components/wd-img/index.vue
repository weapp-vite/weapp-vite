...
definePageJson({ navigationBarTitleText: 'wd-img' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['wd-img'].includes(child?.$options?.name))
    : null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="wd-img">
    <view class="scenario-header">
      <view class="scenario-title">wd-img</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <wd-img id="e2e-component" ref="e2eComponent" width="120" height="80" src="/assets/sample.png" mode="aspectFill" @click="markInteraction" />
    </view>
