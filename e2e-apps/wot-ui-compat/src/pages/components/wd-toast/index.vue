...
definePageJson({ navigationBarTitleText: 'wd-toast' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['wd-toast'].includes(child?.$options?.name))
    : null
  ...
  if (callable) {
    try {
      const commandResult = command.apply(commandReceiver, [{ show: true, msg: 'Stable toast', duration: 0 }])
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="wd-toast">
    <view class="scenario-header">
      <view class="scenario-title">wd-toast</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <wd-toast id="e2e-component" ref="e2eComponent" />
    </view>
