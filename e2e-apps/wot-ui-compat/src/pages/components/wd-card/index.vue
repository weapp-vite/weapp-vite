...
definePageJson({ navigationBarTitleText: 'wd-card' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['wd-card'].includes(child?.$options?.name))
    : null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="wd-card">
    <view class="scenario-header">
      <view class="scenario-title">wd-card</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <wd-card id="e2e-component" ref="e2eComponent" title="Compatibility card"><view>Stable card content</view></wd-card>
    </view>
