...
definePageJson({ navigationBarTitleText: 'wd-col' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['wd-col'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('wd-col')
    ?? page?.selectComponent?.('wd-row wd-col')
    ?? registeredChild
    ?? null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="wd-col">
    <view class="scenario-header">
      <view class="scenario-title">wd-col</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <wd-row id="e2e-parent" :gutter="12"><wd-col id="e2e-component" ref="e2eComponent" :span="12"><view class="grid-block">Column</view></wd-col></wd-row>
    </view>
