...
definePageJson({ navigationBarTitleText: 'wd-step' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['wd-step'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('wd-step')
    ?? page?.selectComponent?.('wd-steps wd-step')
    ?? registeredChild
    ?? null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="wd-step">
    <view class="scenario-header">
      <view class="scenario-title">wd-step</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <wd-steps id="e2e-parent" v-model="modelValue"><wd-step id="e2e-component" ref="e2eComponent" title="Done" /><wd-step title="Current" /></wd-steps>
    </view>
