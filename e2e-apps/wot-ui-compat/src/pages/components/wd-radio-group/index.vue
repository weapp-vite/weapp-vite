...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['wd-radio-group'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('wd-radio-group')
    ?? page?.selectComponent?.('wd-radio-group wd-radio-group')
    ?? registeredChild
    ?? null
  const rendered = target !== null
  const commandReceiver = target
  const command = commandReceiver?.updateValue
  const callable = typeof command === 'function'
  let commandError = ''
  if (callable) {
    try {
      const commandResult = command.apply(commandReceiver, ['option-b'])
  ...
  if (ok) {
    scenarioState.value = 'pass:command:updateValue'
  }
  else if (commandError) {
    scenarioState.value = `fail:error:${commandError}`
  }
  else if (!callable) {
    scenarioState.value = 'fail:missing-command:updateValue'
  }
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="wd-radio-group">
    <view class="scenario-header">
      <view class="scenario-title">wd-radio-group</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <wd-radio-group id="e2e-component" ref="e2eComponent" v-model="modelValue" @change="markInteraction"><wd-radio value="option-a">Option A</wd-radio><wd-radio value="option-b">Option B</wd-radio></wd-radio-group>
    </view>
