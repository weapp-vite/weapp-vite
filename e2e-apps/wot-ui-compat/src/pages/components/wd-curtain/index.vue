...
definePageJson({ navigationBarTitleText: 'wd-curtain' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['wd-curtain'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('wd-curtain')
    ?? page?.selectComponent?.('wd-curtain')
    ?? registeredChild
    ?? null
  const rendered = target !== null
  const commandReceiver = target
  const command = commandReceiver?.clickImage
  ...
  if (ok) {
    scenarioState.value = 'pass:command:clickImage'
  }
  else if (commandError) {
    scenarioState.value = `fail:error:${commandError}`
  }
  else if (!callable) {
    scenarioState.value = 'fail:missing-command:clickImage'
  }
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="wd-curtain">
    <view class="scenario-header">
      <view class="scenario-title">wd-curtain</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <wd-curtain id="e2e-component" ref="e2eComponent" v-model="modelValue" src="/assets/sample.png" :width="300" @click="markInteraction" />
    </view>
