...
definePageJson({ navigationBarTitleText: 'wd-button' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['wd-button'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('wd-button')
    ?? page?.selectComponent?.('wd-button')
    ?? registeredChild
    ?? null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="wd-button">
    <view class="scenario-header">
      <view class="scenario-title">wd-button</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <wd-button id="e2e-component" ref="e2eComponent" type="primary" @click="markInteraction">Primary button</wd-button>
    </view>
