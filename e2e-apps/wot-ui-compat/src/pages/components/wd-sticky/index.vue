...
definePageJson({ navigationBarTitleText: 'wd-sticky' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['wd-sticky'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('wd-sticky')
    ?? page?.selectComponent?.('wd-sticky')
    ?? registeredChild
    ?? null
  const rendered = target !== null
  const commandReceiver = target
  const command = commandReceiver?.setPosition
  const callable = typeof command === 'function'
  let commandError = ''
  if (callable) {
    try {
      const commandResult = command.apply(commandReceiver, [true, 'fixed', 16])
  ...
  if (ok) {
    scenarioState.value = 'pass:command:setPosition'
  }
  else if (commandError) {
    scenarioState.value = `fail:error:${commandError}`
  }
  else if (!callable) {
    scenarioState.value = 'fail:missing-command:setPosition'
  }
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="wd-sticky">
    <view class="scenario-header">
      <view class="scenario-title">wd-sticky</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <wd-sticky id="e2e-component" ref="e2eComponent" :offset-top="0"><view class="sticky-content">Sticky content</view></wd-sticky>
    </view>
