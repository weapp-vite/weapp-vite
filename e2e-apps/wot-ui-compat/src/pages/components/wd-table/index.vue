...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['wd-table'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('wd-table')
    ?? page?.selectComponent?.('wd-table wd-table')
    ?? registeredChild
    ?? null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="wd-table">
    <view class="scenario-header">
      <view class="scenario-title">wd-table</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <wd-table id="e2e-component" ref="e2eComponent" :data="tableData" @row-click="markInteraction"><wd-table-column prop="name" label="Name" /><wd-table-column prop="score" label="Score" /></wd-table>
    </view>
