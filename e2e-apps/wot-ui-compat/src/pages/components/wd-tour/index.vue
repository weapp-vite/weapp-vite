...
definePageJson({ navigationBarTitleText: 'wd-tour' })

const interactionCount = ref(0)
const scenarioState = ref('pending')
const e2eComponent = ref<Record<string, unknown> | null>(null)
const modelValue = ref(true)
const tourSteps = [{ element: '#e2e-tour-anchor', content: 'First step' }, { element: '#e2e-tour-anchor', content: 'Second step' }]
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['wd-tour'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('wd-tour')
    ?? page?.selectComponent?.('wd-tour')
    ?? registeredChild
    ?? null
  const rendered = target !== null
  const commandReceiver = target
  const command = commandReceiver?.handleNext
  ...
  if (ok) {
    scenarioState.value = 'pass:command:handleNext'
  }
  else if (commandError) {
    scenarioState.value = `fail:error:${commandError}`
  }
  else if (!callable) {
    scenarioState.value = 'fail:missing-command:handleNext'
  }
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="wd-tour">
    <view class="scenario-header">
      <view class="scenario-title">wd-tour</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <view><view id="e2e-tour-anchor" class="tour-anchor">Tour anchor</view><wd-tour id="e2e-component" ref="e2eComponent" v-model="modelValue" :steps="tourSteps" /></view>
    </view>
