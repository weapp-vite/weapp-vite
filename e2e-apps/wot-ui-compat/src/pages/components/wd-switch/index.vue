...
definePageJson({ navigationBarTitleText: 'wd-switch' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['wd-switch'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('wd-switch')
    ?? page?.selectComponent?.('wd-switch')
    ?? registeredChild
    ?? null
  const rendered = target !== null
  const commandReceiver = target
  const command = commandReceiver?.switchValue
  ...
  if (ok) {
    scenarioState.value = 'pass:command:switchValue'
  }
  else if (commandError) {
    scenarioState.value = `fail:error:${commandError}`
  }
  else if (!callable) {
    scenarioState.value = 'fail:missing-command:switchValue'
  }
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="wd-switch">
    <view class="scenario-header">
      <view class="scenario-title">wd-switch</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <wd-switch id="e2e-component" ref="e2eComponent" v-model="modelValue" @change="markInteraction" />
    </view>
