...
definePageJson({ navigationBarTitleText: 'wd-slide-verify' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['wd-slide-verify'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('wd-slide-verify')
    ?? page?.selectComponent?.('wd-slide-verify')
    ?? registeredChild
    ?? null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="wd-slide-verify">
    <view class="scenario-header">
      <view class="scenario-title">wd-slide-verify</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <wd-slide-verify id="e2e-component" ref="e2eComponent" />
    </view>
