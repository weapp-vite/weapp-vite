...
definePageJson({ navigationBarTitleText: 'wd-divider' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['wd-divider'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('wd-divider')
    ?? page?.selectComponent?.('wd-divider')
    ?? registeredChild
    ?? null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="wd-divider">
    <view class="scenario-header">
      <view class="scenario-title">wd-divider</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <wd-divider id="e2e-component" ref="e2eComponent">Divider label</wd-divider>
    </view>
