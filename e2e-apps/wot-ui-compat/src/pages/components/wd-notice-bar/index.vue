...
definePageJson({ navigationBarTitleText: 'wd-notice-bar' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['wd-notice-bar'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('wd-notice-bar')
    ?? page?.selectComponent?.('wd-notice-bar')
    ?? registeredChild
    ?? null
  const rendered = target !== null
  const commandReceiver = target
  const command = commandReceiver?.reset
  ...
  if (ok) {
    scenarioState.value = 'pass:command:reset'
  }
  else if (commandError) {
    scenarioState.value = `fail:error:${commandError}`
  }
  else if (!callable) {
    scenarioState.value = 'fail:missing-command:reset'
  }
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="wd-notice-bar">
    <view class="scenario-header">
      <view class="scenario-title">wd-notice-bar</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <wd-notice-bar id="e2e-component" ref="e2eComponent" text="Stable compatibility notice" :scrollable="false" />
    </view>
