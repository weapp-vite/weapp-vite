...
definePageJson({ navigationBarTitleText: 'wd-drop-menu-item' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['wd-drop-menu-item'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('wd-drop-menu-item')
    ?? page?.selectComponent?.('wd-drop-menu wd-drop-menu-item')
    ?? registeredChild
    ?? null
  const rendered = target !== null || parent !== null
  const commandReceiver = target
  const command = commandReceiver?.open
  ...
  const targetStateMatched = typeof target?.getShowPop === 'function' && Object.is(target.getShowPop(), true)
  const ok = rendered && callable && !commandError && eventMatched && stateMatched && targetStateMatched
  if (ok) {
    scenarioState.value = 'pass:command:open'
  }
  else if (commandError) {
    scenarioState.value = `fail:error:${commandError}`
  }
  else if (!callable) {
    scenarioState.value = 'fail:missing-command:open'
  }
    ...
    scenarioState.value = 'fail:target-state:getShowPop'
  }
  return {
    ok,
    component: 'wd-drop-menu-item',
    rendered,
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="wd-drop-menu-item">
    <view class="scenario-header">
      <view class="scenario-title">wd-drop-menu-item</view>
