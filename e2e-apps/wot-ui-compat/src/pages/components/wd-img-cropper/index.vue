...
definePageJson({ navigationBarTitleText: 'wd-img-cropper' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['wd-img-cropper'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('wd-img-cropper')
    ?? page?.selectComponent?.('wd-img-cropper')
    ?? registeredChild
    ?? null
  const rendered = target !== null
  const commandReceiver = target
  const command = commandReceiver?.resetImg
  ...
  if (ok) {
    scenarioState.value = 'pass:command:resetImg'
  }
  else if (commandError) {
    scenarioState.value = `fail:error:${commandError}`
  }
  else if (!callable) {
    scenarioState.value = 'fail:missing-command:resetImg'
  }
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="wd-img-cropper">
    <view class="scenario-header">
      <view class="scenario-title">wd-img-cropper</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <wd-img-cropper id="e2e-component" ref="e2eComponent" v-model="modelValue" img-src="/assets/sample.png" />
    </view>
