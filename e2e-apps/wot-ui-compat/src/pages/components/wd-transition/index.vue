...
definePageJson({ navigationBarTitleText: 'wd-transition' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['wd-transition'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('wd-transition')
    ?? page?.selectComponent?.('wd-transition')
    ?? registeredChild
    ?? null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="wd-transition">
    <view class="scenario-header">
      <view class="scenario-title">wd-transition</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <wd-transition id="e2e-component" ref="e2eComponent" :show="true" name="fade" @click="markInteraction"><view class="transition-content">Visible transition</view></wd-transition>
    </view>
