...
definePageJson({ navigationBarTitleText: 'wd-text' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['wd-text'].includes(child?.$options?.name))
    : null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="wd-text">
    <view class="scenario-header">
      <view class="scenario-title">wd-text</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <wd-text id="e2e-component" ref="e2eComponent" text="Stable text content" :lines="2" @click="markInteraction" />
    </view>
