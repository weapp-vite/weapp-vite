...
definePageJson({ navigationBarTitleText: 'wd-backtop' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['wd-backtop'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('wd-backtop')
    ?? page?.selectComponent?.('wd-backtop')
    ?? registeredChild
    ?? null
  const rendered = target !== null
  const commandReceiver = target
  const command = commandReceiver?.handleBacktop
  ...
  if (ok) {
    scenarioState.value = 'pass:command:handleBacktop'
  }
  else if (commandError) {
    scenarioState.value = `fail:error:${commandError}`
  }
  else if (!callable) {
    scenarioState.value = 'fail:missing-command:handleBacktop'
  }
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="wd-backtop">
    <view class="scenario-header">
      <view class="scenario-title">wd-backtop</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <wd-backtop id="e2e-component" ref="e2eComponent" :scroll-top="500" :top="100" text="Top" />
    </view>
