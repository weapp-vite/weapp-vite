...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['wd-radio'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('wd-radio')
    ?? page?.selectComponent?.('wd-radio-group wd-radio')
    ?? registeredChild
    ?? null
  ...
  const stateMatched = Object.is(modelValue.value, 'option-a')
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="wd-radio">
    <view class="scenario-header">
      <view class="scenario-title">wd-radio</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <wd-radio-group id="e2e-parent" v-model="modelValue"><wd-radio id="e2e-component" ref="e2eComponent" value="option-a">Option A</wd-radio></wd-radio-group>
    </view>
