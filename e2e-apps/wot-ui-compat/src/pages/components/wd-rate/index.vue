...
definePageJson({ navigationBarTitleText: 'wd-rate' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['wd-rate'].includes(child?.$options?.name))
    : null
  ...
  if (callable) {
    try {
      const commandResult = command.apply(commandReceiver, [3])
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="wd-rate">
    <view class="scenario-header">
      <view class="scenario-title">wd-rate</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <wd-rate id="e2e-component" ref="e2eComponent" v-model="modelValue" :count="5" @change="markInteraction" />
    </view>
