...
definePageJson({ navigationBarTitleText: 'wd-picker' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['wd-picker'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('wd-picker')
    ?? page?.selectComponent?.('wd-picker')
    ?? registeredChild
    ?? null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="wd-picker">
    <view class="scenario-header">
      <view class="scenario-title">wd-picker</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <wd-picker id="e2e-component" ref="e2eComponent" v-model="modelValue" :columns="columns" label="Picker" @open="markInteraction" />
    </view>
