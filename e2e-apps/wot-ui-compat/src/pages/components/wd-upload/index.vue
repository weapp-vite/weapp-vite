...
definePageJson({ navigationBarTitleText: 'wd-upload' })

const interactionCount = ref(0)
const scenarioState = ref('pending')
const e2eComponent = ref<Record<string, unknown> | null>(null)
const modelValue = ref<Array<{ url: string }>>([{ url: '/assets/sample.png' }])
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['wd-upload'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('wd-upload')
    ?? page?.selectComponent?.('wd-upload')
    ?? registeredChild
    ?? null
  const rendered = target !== null
  const commandReceiver = target
  const command = commandReceiver?.abort
  ...
  if (ok) {
    scenarioState.value = 'pass:command:abort'
  }
  else if (commandError) {
    scenarioState.value = `fail:error:${commandError}`
  }
  else if (!callable) {
    scenarioState.value = 'fail:missing-command:abort'
  }
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="wd-upload">
    <view class="scenario-header">
      <view class="scenario-title">wd-upload</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <wd-upload id="e2e-component" ref="e2eComponent" v-model:file-list="modelValue" :limit="1" />
    </view>
