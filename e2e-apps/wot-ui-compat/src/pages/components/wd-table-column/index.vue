...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['wd-table-column'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('wd-table-column')
    ?? page?.selectComponent?.('wd-table wd-table-column')
    ?? registeredChild
    ?? null
  const rendered = target !== null || parent !== null
  const commandReceiver = parent
  const command = commandReceiver?.rowClick
  const callable = typeof command === 'function'
  let commandError = ''
  if (callable) {
    try {
      const commandResult = command.apply(commandReceiver, [0])
  ...
  if (ok) {
    scenarioState.value = 'pass:command:rowClick'
  }
  else if (commandError) {
    scenarioState.value = `fail:error:${commandError}`
  }
  else if (!callable) {
    scenarioState.value = 'fail:missing-command:rowClick'
  }
  else if (!eventMatched) {
    scenarioState.value = 'fail:event:row-click'
  }
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="wd-table-column">
    <view class="scenario-header">
      <view class="scenario-title">wd-table-column</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <wd-table id="e2e-parent" :data="tableData" @row-click="markInteraction"><wd-table-column id="e2e-component" ref="e2eComponent" prop="name" label="Name" /></wd-table>
    </view>
