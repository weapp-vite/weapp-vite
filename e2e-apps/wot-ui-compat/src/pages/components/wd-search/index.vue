...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['wd-search'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('wd-search')
    ?? page?.selectComponent?.('wd-search')
    ?? registeredChild
    ?? null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="wd-search">
    <view class="scenario-header">
      <view class="scenario-title">wd-search</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <wd-search id="e2e-component" ref="e2eComponent" v-model="modelValue" placeholder="Search" @clear="markInteraction" />
    </view>
