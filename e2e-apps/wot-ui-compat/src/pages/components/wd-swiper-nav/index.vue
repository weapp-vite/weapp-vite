...
definePageJson({ navigationBarTitleText: 'wd-swiper-nav' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['wd-swiper-nav'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('wd-swiper-nav')
    ?? page?.selectComponent?.('wd-swiper wd-swiper-nav')
    ?? registeredChild
    ?? null
  const rendered = target !== null || parent !== null
  const commandReceiver = target
  const command = commandReceiver?.handleNav
  const callable = typeof command === 'function'
  let commandError = ''
  if (callable) {
    try {
      const commandResult = command.apply(commandReceiver, ['next'])
  ...
  if (ok) {
    scenarioState.value = 'pass:command:handleNav'
  }
  else if (commandError) {
    scenarioState.value = `fail:error:${commandError}`
  }
  else if (!callable) {
    scenarioState.value = 'fail:missing-command:handleNav'
  }
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="wd-swiper-nav">
    <view class="scenario-header">
      <view class="scenario-title">wd-swiper-nav</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <wd-swiper-nav id="e2e-component" ref="e2eComponent" v-model="modelValue" :total="2" @change="markInteraction" />
    </view>
