...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['wd-form-item'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('wd-form-item')
    ?? page?.selectComponent?.('wd-form wd-form-item')
    ?? registeredChild
    ?? null
  const rendered = target !== null || parent !== null
  const commandReceiver = target
  const command = commandReceiver?.validateByTrigger
  const callable = typeof command === 'function'
  let commandError = ''
  if (callable) {
    try {
      const commandResult = command.apply(commandReceiver, ['change'])
  ...
  if (ok) {
    scenarioState.value = 'pass:command:validateByTrigger'
  }
  else if (commandError) {
    scenarioState.value = `fail:error:${commandError}`
  }
  else if (!callable) {
    scenarioState.value = 'fail:missing-command:validateByTrigger'
  }
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="wd-form-item">
    <view class="scenario-header">
      <view class="scenario-title">wd-form-item</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <wd-form id="e2e-parent" :model="formModel"><wd-form-item id="e2e-component" ref="e2eComponent" label="Name" prop="name"><wd-input v-model="formModel.name" /></wd-form-item></wd-form>
    </view>
