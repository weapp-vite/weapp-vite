...
definePageJson({ navigationBarTitleText: 'wd-progress' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['wd-progress'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('wd-progress')
    ?? page?.selectComponent?.('wd-progress')
    ?? registeredChild
    ?? null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="wd-progress">
    <view class="scenario-header">
      <view class="scenario-title">wd-progress</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <wd-progress id="e2e-component" ref="e2eComponent" :percentage="64" color="#16a34a" />
    </view>
