...
definePageJson({ navigationBarTitleText: 'wd-index-anchor' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['wd-index-anchor'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('wd-index-anchor')
    ?? page?.selectComponent?.('wd-index-bar wd-index-anchor')
    ?? registeredChild
    ?? null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="wd-index-anchor">
    <view class="scenario-header">
      <view class="scenario-title">wd-index-anchor</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <wd-index-bar id="e2e-parent"><wd-index-anchor id="e2e-component" ref="e2eComponent" index="A" /><wd-cell title="Alpha" /></wd-index-bar>
    </view>
