...
definePageJson({ navigationBarTitleText: 'wd-sidebar-item' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['wd-sidebar-item'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('wd-sidebar-item')
    ?? page?.selectComponent?.('wd-sidebar wd-sidebar-item')
    ?? registeredChild
    ?? null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="wd-sidebar-item">
    <view class="scenario-header">
      <view class="scenario-title">wd-sidebar-item</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <wd-sidebar id="e2e-parent" v-model="modelValue"><wd-sidebar-item id="e2e-component" ref="e2eComponent" label="Standalone item" /></wd-sidebar>
    </view>
