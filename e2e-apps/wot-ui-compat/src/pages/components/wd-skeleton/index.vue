...
definePageJson({ navigationBarTitleText: 'wd-skeleton' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['wd-skeleton'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('wd-skeleton')
    ?? page?.selectComponent?.('wd-skeleton')
    ?? registeredChild
    ?? null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="wd-skeleton">
    <view class="scenario-header">
      <view class="scenario-title">wd-skeleton</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <wd-skeleton id="e2e-component" ref="e2eComponent" />
    </view>
