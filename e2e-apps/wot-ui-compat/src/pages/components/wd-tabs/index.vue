...
definePageJson({ navigationBarTitleText: 'wd-tabs' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['wd-tabs'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('wd-tabs')
    ?? page?.selectComponent?.('wd-tabs wd-tabs')
    ?? registeredChild
    ?? null
  const rendered = target !== null
  await new Promise<void>(resolve => setTimeout(resolve, 300))
  const commandReceiver = target
  const command = commandReceiver?.setActive
  const callable = typeof command === 'function'
  let commandError = ''
  if (callable) {
    try {
      const commandResult = command.apply(commandReceiver, ['second', false, true])
  ...
  const stateMatched = Object.is(modelValue.value, 'second')
  const targetStateMatched = true
  const ok = rendered && callable && !commandError && eventMatched && stateMatched && targetStateMatched
  if (ok) {
    scenarioState.value = 'pass:command:setActive'
  }
  else if (commandError) {
    scenarioState.value = `fail:error:${commandError}`
  }
  else if (!callable) {
    scenarioState.value = 'fail:missing-command:setActive'
  }
  else if (!eventMatched) {
    scenarioState.value = 'fail:event:change'
  }
  else if (!stateMatched) {
    scenarioState.value = 'fail:state:modelValue'
  }
  else {
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="wd-tabs">
    <view class="scenario-header">
      <view class="scenario-title">wd-tabs</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <wd-tabs id="e2e-component" ref="e2eComponent" v-model="modelValue" @change="markInteraction"><wd-tab name="first" title="First">First tab</wd-tab><wd-tab name="second" title="Second">Second tab</wd-tab></wd-tabs>
    </view>
