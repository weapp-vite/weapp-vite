...
definePageJson({ navigationBarTitleText: 'wd-icon' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['wd-icon'].includes(child?.$options?.name))
    : null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="wd-icon">
    <view class="scenario-header">
      <view class="scenario-title">wd-icon</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <wd-icon id="e2e-component" ref="e2eComponent" name="check" size="24px" color="#16a34a" @click="markInteraction" />
    </view>
