...
definePageJson({ navigationBarTitleText: 'wd-config-provider' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['wd-config-provider'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('wd-config-provider')
    ?? page?.selectComponent?.('wd-config-provider wd-config-provider')
    ?? registeredChild
    ?? null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="wd-config-provider">
    <view class="scenario-header">
      <view class="scenario-title">wd-config-provider</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <wd-config-provider id="e2e-component" ref="e2eComponent"><wd-button type="primary">Provided button</wd-button></wd-config-provider>
    </view>
