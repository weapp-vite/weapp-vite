...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['wd-floating-panel'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('wd-floating-panel')
    ?? page?.selectComponent?.('wd-floating-panel')
    ?? registeredChild
    ?? null
  const rendered = target !== null
  const commandReceiver = target
  const command = commandReceiver?.updateHeight
  const callable = typeof command === 'function'
  let commandError = ''
  if (callable) {
    try {
      const commandResult = command.apply(commandReceiver, [160])
  ...
  if (ok) {
    scenarioState.value = 'pass:command:updateHeight'
  }
  else if (commandError) {
    scenarioState.value = `fail:error:${commandError}`
  }
  else if (!callable) {
    scenarioState.value = 'fail:missing-command:updateHeight'
  }
  else if (!eventMatched) {
    scenarioState.value = 'fail:event:update:height'
  }
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="wd-floating-panel">
    <view class="scenario-header">
      <view class="scenario-title">wd-floating-panel</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <wd-floating-panel id="e2e-component" ref="e2eComponent" :anchors="floatingAnchors" :height="120" @update:height="markInteraction"><view class="floating-content">Floating panel</view></wd-floating-panel>
    </view>
