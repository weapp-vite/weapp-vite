...
definePageJson({ navigationBarTitleText: 'wd-badge' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['wd-badge'].includes(child?.$options?.name))
    : null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="wd-badge">
    <view class="scenario-header">
      <view class="scenario-title">wd-badge</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <wd-badge id="e2e-component" ref="e2eComponent" :value="8"><view class="badge-anchor">Badge</view></wd-badge>
    </view>
