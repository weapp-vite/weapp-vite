...
definePageJson({ navigationBarTitleText: 'wd-resize' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['wd-resize'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('wd-resize')
    ?? page?.selectComponent?.('wd-resize')
    ?? registeredChild
    ?? null
  const rendered = target !== null
  const commandReceiver = target
  const command = commandReceiver?.scrollToBottom
  const callable = typeof command === 'function'
  let commandError = ''
  if (callable) {
    try {
      const commandResult = command.apply(commandReceiver, [{ lastWidth: 120, lastHeight: 80 }])
  ...
  if (ok) {
    scenarioState.value = 'pass:command:scrollToBottom'
  }
  else if (commandError) {
    scenarioState.value = `fail:error:${commandError}`
  }
  else if (!callable) {
    scenarioState.value = 'fail:missing-command:scrollToBottom'
  }
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="wd-resize">
    <view class="scenario-header">
      <view class="scenario-title">wd-resize</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <wd-resize id="e2e-component" ref="e2eComponent"><view class="resize-content">Resize target</view></wd-resize>
    </view>
