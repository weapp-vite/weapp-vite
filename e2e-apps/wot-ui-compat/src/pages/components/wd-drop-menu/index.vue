...
definePageJson({ navigationBarTitleText: 'wd-drop-menu' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['wd-drop-menu'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('wd-drop-menu')
    ?? page?.selectComponent?.('wd-drop-menu wd-drop-menu')
    ?? registeredChild
    ?? null
  const rendered = target !== null
  const commandReceiver = target
  const command = commandReceiver?.toggle
  ...
  if (ok) {
    scenarioState.value = 'pass:command:toggle'
  }
  else if (commandError) {
    scenarioState.value = `fail:error:${commandError}`
  }
  else if (!callable) {
    scenarioState.value = 'fail:missing-command:toggle'
  }
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="wd-drop-menu">
    <view class="scenario-header">
      <view class="scenario-title">wd-drop-menu</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <wd-drop-menu id="e2e-component" ref="e2eComponent"><wd-drop-menu-item v-model="modelValue" :options="options" title="Option A" /></wd-drop-menu>
    </view>
