...
definePageJson({ navigationBarTitleText: 'wd-pagination' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['wd-pagination'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('wd-pagination')
    ?? page?.selectComponent?.('wd-pagination')
    ?? registeredChild
    ?? null
  const rendered = target !== null
  const commandReceiver = target
  const command = commandReceiver?.add
  ...
  if (ok) {
    scenarioState.value = 'pass:command:add'
  }
  else if (commandError) {
    scenarioState.value = `fail:error:${commandError}`
  }
  else if (!callable) {
    scenarioState.value = 'fail:missing-command:add'
  }
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="wd-pagination">
    <view class="scenario-header">
      <view class="scenario-title">wd-pagination</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <wd-pagination id="e2e-component" ref="e2eComponent" v-model="modelValue" :total="60" :page-size="10" @change="markInteraction" />
    </view>
