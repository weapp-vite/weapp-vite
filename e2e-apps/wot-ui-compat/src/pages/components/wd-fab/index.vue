...
definePageJson({ navigationBarTitleText: 'wd-fab' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['wd-fab'].includes(child?.$options?.name))
    : null
  ...
  if (ok) {
    ...
    scenarioState.value = 'fail:event:update:active'
  }
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="wd-fab">
    <view class="scenario-header">
      <view class="scenario-title">wd-fab</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <wd-fab id="e2e-component" ref="e2eComponent" @update:active="markInteraction"><wd-button size="small">Action</wd-button></wd-fab>
    </view>
