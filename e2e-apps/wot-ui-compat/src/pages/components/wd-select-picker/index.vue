...
definePageJson({ navigationBarTitleText: 'wd-select-picker' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['wd-select-picker'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('wd-select-picker')
    ?? page?.selectComponent?.('wd-select-picker')
    ?? registeredChild
    ?? null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="wd-select-picker">
    <view class="scenario-header">
      <view class="scenario-title">wd-select-picker</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <wd-select-picker id="e2e-component" ref="e2eComponent" v-model="modelValue" :columns="options" label="Select" @open="markInteraction" />
    </view>
