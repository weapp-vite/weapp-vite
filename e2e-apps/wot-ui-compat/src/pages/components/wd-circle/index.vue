...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['wd-circle'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('wd-circle')
    ?? page?.selectComponent?.('wd-circle')
    ?? registeredChild
    ?? null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="wd-circle">
    <view class="scenario-header">
      <view class="scenario-title">wd-circle</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <wd-circle id="e2e-component" ref="e2eComponent" v-model="modelValue" text="64%" />
    </view>
