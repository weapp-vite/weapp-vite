...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['wd-collapse-item'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('wd-collapse-item')
    ?? page?.selectComponent?.('wd-collapse wd-collapse-item')
    ?? registeredChild
    ?? null
  const rendered = target !== null || parent !== null
  const commandReceiver = target
  const command = commandReceiver?.updateExpand
  const callable = typeof command === 'function'
  let commandError = ''
  if (callable) {
    try {
      const commandResult = command.apply(commandReceiver, [false])
  ...
  const eventMatched = true
  const stateMatched = true
  const targetStateMatched = typeof target?.getExpanded === 'function' && Object.is(target.getExpanded(), true)
  const ok = rendered && callable && !commandError && eventMatched && stateMatched && targetStateMatched
  if (ok) {
    scenarioState.value = 'pass:command:updateExpand'
  }
  else if (commandError) {
    scenarioState.value = `fail:error:${commandError}`
  }
  else if (!callable) {
    scenarioState.value = 'fail:missing-command:updateExpand'
  }
  else if (!eventMatched) {
    scenarioState.value = 'fail:event:none'
  }
  else if (!stateMatched) {
    scenarioState.value = 'fail:state:none'
  }
  else {
    scenarioState.value = 'fail:target-state:getExpanded'
  }
  return {
    ok,
    component: 'wd-collapse-item',
    rendered,
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="wd-collapse-item">
    <view class="scenario-header">
      <view class="scenario-title">wd-collapse-item</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <wd-collapse id="e2e-parent" v-model="modelValue"><wd-collapse-item id="e2e-component" ref="e2eComponent" name="item" title="Standalone item">Expanded content</wd-collapse-item></wd-collapse>
    </view>
