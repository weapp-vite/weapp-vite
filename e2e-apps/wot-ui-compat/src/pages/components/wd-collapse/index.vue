...
definePageJson({ navigationBarTitleText: 'wd-collapse' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['wd-collapse'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('wd-collapse')
    ?? page?.selectComponent?.('wd-collapse wd-collapse')
    ?? registeredChild
    ?? null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="wd-collapse">
    <view class="scenario-header">
      <view class="scenario-title">wd-collapse</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <wd-collapse id="e2e-component" ref="e2eComponent" v-model="modelValue" @change="markInteraction"><wd-collapse-item name="item" title="Item">Expanded content</wd-collapse-item></wd-collapse>
    </view>
