...
definePageJson({ navigationBarTitleText: 'wd-steps' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['wd-steps'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('wd-steps')
    ?? page?.selectComponent?.('wd-steps wd-steps')
    ?? registeredChild
    ?? null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="wd-steps">
    <view class="scenario-header">
      <view class="scenario-title">wd-steps</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <wd-steps id="e2e-component" ref="e2eComponent" v-model="modelValue"><wd-step title="Done" /><wd-step title="Current" /></wd-steps>
    </view>
