...
definePageJson({ navigationBarTitleText: 'wd-calendar-view' })

const interactionCount = ref(0)
const scenarioState = ref('pending')
const e2eComponent = ref<Record<string, unknown> | null>(null)
const modelValue = ref(Date.UTC(2025, 0, 15))
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['wd-calendar-view'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('wd-calendar-view')
    ?? page?.selectComponent?.('wd-calendar-view')
    ?? registeredChild
    ?? null
  const rendered = target !== null
  const commandReceiver = target
  const command = commandReceiver?.scrollIntoView
  ...
  if (ok) {
    scenarioState.value = 'pass:command:scrollIntoView'
  }
  else if (commandError) {
    scenarioState.value = `fail:error:${commandError}`
  }
  else if (!callable) {
    scenarioState.value = 'fail:missing-command:scrollIntoView'
  }
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="wd-calendar-view">
    <view class="scenario-header">
      <view class="scenario-title">wd-calendar-view</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <wd-calendar-view id="e2e-component" ref="e2eComponent" v-model="modelValue" :min-date="minDate" :max-date="maxDate" />
    </view>
