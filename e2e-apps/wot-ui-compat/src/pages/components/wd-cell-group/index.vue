...
definePageJson({ navigationBarTitleText: 'wd-cell-group' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['wd-cell-group'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('wd-cell-group')
    ?? page?.selectComponent?.('wd-cell-group wd-cell-group')
    ?? registeredChild
    ?? null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="wd-cell-group">
    <view class="scenario-header">
      <view class="scenario-title">wd-cell-group</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <wd-cell-group id="e2e-component" ref="e2eComponent" title="Group"><wd-cell title="Status" value="Ready" /></wd-cell-group>
    </view>
