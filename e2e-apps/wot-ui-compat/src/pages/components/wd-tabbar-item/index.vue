...
definePageJson({ navigationBarTitleText: 'wd-tabbar-item' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['wd-tabbar-item'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('wd-tabbar-item')
    ?? page?.selectComponent?.('wd-tabbar wd-tabbar-item')
    ?? registeredChild
    ?? null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="wd-tabbar-item">
    <view class="scenario-header">
      <view class="scenario-title">wd-tabbar-item</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <wd-tabbar id="e2e-parent" v-model="modelValue" fixed="false"><wd-tabbar-item id="e2e-component" ref="e2eComponent" title="Home" icon="home" /></wd-tabbar>
    </view>
