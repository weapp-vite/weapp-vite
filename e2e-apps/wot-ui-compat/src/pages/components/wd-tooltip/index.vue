...
definePageJson({ navigationBarTitleText: 'wd-tooltip' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['wd-tooltip'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('wd-tooltip')
    ?? page?.selectComponent?.('wd-tooltip')
    ?? registeredChild
    ?? null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="wd-tooltip">
    <view class="scenario-header">
      <view class="scenario-title">wd-tooltip</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <wd-tooltip id="e2e-component" ref="e2eComponent" v-model="modelValue" content="Tooltip content" @open="markInteraction"><wd-button size="small">Tooltip</wd-button></wd-tooltip>
    </view>
