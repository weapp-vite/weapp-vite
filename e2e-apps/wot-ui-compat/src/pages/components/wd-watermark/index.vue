...
definePageJson({ navigationBarTitleText: 'wd-watermark' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['wd-watermark'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('wd-watermark')
    ?? page?.selectComponent?.('wd-watermark')
    ?? registeredChild
    ?? null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="wd-watermark">
    <view class="scenario-header">
      <view class="scenario-title">wd-watermark</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <wd-watermark id="e2e-component" ref="e2eComponent" content="Wot UI"><view class="watermark-content">Watermark area</view></wd-watermark>
    </view>
