...
definePageJson({ navigationBarTitleText: 'wd-input-number' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['wd-input-number'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('wd-input-number')
    ?? page?.selectComponent?.('wd-input-number')
    ?? registeredChild
    ?? null
  ...
  if (callable) {
    try {
      const commandResult = command.apply(commandReceiver, ['add'])
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="wd-input-number">
    <view class="scenario-header">
      <view class="scenario-title">wd-input-number</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <wd-input-number id="e2e-component" ref="e2eComponent" v-model="modelValue" :min="0" :max="5" @change="markInteraction" />
    </view>
