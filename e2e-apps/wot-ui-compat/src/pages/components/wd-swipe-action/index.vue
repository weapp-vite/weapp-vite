...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['wd-swipe-action'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('wd-swipe-action')
    ?? page?.selectComponent?.('wd-swipe-action')
    ?? registeredChild
    ?? null
  const rendered = target !== null
  const commandReceiver = target
  const command = commandReceiver?.close
  ...
  if (ok) {
    scenarioState.value = 'pass:command:close'
  }
  else if (commandError) {
    scenarioState.value = `fail:error:${commandError}`
  }
  else if (!callable) {
    scenarioState.value = 'fail:missing-command:close'
  }
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="wd-swipe-action">
    <view class="scenario-header">
      <view class="scenario-title">wd-swipe-action</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <wd-swipe-action id="e2e-component" ref="e2eComponent" v-model="modelValue"><wd-cell title="Swipe row" /><template #right><wd-button type="error">Delete</wd-button></template></wd-swipe-action>
    </view>
