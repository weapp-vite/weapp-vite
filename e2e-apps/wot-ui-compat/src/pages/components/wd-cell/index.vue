...
definePageJson({ navigationBarTitleText: 'wd-cell' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['wd-cell'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('wd-cell')
    ?? page?.selectComponent?.('wd-cell')
    ?? registeredChild
    ?? null
  const rendered = target !== null
  const commandReceiver = target
  const command = commandReceiver?.onClick
  ...
  if (ok) {
    scenarioState.value = 'pass:command:onClick'
  }
  else if (commandError) {
    scenarioState.value = `fail:error:${commandError}`
  }
  else if (!callable) {
    scenarioState.value = 'fail:missing-command:onClick'
  }
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="wd-cell">
    <view class="scenario-header">
      <view class="scenario-title">wd-cell</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <wd-cell id="e2e-component" ref="e2eComponent" title="Status" value="Ready" label="Representative cell" clickable @click="markInteraction" />
    </view>
