...
definePageJson({ navigationBarTitleText: 'wd-sidebar' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['wd-sidebar'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('wd-sidebar')
    ?? page?.selectComponent?.('wd-sidebar wd-sidebar')
    ?? registeredChild
    ?? null
  ...
  if (callable) {
    try {
      const commandResult = command.apply(commandReceiver, [1, 'Second'])
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="wd-sidebar">
    <view class="scenario-header">
      <view class="scenario-title">wd-sidebar</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <wd-sidebar id="e2e-component" ref="e2eComponent" v-model="modelValue" @change="markInteraction"><wd-sidebar-item label="First" /><wd-sidebar-item label="Second" /></wd-sidebar>
    </view>
