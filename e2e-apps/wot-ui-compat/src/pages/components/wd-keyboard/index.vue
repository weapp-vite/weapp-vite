...
definePageJson({ navigationBarTitleText: 'wd-keyboard' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['wd-keyboard'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('wd-keyboard')
    ?? page?.selectComponent?.('wd-keyboard')
    ?? registeredChild
    ?? null
  const rendered = target !== null
  const commandReceiver = target
  const command = commandReceiver?.handleClose
  ...
  if (ok) {
    scenarioState.value = 'pass:command:handleClose'
  }
  else if (commandError) {
    scenarioState.value = `fail:error:${commandError}`
  }
  else if (!callable) {
    scenarioState.value = 'fail:missing-command:handleClose'
  }
  else if (!eventMatched) {
    scenarioState.value = 'fail:event:close'
  }
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="wd-keyboard">
    <view class="scenario-header">
      <view class="scenario-title">wd-keyboard</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <wd-keyboard id="e2e-component" ref="e2eComponent" v-model="modelValue" mode="custom" @close="markInteraction" />
    </view>
