...
definePageJson({ navigationBarTitleText: 'wd-loading' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['wd-loading'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('wd-loading')
    ?? page?.selectComponent?.('wd-loading')
    ?? registeredChild
    ?? null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="wd-loading">
    <view class="scenario-header">
      <view class="scenario-title">wd-loading</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <wd-loading id="e2e-component" ref="e2eComponent" size="28px">Loading</wd-loading>
    </view>
