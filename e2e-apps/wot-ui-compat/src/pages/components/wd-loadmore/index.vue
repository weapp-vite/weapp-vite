...
definePageJson({ navigationBarTitleText: 'wd-loadmore' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['wd-loadmore'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('wd-loadmore')
    ?? page?.selectComponent?.('wd-loadmore')
    ?? registeredChild
    ?? null
  const rendered = target !== null
  const commandReceiver = target
  const command = commandReceiver?.reload
  ...
  if (ok) {
    scenarioState.value = 'pass:command:reload'
  }
  else if (commandError) {
    scenarioState.value = `fail:error:${commandError}`
  }
  else if (!callable) {
    scenarioState.value = 'fail:missing-command:reload'
  }
  else if (!eventMatched) {
    scenarioState.value = 'fail:event:reload'
  }
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="wd-loadmore">
    <view class="scenario-header">
      <view class="scenario-title">wd-loadmore</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <wd-loadmore id="e2e-component" ref="e2eComponent" state="error" error-text="Retry loading" @reload="markInteraction" />
    </view>
