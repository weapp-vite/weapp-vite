...
definePageJson({ navigationBarTitleText: 'wd-datetime-picker-view' })

const interactionCount = ref(0)
const scenarioState = ref('pending')
const e2eComponent = ref<Record<string, unknown> | null>(null)
const modelValue = ref(Date.UTC(2025, 0, 15, 8, 0))
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['wd-datetime-picker-view'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('wd-datetime-picker-view')
    ?? page?.selectComponent?.('wd-datetime-picker-view')
    ?? registeredChild
    ?? null
  const rendered = target !== null
  const commandReceiver = target
  const command = commandReceiver?.getSelectedOptions
  ...
  if (ok) {
    scenarioState.value = 'pass:command:getSelectedOptions'
  }
  else if (commandError) {
    scenarioState.value = `fail:error:${commandError}`
  }
  else if (!callable) {
    scenarioState.value = 'fail:missing-command:getSelectedOptions'
  }
  ...
  return {
    ok,
    component: 'wd-datetime-picker-view',
    rendered,
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="wd-datetime-picker-view">
    <view class="scenario-header">
      <view class="scenario-title">wd-datetime-picker-view</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <wd-datetime-picker-view id="e2e-component" ref="e2eComponent" v-model="modelValue" />
    </view>
