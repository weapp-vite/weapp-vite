...
definePageJson({ navigationBarTitleText: 'wd-checkbox' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['wd-checkbox'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('wd-checkbox')
    ?? page?.selectComponent?.('wd-checkbox')
    ?? registeredChild
    ?? null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="wd-checkbox">
    <view class="scenario-header">
      <view class="scenario-title">wd-checkbox</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <wd-checkbox id="e2e-component" ref="e2eComponent" v-model="modelValue" @change="markInteraction">Checkbox</wd-checkbox>
    </view>
