...
definePageJson({ navigationBarTitleText: 'wd-action-sheet' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['wd-action-sheet'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('wd-action-sheet')
    ?? page?.selectComponent?.('wd-action-sheet')
    ?? registeredChild
    ?? null
  const rendered = target !== null
  const commandReceiver = target
  const command = commandReceiver?.select
  const callable = typeof command === 'function'
  let commandError = ''
  if (callable) {
    try {
      const commandResult = command.apply(commandReceiver, [0, 'action'])
  ...
  if (ok) {
    scenarioState.value = 'pass:command:select'
  }
  else if (commandError) {
    scenarioState.value = `fail:error:${commandError}`
  }
  else if (!callable) {
    scenarioState.value = 'fail:missing-command:select'
  }
  else if (!eventMatched) {
    scenarioState.value = 'fail:event:select'
  }
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="wd-action-sheet">
    <view class="scenario-header">
      <view class="scenario-title">wd-action-sheet</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <wd-action-sheet id="e2e-component" ref="e2eComponent" v-model="modelValue" :actions="actions" :close-on-click-action="false" @select="markInteraction" />
    </view>
