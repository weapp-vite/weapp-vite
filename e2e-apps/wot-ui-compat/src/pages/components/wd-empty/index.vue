...
definePageJson({ navigationBarTitleText: 'wd-empty' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['wd-empty'].includes(child?.$options?.name))
    : null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="wd-empty">
    <view class="scenario-header">
      <view class="scenario-title">wd-empty</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <wd-empty id="e2e-component" ref="e2eComponent" description="No data" />
    </view>
