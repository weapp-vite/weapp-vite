...
definePageJson({ navigationBarTitleText: 'wd-count-down' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['wd-count-down'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('wd-count-down')
    ?? page?.selectComponent?.('wd-count-down')
    ?? registeredChild
    ?? null
  const rendered = target !== null
  const commandReceiver = target
  const command = commandReceiver?.start
  ...
  if (ok) {
    scenarioState.value = 'pass:command:start'
  }
  else if (commandError) {
    scenarioState.value = `fail:error:${commandError}`
  }
  else if (!callable) {
    scenarioState.value = 'fail:missing-command:start'
  }
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="wd-count-down">
    <view class="scenario-header">
      <view class="scenario-title">wd-count-down</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <wd-count-down id="e2e-component" ref="e2eComponent" :time="65000" format="mm:ss" :auto-start="false" />
    </view>
