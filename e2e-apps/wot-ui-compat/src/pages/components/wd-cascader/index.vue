...
definePageJson({ navigationBarTitleText: 'wd-cascader' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['wd-cascader'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('wd-cascader')
    ?? page?.selectComponent?.('wd-cascader')
    ?? registeredChild
    ?? null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="wd-cascader">
    <view class="scenario-header">
      <view class="scenario-title">wd-cascader</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <wd-cascader id="e2e-component" ref="e2eComponent" v-model="modelValue" :options="options" />
    </view>
