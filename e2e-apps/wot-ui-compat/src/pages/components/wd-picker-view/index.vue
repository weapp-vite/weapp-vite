...
definePageJson({ navigationBarTitleText: 'wd-picker-view' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['wd-picker-view'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('wd-picker-view')
    ?? page?.selectComponent?.('wd-picker-view')
    ?? registeredChild
    ?? null
  const rendered = target !== null
  const commandReceiver = target
  const command = commandReceiver?.getSelectedValues
  ...
  if (ok) {
    scenarioState.value = 'pass:command:getSelectedValues'
  }
  else if (commandError) {
    scenarioState.value = `fail:error:${commandError}`
  }
  else if (!callable) {
    scenarioState.value = 'fail:missing-command:getSelectedValues'
  }
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="wd-picker-view">
    <view class="scenario-header">
      <view class="scenario-title">wd-picker-view</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <wd-picker-view id="e2e-component" ref="e2eComponent" v-model="modelValue" :columns="columns" />
    </view>
