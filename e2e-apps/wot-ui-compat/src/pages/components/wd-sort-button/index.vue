...
definePageJson({ navigationBarTitleText: 'wd-sort-button' })
...
async function runE2E() {
  ...
  const registeredChild = Array.isArray(parentProxy?.children)
    ? parentProxy.children.find((child: any) => ['wd-sort-button'].includes(child?.$options?.name))
    : null
  const target = e2eComponent.value
    ...
    ?? page?.selectComponent?.('wd-sort-button')
    ?? page?.selectComponent?.('wd-sort-button')
    ?? registeredChild
    ?? null
...
<template>
  <view id="e2e-root" class="scenario-page" data-component="wd-sort-button">
    <view class="scenario-header">
      <view class="scenario-title">wd-sort-button</view>
      <view class="scenario-status">rendered / interactive</view>
    </view>
    <view id="e2e-target" class="scenario-subject">
      <wd-sort-button id="e2e-component" ref="e2eComponent" v-model="modelValue" title="Sort" @change="markInteraction" />
    </view>
