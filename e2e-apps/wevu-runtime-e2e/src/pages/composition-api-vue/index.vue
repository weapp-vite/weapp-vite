...
import { COMPOSITION_API_E2E_NAMES } from '../../shared/compositionApiCoverage'
import { buildResult, stringifyResult } from '../../shared/e2e'

const COVERAGE_API_NAMES = COMPOSITION_API_E2E_NAMES
...
const localProvideKey = 'composition-vue:local'
const globalProvideKey = 'composition-vue:global'
...
onShareAppMessage(() => {
  ...
  return { title: 'share-ok-vue' }
})
onShareTimeline(() => {
  addHook('onShareTimeline')
  return { title: 'timeline-ok-vue' }
})
onAddToFavorites(() => {
  addHook('onAddToFavorites')
  return { title: 'fav-ok-vue' }
})
onSaveExitState(() => {
...
async function waitTemplateRefReady() {
  for (let i = 0; i < 6; i += 1) {
...
async function runE2E() {
  ...
  bindModelPayload.onInput({
    detail: {
      value: 'bind-model-value-vue',
    },
  })
  bindModelInput({
    detail: {
      value: 'bind-model-value-vue-2',
    },
  })

  modelProps.titleModel = '  from-props-vue  '
  modelRef.value = '  emit-next-vue  '

  apiStore.inc()
  await nextTick()

  const currentPages = getCurrentPages() as any[]
  const target = currentPages[currentPages.length - 1] as any
  ...
  callHookList(target, 'onTabItemTap', [{ index: 0, pagePath: 'pages/composition-api-vue/index' }])
  ...
  const coverage: Record<(typeof COVERAGE_API_NAMES)[number], boolean> = {
    ...
    customRef: custom.value === 'beta-vue',
    ...
    onShareAppMessage: shareResult?.title === 'share-ok-vue',
    onShareTimeline: timelineResult?.title === 'timeline-ok-vue',
    onAddToFavorites: favoritesResult?.title === 'fav-ok-vue',
  ...
  const result = buildResult('composition-api-vue', coverage, {
...
<template>
  <view class="page">
    <view id="api-ref-vue" ref="apiRef" class="anchor">
      composition api vue anchor
    </view>
    <view class="title">
