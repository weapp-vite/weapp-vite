import type { MiniProgramPageIdentityRule, MiniProgramRuntimeCapabilities, MiniProgramRuntimeCapabilityName, MpPlatform } from '../types'
import type { MiniProgramRuntimeDescriptor } from './descriptors'
import { DEFAULT_RUNTIME_CAPABILITIES, MINI_PROGRAM_RUNTIME_DESCRIPTORS } from './descriptors'

const LEADING_SLASH_RE = /^\/+/

function createMiniProgramPlatformRegistry(descriptors: readonly MiniProgramRuntimeDescriptor[]) {
  const descriptorById = new Map<MpPlatform, MiniProgramRuntimeDescriptor>()
...
} = createMiniProgramPlatformRegistry(MINI_PROGRAM_RUNTIME_DESCRIPTORS)

const SUPPORTED_MINI_PROGRAM_PLATFORMS = Object.freeze(
  MINI_PROGRAM_RUNTIME_DESCRIPTORS.map(descriptor => descriptor.id),
) as readonly MpPlatform[]
const ORDERED_RUNTIME_GLOBAL_KEYS = Object.freeze(
  Array.from(new Set(MINI_PROGRAM_RUNTIME_DESCRIPTORS.map(descriptor => descriptor.runtime.globalObjectKey))),
) as readonly string[]
...
function getOrderedRuntimeGlobalKeysByPriority(
  ...
  const ordered = [...MINI_PROGRAM_RUNTIME_DESCRIPTORS].sort((left, right) => {
    ...
    return MINI_PROGRAM_RUNTIME_DESCRIPTORS.indexOf(left) - MINI_PROGRAM_RUNTIME_DESCRIPTORS.indexOf(right)
  })
 ...
 * @description 获取平台描述。
 */
export function getMiniProgramRuntimeDescriptor(platform: MpPlatform): MiniProgramRuntimeDescriptor {
...
export function getMiniProgramRuntimeGlobalKey(platform: MpPlatform): string {
  return getMiniProgramRuntimeDescriptor(platform).runtime.globalObjectKey
}
...
export function getMiniProgramRuntimeHostConfigKey(platform?: MpPlatform): string {
  ...
  return getMiniProgramRuntimeDescriptor(platform).runtime.hostConfigKey
}
...
export function getMiniProgramRuntimeCapabilities(platform?: MpPlatform): MiniProgramRuntimeCapabilities {
  ...
  return getMiniProgramRuntimeDescriptor(platform).runtime.capabilities
}
...
function getMiniProgramPageIdentityRules(platform?: MpPlatform) {
  if (platform) {
    return getMiniProgramRuntimeDescriptor(platform).runtime.pageIdentityRules
  }
  const merged = new Map<string, MiniProgramPageIdentityRule>()
  for (const descriptor of MINI_PROGRAM_RUNTIME_DESCRIPTORS) {
