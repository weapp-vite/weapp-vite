...
`@weapp-core/api` 提供跨平台的小程序 API 封装，默认推荐 Promise 风格，同时兼容传统回调风格。

## 特性

- 不依赖 UI 框架、编译器或特定小程序 runtime
- 动态代理，覆盖微信小程序全量 API
...
`@weapp-core/api` 的默认实例 `api` 会同时对齐，`wpi` 作为兼容名称继续可用：
...
| --- | --- | --- | --- | --- | --- | --- |
| 微信小程序 (`wx`) | 495 | 495 | 0 | 495 | 100.00% | 100.00% |
| 支付宝小程序 (`my`) | 201 | 201 | 0 | 495 | 40.61% | 40.61% |
| 抖音小程序 (`tt`) | 133 | 133 | 0 | 495 | 26.87% | 26.87% |
| 三端可调用完全对齐 (wx/my/tt) | 117 | - | - | 495 | 23.64% | - |
| 三端语义完全对齐 (wx/my/tt) | - | 117 | - | 495 | - | 23.64% |
...
| `getSystemInfoAsync` | 异步获取系统信息。 | 直连 `wx.getSystemInfoAsync` | 映射到 `my.getSystemInfo` | 映射到 `tt.getSystemInfo` | ⚠️ |
...
| `closeBLEConnection` | 断开低功耗蓝牙连接。 | 直连 `wx.closeBLEConnection` | `deviceId` 对齐后映射到 `my.disconnectBLEDevice`，并将 `errorCode/errorMessage` 映射为 `errCode/errMsg` | 无同等 API，调用时按 unsupported 报错 | ⚠️ |
| `createBLEConnection` | 创建低功耗蓝牙连接。 | 直连 `wx.createBLEConnection` | `deviceId/timeout` 对齐后映射到 `my.connectBLEDevice`，并将 `error/errorMessage` 映射为 `errCode/errMsg` | 无同等 API，调用时按 unsupported 报错 | ⚠️ |
...
```ts
import { api } from '@weapp-core/api'
...
```ts
import { createApi } from '@weapp-core/api'
