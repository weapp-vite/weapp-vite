...
| 支付宝独有方法数（不在小程序基准命名） | ${myOnlyMethods.length} |
| 抖音独有方法数（不在小程序基准命名） | ${ttOnlyMethods.length} |
| 支付宝可按小程序基准命名调用的方法数 | ${alipaySupportedMethods.length} |
| 支付宝语义对齐方法数 | ${alipaySemanticAlignedMethods.length} |
| 支付宝 fallback 方法数 | ${alipayFallbackMethods.length} |
| 抖音可按小程序基准命名调用的方法数 | ${douyinSupportedMethods.length} |
...
| 三端命名交集方法数（miniProgram∩my∩tt） | ${intersectionCoverage.totalApis} |
...
| 三端可调用完全对齐 (miniProgram/my/tt) | ${report.fullyAlignedApis} | - | - | ${report.totalApis} | ${report.fullyAlignedCoverage} | - |
| 三端语义完全对齐 (miniProgram/my/tt) | - | ${report.fullySemanticallyAlignedApis} | - | ${report.totalApis} | - | ${report.fullySemanticallyAlignedCoverage} |
| 三端命名交集可调用完全对齐 (miniProgram∩my∩tt) | ${intersectionCoverage.fullyAlignedApis} | - | - | ${intersectionCoverage.totalApis} | ${intersectionCoverage.fullyAlignedCoverage} | - |
| 三端命名交集语义完全对齐 (miniProgram∩my∩tt) | - | ${intersectionCoverage.fullySemanticallyAlignedApis} | - | ${intersectionCoverage.totalApis} | - | ${intersectionCoverage.fullySemanticallyAlignedCoverage} |
...
- \`pnpm --filter @weapp-core/api build\`
- \`pnpm --filter @weapp-core/api test\`
- \`pnpm --filter @weapp-core/api test:types\`
...
- 支付宝侧不兼容（按小程序基准命名调用失败）方法：${alipayUnsupportedMethods.length}
- 抖音侧不兼容（按小程序基准命名调用失败）方法：${douyinUnsupportedMethods.length}
