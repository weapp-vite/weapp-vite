...
1. `3` 次：`[runtime/runtime/error] [wevu] 模板运行时表达式执行失败: __wv_bind_0 = JSON.stringify(state.requestLog) {}`
   - 示例原始行：`[error] [runtime:runtime] [wevu] 模板运行时表达式执行失败: __wv_bind_0 = JSON.stringify(state.requestLog) {}`
...
1. `1` 次：`[runtime/launch-bridge/info] connected=ws://127.0.0.1:50576`
   - 示例原始行：`[info] [runtime:launch-bridge] connected=ws://127.0.0.1:50576`
