...
weapp-vite 若要支持标准 React 18、函数组件和 Hooks，应该采用与 Taro 3/4 同类的运行时渲染架构：
...
app.tsx / page.tsx
        |
        v
React JSX automatic transform (Oxc)
        |
        v
React 18 + react-reconciler
        |
        v
...
不新增独立 React compiler 包。通用模板生成依赖 weapp-vite 的平台、entry、JSON 和 emit context，放到运行时包会形成反向依赖。
...
构建转换使用 Vite 8 的 `transformWithOxc` 或等价 Oxc plugin，不新增 Babel JSX 热路径，也不使用已废弃的 `transformWithEsbuild`。
...
React CommonJS entry 依赖 `process.env.NODE_ENV` 选择 production build。Spike 的第一次构建未显式注入该常量，主包报告为 610 KB；注入 production 并启用 minify 后降到 114 KB。
...
该结果只包含 4 种 host component 和 5 层模板，不能直接外推完整 catalog。正式实现必须做 component usage pruning。
...
测试过程中首次真实运行暴露 production HostConfig 缺少 `insertBefore` / `insertInContainerBefore`。Node 测试只覆盖尾部 append，因此新增 keyed prepend 单测。正式包必须用 HostConfig contract test 覆盖所有 mutation 方法，不能只依赖常见页面行为。
...
- 新建 `@weapp-vite/react`。
- 固定 React 18 与 reconciler 0.29 兼容矩阵。
...
- React 18 function/class component。
...
- 简单页面 React 相关 JS gzip 目标不高于 45 KB。
...
- React 18 固定 peer range；React 19 需要新的 reconciler 适配和完整 contract regression，不在同一首版顺带支持。
...
8. 首版微信 + React 18，React 19 和多平台后续单独验收。
