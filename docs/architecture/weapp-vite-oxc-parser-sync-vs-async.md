...
`weapp-vite` 当前在 [`packages/ast/src/engines/oxc.ts`](../../packages/ast/src/engines/oxc.ts) 中继续使用 `parseSync`，这是符合上游实现建议的。

## 上游实现依据

`oxc-parser` 在 `node_modules/oxc-parser/src-js/index.js` 的注释里明确说明：
...
- 样本文件：[`packages/weapp-vite/src/runtime/autoImport/weappBuiltinHtmlTagsData.ts`](../../packages/weapp-vite/src/runtime/autoImport/weappBuiltinHtmlTagsData.ts)
- 文件大小：约 `106 KB`
- 每轮迭代：`60` 次
- 轮数：`5`
