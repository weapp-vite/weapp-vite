...
| `export`           | `defineComponent({ export() { return ... } })`（或配合 `setup(_, { expose })`）                                                   | 原生导出用于 `behavior: wx://component-export`。在 wevu 里可直接写 `export()`；若希望“按 Vue 风格暴露”，可在 `setup` 里 `expose({ ... })`，再在 `export()` 里返回 `this.__wevuExposed ?? {}`。 |
