...
description: wevu/router 完整 API 参考，覆盖路由编译宏、入口函数、Router 实例、导航守卫、动态路由和公开类型。
outline:
  level: [2, 2]
keywords:
  - wevu/router
...
本页对应 `wevu/router` 的运行时公开导出、公开类型，以及由编译器识别并擦除的 `definePage()` 宏。Wevu Router 对齐 Vue Router 的主要使用心智，但最终导航仍受小程序页面栈、tabBar 和宿主 API 约束。
