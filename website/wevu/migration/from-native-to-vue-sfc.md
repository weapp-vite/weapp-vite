...
description: 这是一份面向实际项目的迁移手册，不是概念介绍。目标是让你在不重写业务的前提下，按需迁移到 Weapp-vite + 原生，或继续升级到 Weapp-vite + Wevu + Vue SFC。
keywords:
  - Wevu
  - Vue SFC
  - 微信小程序
...
这是一份面向实际项目的迁移手册，不是概念介绍。目标是让你在**不重写业务**的前提下，把原生小程序稳定迁移到 `weapp-vite + 原生`，或在同一条迁移链路上继续升级到 `weapp-vite + Wevu + Vue SFC`。

适用场景：

- 线上已有原生小程序，想先保留 `Page/Component + WXML/WXSS/JSON`，只升级构建、TypeScript、调试、AI 协作和验收链路。
- 线上已有原生小程序，想分批把页面或组件迁移到 `Wevu + Vue SFC`，但不能一次性重写。
- 新项目计划直接采用 Vue SFC 写法，同时保留小程序原生能力。

## 先选迁移目标

不要把“接入 Weapp-vite”误解成“必须立刻 Vue 化”。存量项目通常有两条合理路线：

| 路线 | 目标组合                      | 适合项目                                                                                                      | 完成标准                                                                          |
| ---- | ----------------------------- | ------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------- |
| A    | `weapp-vite + 原生`           | 团队希望继续写原生 `Page/Component`，但想获得 Vite 构建、TS、别名、资源处理、DevTools、截图日志和 AI 维护能力 | 原生页面不改写也能稳定 dev/build/open，关键链路可验证，新增页面仍可按原生方式交付 |
| B    | `weapp-vite + Wevu + Vue SFC` | 团队希望逐步引入组合式 API、响应式状态、SFC 组织方式和更类型化的组件契约                                      | 原生与 SFC 能共存，目标页面族迁到 `.vue`，核心状态和事件契约完成语义升级          |

推荐判断：

- 如果当前最大痛点是工程效率、构建速度、路径别名、TS、调试和 AI 协作，先走路线 A。
- 如果当前最大痛点是页面脚本膨胀、`setData` 大对象、组件契约隐式、状态难维护，再在路线 A 的基础上推进路线 B。
- 如果项目里有复杂插件、`wxs/sjs`、云开发、地图/直播/支付、老旧 Behavior 或大量宿主 API，先保留原生资产，等验证闭环稳定后再选页面族迁到 Wevu。
- 同一个项目可以长期混合：核心老页面保留原生，新页面或低风险页面使用 SFC。

## 迁移目标与验收标准

建议先明确“迁移完成”意味着什么，避免路线 A 被误收敛成路线 B，也避免路线 B 只完成语法替换。

路线 A 的验收标准：

- 原生页面、组件、分包、插件和 npm 依赖在 Weapp-vite 构建链路下行为等价。
- `dev/build/open` 或项目内等价脚本稳定可用。
- `wv prepare` 生成的 `.weapp-vite` 支持文件、AI 指引和本地随包文档接入稳定。
- 路径别名、环境变量、静态资源、DevTools 项目配置和 CI 命令已对齐。
- 至少一条关键链路有可复现的构建、打开、日志或截图验收。
...
```mermaid
flowchart LR
  A[阶段 0<br/>原生项目体检] --> B[阶段 1<br/>接入 weapp-vite]
  B --> C{是否继续迁到 Wevu?}
  C -->|否| D[路线 A<br/>原生链路收口]
  C -->|是| E[路线 B<br/>SFC 试点与共存]
  E --> F[页面族迁移]
  F --> G[语义升级与回归]
```
...
- `app.json`、页面路由、tabBar、分包和插件。
- 自定义组件、Behavior、`wxs/sjs`、npm 构建和静态资源。
- 云开发、地图/直播/支付等强宿主能力。
- DevTools 打开方式、CI 命令、预览/上传流程。
- 至少一条关键链路的入口、操作、接口和已知异常。
...
如果目标只是路线 A，这一步不是“过渡态”，而是可以作为阶段性交付终点。

### 阶段 2A：路线 A 收口（继续原生）

保留原生页面和组件，重点收口工程能力：

- 把 `vite.config.ts`、脚本、输出目录和 DevTools 配置写成团队稳定入口。
- 让 `.js/.ts/.wxml/.wxss/.json` 原生资产在 Weapp-vite 下继续由构建链路处理。
- 对齐 `wv prepare`、`.weapp-vite` 类型支持文件、AI 指引、本地 `dist/docs` 阅读顺序。
- 关键页面补截图、日志或轻量 e2e，证明迁移后不是“只能构建，不能运行”。
- 新页面仍可按原生方式新增；需要 SFC 时再显式进入路线 B。
...
| 波次   | 范围           | 目标                           | 退出条件                         |
| ------ | -------------- | ------------------------------ | -------------------------------- |
| Wave 0 | 原生项目体检   | 明确路线 A / B、风险资产和基线 | 迁移边界、回滚点、关键路径清楚   |
| Wave 1 | 工具链接入     | 接入 Weapp-vite，业务 0 改动   | `build` 可过，首页可打开         |
| Wave 2 | 路线 A 收口    | 原生页面继续稳定交付           | dev/build/open/日志或截图闭环    |
| Wave 3 | 路线 B 试点    | 低风险页或新页面迁移到 SFC     | 原生页和 SFC 页互跳，冒烟通过    |
| Wave 4 | 页面族迁移     | 表单页、筛选页、详情页等页面族 | 正/反向流程通过，参数健壮性通过  |
| Wave 5 | 高风险链路收口 | 下单、支付、售后等核心路径     | 关键链路 e2e 通过 + 真机回归通过 |
| Wave 6 | 语义升级       | 清理遗留 `setData` 大对象写法  | 编码规范统一，迁移文档补齐       |

每个波次建议都固定执行这 4 步：

1. 迁移前记录基线行为（页面入口、关键交互、关键接口）。
2. 明确本轮属于路线 A 还是路线 B。
3. 路线 A 只改工程入口，路线 B 再做 `.vue` 机械迁移和语义升级。
4. 做定向验证并沉淀坑位清单。
...
根因：误以为迁移到 Weapp-vite 后必须继续迁到 Wevu / Vue SFC。

修复：如果项目当前只需要工程化收益，就把 `weapp-vite + 原生` 作为明确交付目标，等页面维护成本或新功能形态需要时再进入路线 B。
...
1. 原生页面和组件保持 `js/wxml/wxss/json` 结构。
2. 接入或修正 `vite.config.ts` 的 `weapp` 配置。
3. 对齐路径别名、环境变量、静态资源、npm 构建和输出目录。
4. 运行 `wv prepare`，确认 `.weapp-vite` 支持文件稳定。
5. build 并在 DevTools 打开至少一条关键路径。
...
如果目标是路线 A，当以下条件都成立，迁移可以视为完成：

- 原生页面和组件仍使用原有小程序模型，但已稳定纳入 Weapp-vite 构建链路。
- `dev/build/open`、路径别名、环境变量、资源处理和 DevTools 项目配置可复现。
- `wv prepare`、本地随包文档和项目 AI 指引已对齐。
- 至少一条关键链路有构建、打开、日志或截图验收。

如果目标是路线 B，当以下条件都成立，迁移可以视为完成：
