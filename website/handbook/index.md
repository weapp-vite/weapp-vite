...
<!-- tutorial-e2e:handbook-wevu-counter:start -->
...
打开脚手架已经生成的 `src/pages/index/index.vue`，把内容替换为下面这个最简单的计数器：

<!-- tutorial-source: website/snippets/tutorial-e2e/handbook-counter.vue -->

<<< ../snippets/tutorial-e2e/handbook-counter.vue
...
Wevu 模板已经在 `src/app.vue` 中启用了自动路由：
...
因此 `src/pages/index/index.vue` 会自动成为 `pages/index/index` 路由，不需要额外创建或维护 `src/app.json`。如果你关闭了自动路由，再改为在应用配置中显式维护页面列表。
...
| 症状             | 先查什么                                                           |
| ---------------- | ------------------------------------------------------------------ |
| 页面白屏         | `src/app.vue` 是否仍使用 `autoRoutes.pages`                        |
| 点按钮没反应     | `ref` 是不是从 `wevu` 导入的                                       |
| 数据改了页面没变 | 同上，从 `vue` 导入的 `ref` 在小程序里不会触发更新                 |
| 标题没生效       | 看 `dist/pages/index/index.json` 里有没有 `navigationBarTitleText` |
...
<!-- tutorial-e2e:handbook-wevu-counter:end -->
