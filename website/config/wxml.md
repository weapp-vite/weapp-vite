...
微信旧式模板的双层反斜杠转义与 glass-easel / 其他平台的 XML 引号边界分别处理，不会改写保留的属性值。微信编译器语法按 `componentFramework` 及组件、模板依赖继承确定，不以 `glassEaselWebview` 等运行时检测结果统一切换所有页面。完整构建替换编译输入，局部 HMR 更新仅合并当前输入，避免沿用过期配置。
