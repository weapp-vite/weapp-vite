...
新项目如果还需要输出微信、支付宝、抖音、百度、京东和小红书，可以按源码风格选择原生 `multi-platform` 或 Wevu + Vue SFC 的 `multi-platform-sfc` 模板。两者都配置了独立的 Web 脚本和输出目录，完整工作流见[多平台构建指南](/guide/multi-platform)。
