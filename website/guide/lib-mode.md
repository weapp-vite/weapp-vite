...
这样可以避免把 `wevu`、`wevu/router`、`wevu/api` 等运行时子路径直接打进组件库产物，同时保留本地开发与类型检查所需的依赖。
...
### 发布到 npm 时，为什么不只写 `external: ['wevu']`？

因为组件库内部可能还会引用 `wevu/router`、`wevu/api`、`wevu/fetch` 等子路径。只 external 根包时，这些子路径仍可能被打进最终产物。推荐直接使用：
