...
> [!TIP]
> 默认场景建议直接使用原生 `resolve.tsconfigPaths`。如果需要指定多个 `tsconfig` 或忽略某些目录，再在 `vite.config.ts` 或 `weapp-vite.config.ts` 中通过 [`weapp.tsconfigPaths`](/config/js.md#weapp-tsconfigpaths) 传入对象启用 `vite-tsconfig-paths` 的高级选项。
...
- **同时使用多个 `tsconfig`？** 可以在 `vite.config.ts` 或 `weapp-vite.config.ts` 中配置 [`weapp.tsconfigPaths.projects`](../config/js.md#weapp-tsconfigpaths) 指定额外的配置文件，让 monorepo 子包共享同一套别名。
