...
`weapp-vite` 默认沿用原生小程序的思路：`app.*` 仍放在项目根 `src/` 下，业务页面位于 `pages/`，分包在 `packages/`。在此基础上框架约定了一些“可选但推荐”的目录，让自动路由、自动导入组件与分包增强可以即插即用。
...
> 大多数模板会把 `weapp.srcRoot` 显式设置为 `src/`，以便与下述目录约定配套。如果你的源码位于其他目录（如 `miniprogram/`），只需在 `vite.config.ts` 中调整 `weapp.srcRoot`，其余能力依旧可用。
