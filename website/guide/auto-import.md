...
外部组件是 uni-app Vue SFC 时，resolver 还需要返回 `sourceType: 'wevu-sfc'` 与真实 `resolvedId`，并用 `weapp.uniApp.include` 明确允许参与方言转换的 npm 包。Wot UI 2.2.0 与 uview-plus 3.8.112 可以分别使用内置 `WotUiResolver()`、`UviewPlusResolver()`；完整配置与兼容矩阵见 [Wot UI 与 uni-app 组件库](/integration/wot-ui) 和 [uview-plus 与 uni-app 组件库](/integration/uview-plus)。
