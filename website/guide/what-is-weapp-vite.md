...
- **开发、构建与上传**：`wv dev/build/prepare/analyze/open` 负责工程主链路，`wv upload` 提供六端构建并上传（不含提审和正式发布）。
- **IDE 自动化**：通过 `weapp-ide-cli` 透传 `preview/config/screenshot/compare` 等命令；微信 IDE 上传使用 `wv ide upload`。
