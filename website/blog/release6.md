...
- **vue-mini**：做得很好，纯运行时库，Vue 3 语法写小程序。但深度绑定其工具链，和 Web Vue 代码不能复用。
...
### wevu vs vue-mini：两个小程序运行时的选择

wevu 和 vue-mini 都是小程序的 Vue 3 风格运行时，但设计路径不同。

| 对比维度 | wevu | vue-mini |
|:---|:---|
| **渲染方式** | Snapshot Diff → setData | Virtual DOM Diff → setData |
| **数据模型** | Data Snapshots（纯对象） | VNode 树 |
| **组件定义** | `createWevuComponent` | `defineComponent` |
| **SFC 支持** | weapp-vite 内置编译器 | 需要配置 webpack loader |
| **状态管理** | 内置 `defineStore` | 需要额外集成 Pinia |
| **独立使用** | ✅ 可单独作为运行时库 | ❌ 深度绑定 vue-mini 工具链 |
| **包体积** | 更轻量（无 VNode 层） | 较大（完整 Virtual DOM） |

**渲染层差异**

wevu 采用快照 Diff，直接对数据对象做比较：

```ts
// wevu 的思路
const oldData = { count: 0, name: 'test' }
const newData = { count: 1, name: 'test' }

// diff 结果：{ count: 1 }
// 调用：setData({ count: 1 })
```

vue-mini 保留 Virtual DOM，需要先构建 VNode 再 diff：

```ts
// vue-mini 的思路
const oldVNode = h('view', { count: 0 }, ...)
const newVNode = h('view', { count: 1 }, ...)

// VNode Diff → 计算出变化 → setData
```
...
```ts
// vue-mini（需要配合其工具链）
import { ref } from 'vue-mini'

const count = ref(0)
```

API 层面两者高度一致，主要差异在于底层渲染策略和工具链集成方式。
...
- **vue-mini**：纯运行时库，如果你只需要运行时支持、不需要 SFC 编译，它是不错的选择。weapp-vite + wevu 则提供了编译时 + 运行时的完整方案，且 wevu 可独立使用。
