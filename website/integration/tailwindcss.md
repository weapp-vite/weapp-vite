...
显式设置 `tailwindcss: false` 可以关闭自动检测。`cssEntries` 只声明入口集合，不能代替 CSS 模块图导入。内置 compiler 会统一处理 WXSS、WXML 和 JavaScript，并默认完成 WXSS 最终化，确保 Tailwind 的 `@plugin`、`@source` 等构建阶段指令不会进入产物。HMR 直接把真实变更文件交给 compiler，并根据 `@source` glob 精确失效关联 root。
