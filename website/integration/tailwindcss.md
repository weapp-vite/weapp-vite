...
`weapp-vite` 已内置 `weapp-tailwindcss@5.4.0` 的 compiler 集成，不需要安装或注册 `weapp-tailwindcss/vite`。
...
显式设置 `tailwindcss: false` 可以关闭自动检测。`cssEntries` 只声明入口集合，不能代替 CSS 模块图导入。内置 compiler 会统一处理 WXSS、WXML 和 JavaScript，并清理 Tailwind 的 `@plugin`、`@source` 等只适用于构建阶段的指令。
